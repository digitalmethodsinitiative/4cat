...
- The version of the checked-out code is on the first line of the `VERSION` file. Read it rather than relying on a number quoted elsewhere. **Do not edit this file casually.**
- The version a given instance was last *migrated* to is on the first line of `config/.current-version`. The two differ whenever code is newer than the database it is pointed at, which is exactly what `migrate.py` acts on.
