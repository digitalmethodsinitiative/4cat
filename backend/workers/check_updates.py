...
class UpdateChecker(BasicWorker):
    ...
    def work(self):
        ...
        try:
            ...
            self.log.warning(
                "'4cat.github_url' may be misconfigured - repository does not exist or is private"
            )
            return
        except requests.Timeout:
            self.log.warning(
                f"GitHub URL '4cat.github_url' did not respond within {timeout} seconds - not checking for new version"
            )
            return
        ...
        if packaging.version.parse(latest_tag) > packaging.version.parse(
            current_version
        ):
            # update available!
            ...
            self.db.execute(
                "DELETE FROM users_notifications WHERE username = '!admin' "
