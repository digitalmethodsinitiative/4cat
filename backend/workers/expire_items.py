...
class ThingExpirer(BasicWorker):
	...
	def expire_users(self):
		...
		for expiring_user in expiring_users:
			...
			if re.match(r"[0-9]{4}-[0-9]{2}-[0-9]{2}", userdata["delete-after"]):
				expires_at = datetime.datetime.strptime("%Y-%m-%d", userdata["delete-after"])
			elif re.match(r"[0-9]+", userdata["delete-after"]):
				expires_at = datetime.datetime.fromtimestamp(int(userdata["delete-after"]))
			else:
				self.log.warning(f"User {username} has invalid expiration date {userdata['delete-after']}")
				continue
