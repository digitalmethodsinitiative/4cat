...
class InternalAPI(BasicWorker):
    ...
    def work(self):
            ...
            try:
                ...
                if has_time and not self.interrupted:
                    self.manager.log.info(
                        "Could not open port %i yet (%s), retrying in 10 seconds"
                        % (self.port, e)
                    )
                    time.sleep(10.0)  # wait a few seconds before retrying
                    continue
                self.manager.log.error(
                    "Port %s is already in use! Local API not available. Check if a residual 4CAT process may still be listening at the port."
                    % self.port
                )
                return
            except ConnectionRefusedError:
                self.manager.log.error(
                    "OS refused listening at port %i! Local API not available."
                    % self.port
                )
                return
        ...
        server.settimeout(2)
        self.manager.log.info(
            "Local API listening for requests at %s:%s" % (self.host, self.port)
        )
    ...
    def process_request(self, request, payload):
        ...
        if request == "cancel-job":
            ...
            self.manager.request_interrupt(
                remote_id=remote_id, jobtype=jobtype, interrupt_level=level
            )
            return "OK"
        ...
        if request == "datasets":
            ...
            items = self.db.fetchall(
                "SELECT * FROM datasets WHERE timestamp > %s ORDER BY timestamp ASC",
                (now - week,),
            )

            response = {"1h": 0, "1d": 0, "1w": 0}
        ...
        if request == "worker-status":
            ...
            open_jobs = self.db.fetchall(
                "SELECT jobtype, timestamp, timestamp_claimed, timestamp_lastclaimed, interval, remote_id FROM jobs ORDER BY jobtype ASC, timestamp ASC, remote_id ASC"
            )
            running = []
            queue = {}
            ...
            for job in open_jobs:
                try:
                    worker = list(
                        filter(
                            lambda worker: worker.job.data["jobtype"] == job["jobtype"]
                            and worker.job.data["remote_id"] == job["remote_id"],
                            self.manager.worker_pool.get(job["jobtype"], []),
                        )
                    )[0]
                except IndexError:
                    worker = None
                    ...
                    running.append(
                        {
                            ...
                            "is_maybe_crashed": job["timestamp_claimed"] > 0
                            and not worker,
            ...
            return {"running": running, "queued": queue}
