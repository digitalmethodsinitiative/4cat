...
class ProcessorAdvancedPreset(ProcessorPreset):
    ...
    def get_processor_pipeline(self):
        ...
        processors are run. Compatibility of processors in the list is not
        """
        advanced_pipeline = self.get_processor_advanced_pipeline(
            attach_to=self.dataset.key
        )
        ...
        if not any(
            "attach_to" in processor["parameters"] for processor in all_processors
        ):
            raise ValueError(
                "No processor in the advanced pipeline has the attach_to parameter"
            )
