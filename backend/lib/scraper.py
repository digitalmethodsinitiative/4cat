    ...
    def work(self):
        ...
        if "file" in self.job.details:
            ...
            if not local_path.exists():
                self.job.finish()
                self.log.error(
                    "Scraper was told to use source file %s, but file does not exist, cancelling job."
                    % self.job.details["file"]
                )
                return

            with local_path.open() as source:
                datafields = {"status_code": 200, "content": source.read()}

                data = collections.namedtuple("object", datafields.keys())(
                    *datafields.values()
                )
        else:
            ...
            try:
                ...
                if (
                    protocol in config.get("SCRAPE_PROXIES", [])
                    and config.get("SCRAPE_PROXIES")[protocol]
                ):
                    proxies = {
                        protocol: random.choice(config.get("SCRAPE_PROXIES")[protocol])
                    }
                else:
                    proxies = None
                ...
                data = requests.get(
                    url,
                    timeout=config.get("SCRAPE_TIMEOUT", 60),
                    proxies=proxies,
                    headers={
                ...
                if self.job.data["attempts"] > 2:
                    self.job.finish()
                    self.log.error(
                        "Could not finish request for %s (%s), cancelling job"
                        % (url, e)
                    )
                else:
                    self.job.release(delay=random.randint(45, 60))
                    self.log.info(
                        "Could not finish request for %s (%s), releasing job" % (url, e)
                    )
                return
        ...
        if data.status_code == 404 or data.status_code == 403:
            ...
            if parsed_data is None:
                if self.job.data["attempts"] < 2:
                    self.log.info(
                        "Data for %s %s could not be parsed, retrying later"
                        % (self.type, id)
                    )
                    self.job.release(
                        delay=random.choice(range(15, 45))
                    )  # try again later
                else:
                    self._logger_method(
                        "Data for %s %s could not be parsed after %i attempts, aborting"
