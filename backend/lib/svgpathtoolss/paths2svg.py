...
def disvg(paths=None, colors=None,
    ...
    if viewbox:
        ...
        xmin -= margin_size + (extra_space_for_style / 2)
        xmax += margin_size + (extra_space_for_style / 2)
        dx += (margin_size * 2) + extra_space_for_style

        ymin -= margin_size + (extra_space_for_style / 2)
        ymax += margin_size + (extra_space_for_style / 2)
        dy += (margin_size * 2) + extra_space_for_style

        viewbox = "%s %s %s %s" % (xmin, ymin, dx, dy)

        width = max(dx, mindim)
        height = max(dy, mindim)
