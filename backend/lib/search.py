...
class Search(BasicProcessor, ABC):
    ...
    def process(self):
            ...
            % str(
                {
                    ...
                    if not self.get_options().get(k, {}).get("sensitive", False)
                }
            )
        )
        ...
        if items:
            ...
            if self.extension == "csv":
                ...
                raise NotImplementedError(
                    "Datasource query cannot be saved as %s file" % results_file.suffix
                )
        ...
        if self.import_warning_count == 0 and self.import_error_count == 0:
            ...
            self.dataset.update_status(
                f"All data imported. {str(self.import_error_count) + ' item(s) had an unexpected format and cannot be used in 4CAT processors. ' if self.import_error_count != 0 else ''}{str(self.import_warning_count) + ' item(s) missing some data fields. ' if self.import_warning_count != 0 else ''}\n\nMissing data is noted in the `missing_fields` column of this dataset's CSV file; see also the dataset log for details.",
                is_final=True,
            )
            self.dataset.finish(num_rows=num_items)
        ...
        with path.open(encoding="utf-8") as infile:
            ...
            for i, line in enumerate(infile):
                ...
                try:
                    ...
                    warning = (
                        f"An item on line {i:,} of the imported file could not be parsed as JSON - this may "
                ...
                if check_map_item:
                    try:
                        ...
                        self.dataset.warn_unmappable_item(
                            item_count=i,
                            processor=self,
                            error_message=e,
                            warn_admins=unmapped_items is False,
                        )
                        unmapped_items = True
    ...
    def items_to_csv(self, results, filepath):
        ...
        pseudonymise_author = (
            self.parameters.get("pseudonymise", None) == "pseudonymise"
        )
            ...
            for row in results:
                ...
                if "timestamp" in row:
                    ...
                    if not isinstance(timestamp, int):
                        if (
                            isinstance(timestamp, str) and "-" not in timestamp
                        ):  # String representation of epoch timestamp
                            timestamp = int(timestamp)
                        elif (
                            isinstance(timestamp, str) and "-" in timestamp
                        ):  # Date string
                            try:
                                timestamp = (
                ...
                if pseudonymise_author:
                    author_fields = [
                        field for field in row.keys() if field.startswith("author")
                    ]
            ...
            for item in items:
                ...
                if self.parameters.get("pseudonymise") == "pseudonymise":
                    item = dict_search_and_update(
                        item, ["author*"], hash_cache.update_cache
                    )
                elif self.parameters.get("anonymise") == "anonymise":
                    item = dict_search_and_update(
                        item, ["author*"], lambda v: "REDACTED"
                    )
...
class SearchWithScope(Search, ABC):
    ...
    def search(self, query):
        ...
        if query.get("search_scope", None) == "dense-threads":
            ...
            self.dataset.update_status(
                "Retrieving thread metadata for %i threads" % len(thread_ids)
            )
            try:
            ...
            if len(qualifying_thread_ids) > 25000:
                self.dataset.update_status(
                    "Too many matching threads (%i) to get full thread data for, aborting. Please try again with a narrower query."
                    % len(qualifying_thread_ids)
                )
                return None

            if qualifying_thread_ids:
                self.dataset.update_status(
                    "Fetching all items in %i threads" % len(qualifying_thread_ids)
                )
                items = self.fetch_threads(tuple(qualifying_thread_ids))
            else:
                self.dataset.update_status(
                    "No threads matched the full thread search parameters."
                )
                return None
            ...
            self.dataset.update_status(
                "Retrieving all items from %i threads" % len(thread_ids)
            )
