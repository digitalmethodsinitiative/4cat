...
class WorkerManager:
        ...
        for jobtype in self.worker_pool:
            ...
            for worker in all_workers:
                if not worker.is_alive():
                    self.log.debug(
                        f"Terminating worker {worker.job.data['jobtype']}/{worker.job.data['remote_id']}"
                    )
                    worker.join()
            ...
            if jobtype in self.modules.workers:
                ...
                if len(self.worker_pool[jobtype]) < worker_class.max_workers:
                    try:
                        job.claim()
                        worker = worker_class(
                            logger=self.log, manager=self, job=job, modules=self.modules
                        )
                        worker.start()
                        self.log.debug(
                            f"Starting new worker of for job {job.data['jobtype']}/{job.data['remote_id']}"
                        )
    ...
    def validate_datasources(self):
        ...
        for datasource in self.modules.datasources:
            if (
                datasource + "-search" not in self.modules.workers
                and datasource + "-import" not in self.modules.workers
            ):
                self.log.error(
                    "No search worker defined for datasource %s or its modules are missing. Search queries will not be executed."
                    % datasource
                )

            self.modules.datasources[datasource]["init"](
    ...
    def abort(self, signal=None, stack=None):
        ...
        for job in self.queue.get_all_jobs("cancel-pg-query", restrict_claimable=False):
            ...
            self.log.debug(
                "Cancelling interruptable Postgres queries for connection %s..."
                % job.data["remote_id"]
            )
            job.claim()
    ...
    def request_interrupt(
        self, interrupt_level, job=None, remote_id=None, jobtype=None
    ):
        """
        Interrupt a job
        ...
        for worker in self.worker_pool[jobtype]:
            if (job and worker.job.data["id"] == job.data["id"]) or (
                worker.job.data["jobtype"] == jobtype
                and worker.job.data["remote_id"] == remote_id
            ):
                # first cancel any interruptable queries for this job's worker
                while True:
                    active_queries = self.queue.get_all_jobs(
                        "cancel-pg-query",
                        remote_id=worker.db.appname,
                ...
                self.log.info(
                    f"Requesting interrupt of job {worker.job.data['id']} ({worker.job.data['jobtype']}/{worker.job.data['remote_id']})"
                )
