...
class SemanticFrameExtractor(): #BasicPostProcessor):
	...
	def process(self):
		...
		"""
		self.query.update_status("Sending post data to PENELOPE API endpoint")
		...
		try:
			...
			with requests.post("https://penelope.vub.be/semantic-frame-extractor/texts-extract-frames",
				...
				with open(self.query.get_results_path(), "wb") as output:
					for chunk in stream.iter_content(chunk_size=1024):
						if chunk:
							output.write(chunk)

		except requests.RequestException:
			self.query.update_status("Trouble reaching PENELOPE endpoint; analysis halted.")
			self.query.finish()

		with open(self.query.get_results_path(), "a") as init:
			init.write("]")

		self.query.update_status("Finished")
		self.query.finish(self.parent.num_rows)
	...
	def chunked_dataset(self):
		...
		"""
		input_file = open(self.source_file, "rb")
