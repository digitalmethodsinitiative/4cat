...
class DataSet(FourcatModule):
    ...
    def __init__(
        ...
        if key is not None:
            ...
            current = self.db.fetchone(
                "SELECT * FROM datasets WHERE (parameters::json->>'job')::text = %s",
                (str(job),),
            )
            if not current:
                raise DataSetNotFoundException(
    ...
    def reserve_result_file(self, parameters=None, extension="csv"):
        ...
        while path.is_file():
            path = self.folder.joinpath(
                file + "-" + str(index) + "." + extension.lower()
            )
            index += 1
        ...
        updated = self.db.update(
            "datasets",
            where={"query": self.data["query"], "key": self.data["key"]},
    ...
    def get_compatible_processors(self, user=None):
        ...
        for processor_type, processor in processors.items():
            ...
            if (
                not hasattr(processor, "is_compatible_with") and not self.key_parent
            ) or (
                hasattr(processor, "is_compatible_with")
                and processor.is_compatible_with(self, user=user)
            ):
    ...
    def get_annotation_fields(self) -> dict:
        ...
        annotation_fields = self.db.fetchone(
            "SELECT annotation_fields FROM datasets WHERE key = %s;", (self.key,)
        )
    ...
    def save_annotations(self, annotations: list, overwrite=True) -> int:
        ...
        for annotation_data in annotations:
            ...
            if not overwrite and annotation_data["label"] in annotation_labels:
                raise AnnotationException(
                    "Can't save annotations; annotation field with label %s "
            ...
            if not overwrite:
                if not field_id:
                    field_id = hash_to_md5(
                        annotation_data["dataset"] + annotation_data["label"] + salt
                    )
    ...
    def save_annotation_fields(self, new_fields: dict, add=False) -> int:
        ...
        for field_id, annotation_field in new_fields.items():
            ...
            if annotation_field["label"] in seen_labels:
                raise AnnotationException(
                    "Can't save annotation fields: labels must be unique (%s)"
