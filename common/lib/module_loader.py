...
class ModuleCollector:
    ...
    def expand_datasources(self):
        ...
        for datasource_id in self.datasources:
            ...
            self.datasources[datasource_id]["worker_type"] = worker.type if worker else None
            self.datasources[datasource_id]["has_options"] = bool(worker) and \
                                                             bool(worker.get_options(config=self.config))
            self.datasources[datasource_id]["importable"] = bool(worker) and \
                                                            hasattr(worker, "is_from_zeeschuimer") and \
                                                            worker.is_from_zeeschuimer
