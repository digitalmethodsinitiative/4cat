    ...
    def load_datasources(self):
        ...
        def _load_datasource(subdirectory, expiration):
            ...
            if datasource_id in self.datasources:
                if self.datasources[datasource_id]["path"] != subdirectory:
                    # 4CAT's own data sources are loaded before extensions, so
                    # this keeps 4CAT's version rather than letting an extension
                    # replace it - and with it the folder that load_modules()
                    # scans for that data source's workers, which would take
                    # those workers, and the settings they declare, out of 4CAT
                    # altogether.
                    self.log_buffer += ("Data source '%s' in %s is already provided by %s, so it is not loaded. Give "
                                        "one of them a different DATASOURCE id.\n" %
                                        (datasource_id, subdirectory, self.datasources[datasource_id]["path"]))

                # the same folder reached a second time is not a clash. The
                # registry is shared by every collector in the process and
                # load_datasources() always rescans, so a second collector meets
                # everything the first one already registered.
                return
