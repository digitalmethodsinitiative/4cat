...
class JobQueue:
    ...
    def get_job_count(self, jobtype="*"):
        ...
        if jobtype == "*":
            ...
            count = self.db.fetchone(
                "SELECT COUNT(*) FROM jobs WHERE jobtype = %s;", (jobtype,)
            )
    ...
    def get_place_in_queue(self, job):
        ...
        return len(
            [
                queued_job
                for queued_job in all_queued
                if queued_job.data["timestamp"] < our_timestamp
                or queued_job.data["timestamp_claimed"] > 0
            ]
        )
