...
if os.path.isdir("extensions"):
    ...
    if extension_packages:
        print(
            "Found extensions, installing additional packages: "
            + str(extension_packages)
        )
        packages = packages.union(extension_packages)

# Some packages don't run on Windows
unix_packages = set(["python-daemon==2.3.2"])
...
setup(
    ...
    version=version,
    description=(
        "4CAT: Capture and Analysis Tool is a comprehensive tool for "
