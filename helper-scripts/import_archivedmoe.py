...
def commit(posts, post_fields, db, datasource, fast=False):
    ...
    if fast:
        ...
        db.execute_many(
            "INSERT INTO posts_" + datasource + " (" + post_fields_sql + ") VALUES %s",
            replacements=posts,
        )
        db.commit()
        ...
        for post in posts:
            new_post = db.insert(
                "posts_" + datasource,
                data={post_fields[i]: post[i] for i in range(0, len(post))},
                safe=True,
                constraints=("id", "board"),
                commit=False,
            )
...
cli.add_argument(
    "-i", "--input", required=True, help="File to read from, containing a CSV dump"
)
    ...
    for post in postscsv:
        ...
        if posts < args.skip:
            if posts % 1000000 == 0:
                print(
                    "Skipped %s/%s rows (%.2f%%)..."
                    % (posts, args.skip, (posts / args.skip) * 100)
                )
            continue
        ...
        thread["timestamp_modified"] = max(
            thread["timestamp_modified"], int(post["timestamp"])
        )
        thread["timestamp_archived"] = max(
            thread["timestamp_archived"], int(post["timestamp_expired"])
        )
        ...
        if post["thread_num"] == post["num"]:
            ...
            if int(post["deleted"]) == 1 and not int(post["sticky"]):
                thread["timestamp_deleted"] = max(
                    int(thread.get("timestamp_modified") or 0), int(post["timestamp"])
                )
        ...
        post_data = (
            ...
            json.dumps({"w": post["media_w"], "h": post["media_h"]})
            if post["media_filename"] != ""
            else "",  # image_dimensions
        )
        ...
        if len(postbuffer) % args.batch == 0:
            new_posts = commit(
                postbuffer, post_fields, db, args.datasource, fast=args.fast
            )
            posts_added += new_posts
            print(
                "Row %i - %i. %i new posts added."
                % (posts - args.batch, posts, posts_added)
            )
            postbuffer = []
...
if deleted_ids:
    print(
        "Also committing %i deleted posts to posts_%s_deleted table."
        % (len(deleted_ids), args.datasource)
    )
    for deleted_id in deleted_ids:
        result = db.fetchone(
            "SELECT id_seq, timestamp FROM posts_"
            + args.datasource
            + " WHERE id = %s AND board = '%s' " % (deleted_id, args.board)
        )
        db.insert(
            "posts_" + args.datasource + "_deleted",
            {"id_seq": result["id_seq"], "timestamp_deleted": result["timestamp"]},
            safe=True,
        )
    deleted_ids = set()
...
for thread_id in thread_ids:
    ...
    exists = db.fetchone(
        "SELECT * FROM threads_" + args.datasource + " WHERE id = %s AND board = %s",
        (
            thread_id,
            args.board,
        ),
    ...
    # so do some quick checks if values are higher/newer than before
        ...
        if thread["timestamp_scraped"] == 0 or int(exists["timestamp_scraped"]) == 0:
            thread["timestamp_scraped"] = max(
                thread["timestamp_scraped"], int(exists["timestamp_scraped"])
            )
        else:
            thread["timestamp_scraped"] = min(
                thread["timestamp_scraped"], int(exists["timestamp_scraped"])
            )
        ...
        thread["post_last"] = max(
            int(thread.get("post_last") or 0), int(exists.get("post_last") or 0)
        )
        thread["timestamp_deleted"] = max(
            int(thread.get("timestamp_deleted") or 0),
            int(exists.get("timestamp_deleted") or 0),
        )
        thread["timestamp_archived"] = max(
        ...
        db.update(
            "threads_" + args.datasource,
            data=thread,
            where={"id": thread_id, "board": args.board},
        )
...
db.execute(
    "UPDATE threads_"
    + args.datasource
    + " AS t SET num_replies = ( SELECT COUNT(*) FROM posts_"
    + args.datasource
    + " AS p WHERE p.thread_id = t.id AND board = %s) WHERE t.id IN %s AND board = %s",
    (
        args.boards,
        tuple(thread_ids),
        args.board,
...
db.execute(
    "UPDATE threads_"
    + args.datasource
    + " AS t SET num_images = ( SELECT COUNT(*) FROM posts_"
    + args.datasource
    + " AS p WHERE p.thread_id = t.id AND image_file != '' AND board = %s) WHERE t.id IN %s AND board = %s",
    (
        args.boards,
        tuple(thread_ids),
        args.board,
