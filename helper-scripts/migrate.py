            ...
            for file in files:
                if file == "fourcat_install.py":
                    ...
                    if result.returncode != 0:
                        print(
                            "Error while running extension installation script: "
...
def finish(args, logger, no_pip=True):
    ...
    if args.restart:
        ...
        result = subprocess.run(
            [interpreter, "4cat-daemon.py", "start"],
        ...
        if "error" in result.stdout.decode("utf-8"):
            logger.info(
                "Could not start 4CAT daemon. Please inspect the error message and restart it manually:\n"
            )
...
cli.add_argument(
    ...
    default=False,
    action="store_true",
    help="Pull and check out the latest 4CAT release from Github before migrating",
)
cli.add_argument(
    "--repository",
    "-r",
    default="https://github.com/digitalmethodsinitiative/4cat.git",
    help="URL of the repository to pull from",
)
cli.add_argument(
    "--restart",
    "-x",
    default=False,
    action="store_true",
    help="Try to restart the 4CAT daemon after finishing migration, and 'touch' the WSGI file to trigger a front-end reload",
)
cli.add_argument(
    "--no-migrate",
    "-m",
    default=False,
    action="store_true",
    help="Do not run scripts to upgrade between minor versions. Use if you only want to use migrate to e.g. upgrade dependencies.",
)
cli.add_argument(
    "--current-version",
    "-v",
    default="config/.current-version",
    help="File path to .current-version file, relative to the 4CAT root",
)
cli.add_argument(
    "--output",
    "-o",
    default="",
    help="By default migrate.py will send output to stdout. If this argument is set, it will write to the given path instead.",
)
cli.add_argument(
    "--component",
    "-c",
    default="both",
    help="Which component of 4CAT to migrate ('both', 'backend', 'frontend'). Skips check for if 4CAT is running when set to 'frontend'. Also used by extensions w/ fourcat_install.py",
)
cli.add_argument(
    "--branch",
    "-b",
    default=False,
    help="Which branch to check out from GitHub. By default, check out the latest release.",
)
args = cli.parse_args()
...
if not cwd.joinpath("backend/4cat.pid").exists():
    ...
    result = subprocess.run(
        [interpreter, "4cat-daemon.py", "--no-version-check", "force-stop"],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        cwd=cwd,
    )
    if result.returncode != 0:
        logger.info(
            "  ...could not shut down 4CAT. Please make sure it is stopped and re-run this script."
        )
        logger.info(result.stdout.decode("utf-8"))
...
# ---------------------------------------------
target_version, target_version_c, current_version, current_version_c = get_versions(
    target_version_file, current_version_file
)
migrate_to_run = []
...
if args.release or args.branch:
    ...
    if args.repository[:4] == "git@":
        repo_id = "/".join(args.repository.split(":")[1].split("/")[-2:]).split(".git")[
            0
        ]
    else:
    ...
    if args.release:
        try:
            ...
            logger.info(
                "Error while retrieving latest release tag via GitHub API. Check that the repository URL is correct."
            )
            exit(1)
        ...
        if tag_version <= current_version_c:
            logger.info(
                "  ...latest release available from GitHub (%s) is older than or equivalent to currently checked out version "
    ...
    remote = subprocess.run(
        oslex.split("git remote add 4cat_migrate %s" % args.repository),
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        cwd=cwd,
        text=True,
    ...
    if remote.stderr:
        if remote.stderr.strip() == "error: remote 4cat_migrate already exists.":
            # Update URL
            remote = subprocess.run(
                oslex.split("git remote set-url 4cat_migrate %s" % args.repository),
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                cwd=cwd,
                text=True,
    ...
    fetch = subprocess.run(
        oslex.split("git fetch 4cat_migrate"),
        stderr=subprocess.PIPE,
        stdout=subprocess.PIPE,
        cwd=cwd,
        text=True,
    ...
    if fetch.returncode != 0:
        if "fatal: could not read Username" in fetch.stderr:
            ...
            if config.get("USING_DOCKER"):
                ...
                unset_authorization = subprocess.run(
                    oslex.split(
                ...
                if fetch.returncode != 0:
                    logger.info(
                        "Error while fetching latest tags with git. Check that the repository URL is correct."
                    )
    ...
    result = subprocess.run(
        oslex.split(command), stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=cwd
    )

    if result.returncode != 0:
        if args.branch:
            logger.info(
                "Error while pulling latest release with git. Check that the repository URL and branch name are correct."
            )
        else:
            logger.info(
                "Error while pulling latest release with git. Check that the repository URL is correct."
            )
        logger.info(result.stderr.decode("ascii"))
        exit(1)
...
if current_version_c > target_version_c:
    logger.info(
        "  ...checked out version is older than version last migrated to. Cannot migrate to older version."
    )
...
if not migrate_to_run:
    ...
    migrate_to_run = sorted(
        migrate_to_run, key=lambda x: make_version_comparable(x.stem.split("-")[1])
    )
...
if config and config.get("USING_DOCKER"):
    logger.info(
        "- Running in Docker environment, checking for and installing any needed compilers..."
    )
    # Pip needs some compilers to successfully update
    needed_packages = ["g++", "gcc"]
    try:
        apt_get = subprocess.run(
        ...
        logger.info(
            "\n".join(
                ["  " + line for line in apt_get.stdout.decode("utf-8").split("\n")]
            )
        )
...
logger.info(
    "- Running pip to install new dependencies and upgrade existing dependencies"
)
...
for file in migrate_to_run:
    ...
    try:
        result = subprocess.run(
            [interpreter, str(file.resolve())], stderr=subprocess.PIPE, cwd=cwd
        )
        if result.returncode != 0:
