...
cli.add_argument(
    ...
    "--input",
    required=True,
    help="text file to read from - should have a post ID on every line, "
    "and optionally a timestamp of deletion after a space (e.g. '349543818 1637922696').",
)
cli.add_argument(
    "-b", "--board", required=True, help="What board the post IDs belong to, e.g. 'mu'"
)
cli.add_argument(
    "-d",
    "--datasource",
...
with open(args.input, "r", encoding="utf-8") as in_txt:
    for line in in_txt.readlines():
        ...
        post = db.fetchone(
            "SELECT id_seq, id, thread_id, timestamp FROM posts_"
            + args.datasource
        ...
        if not post:
            ...
            db.insert(
                ...
                data={"id_seq": id_seq, "timestamp_deleted": timestamp_deleted},
                safe=True,
                commit=False,
            )
            ...
            if post["id"] == post["thread_id"]:
                ...
                db.execute(
                    "UPDATE threads_"
                    + args.datasource
                    + " SET timestamp_deleted = GREATEST(timestamp_deleted, %s) WHERE id = %s AND board = '%s'"
                    % (
                        int(timestamp_deleted),
                        post_id,
                        args.board,
                    )
                )
        ...
        if posts % batch == 0:
            print(
                "Inserting/updating deleted post IDs %s/%s" % (posts - batch, posts),
                end="",
            )
            if threads:
