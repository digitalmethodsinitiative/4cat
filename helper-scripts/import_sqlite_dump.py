...
cli.add_argument(
    ...
    "--input",
    required=True,
    help="File to read from, containing a SQLite archive",
)
...
for post in c:
    ...
    post = {
        k: (
            v.decode("latin-1").encode("utf-8").decode("utf-8")
    ...
    if int(post["chan_id"]) == int(chan_thread_id):
        ...
        scraped_date = datetime.strptime(
            post["chan_post_date"][:19], "%Y-%m-%d %H:%M:%S"
        )
        ...
        if thread["update_date"]:
            modified_date = datetime.strptime(
                thread["update_date"][:19], "%Y-%m-%d %H:%M:%S"
            )
            modified_timestamp = int(
                modified_date.replace(tzinfo=timezone.utc).timestamp()
            )
    ...
    try:
        new_id = db.insert(
            "posts_4chan", post_data, commit=False, safe=False, return_field="id_seq"
        )
    except UniqueViolation:
        print(
            "Duplicate post with id %s in the SQLite dump, skipping." % post_data["id"]
        )
        db.rollback()
        post_data = {}
        continue
...
print(
    "Done - added %i posts from %s to the 4CAT 4chan dataset"
    % (
        count,
        args.board,
    )
