...
from pol_flags import get_country_name
...
with open(args.input, encoding="utf-8") as inputfile:
	...
	for csvpost in postscsv:
		...
		# Set country name of post. This is a bit tricky because troll country names
		# are in the exif field, while we have to retrieve geo country names ourselves.
		country_name = ""
		if args.board == 'pol':
			
			# Older entries don't always have exif data, so let's extract the country name ourselves.
			if int(post["timestamp"]) < 1418515200:
				country_name = get_country_name(post["poster_country"], post["timestamp"])

			# We can assume it's a troll country if it's in "troll country"
			elif post["exif"] and "troll_country" in exif:
				country_name = get_country_name("t_" + post["exif"]["troll_country"], post["timestamp"])
		...
		if int(post["deleted"]) == 1 and (post["num"] != post["thread_num"] and threads[post["thread_num"]]["is_sticky"] != 1):
	...
	if deleted_ids:
		...
		for deleted_id in deleted_ids:
			...
			db.insert("posts_" + args.datasource + "_deleted", {"id_seq": result["id_seq"], "timestamp_deleted": result["timestamp"]})
		deleted_ids = set()
