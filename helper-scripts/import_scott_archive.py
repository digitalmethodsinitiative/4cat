...
cli.add_argument(
    ...
    "--input",
    required=True,
    help="Folder containing the HTML files for /b/ threads.",
)
cli.add_argument(
    "-s",
    "--skip_duplicates",
    type=str,
    required=True,
    help="If duplicate posts should be skipped (useful if there's already data in the table)",
)
...
for html_doc in glob.iglob(os.path.join(args.input, "*.*")):
    ...
    start_timestamp, start_id = [
        (int(datetime.strptime(v, "%Y-%m-%d").timestamp()), int(k))
        for k, v in known_timestamps.items()
        if thread_id >= int(k)
    ][-1]
    end_timestamp, end_id = [
        (int(datetime.strptime(v, "%Y-%m-%d").timestamp()), int(k))
        for k, v in known_timestamps.items()
        if thread_id < int(k)
    ][0]
    ...
    op_data = {
        ...
        "author": soup.find("span", {"class": "postername"}).get_text()
        if soup.find("span", {"class": "postername"})
        else "",
        "author_type": "",
        "author_type_id": "N",
        "author_trip": "",
        ...
        "unsorted_data": json.dumps(
            {"has_image": True if soup.find("span", {"class": "filesize"}) else False}
        ),
    }
    ...
    for text in soup.find_all("td", {"class": "reply"}):
        ...
        post_data = {
            ...
            "author": text.find("span", {"class": "postername"}).get_text()
            if text.find("span", {"class": "postername"})
            else "",
            "author_type": "",
            "author_type_id": "N",
            "author_trip": "",
