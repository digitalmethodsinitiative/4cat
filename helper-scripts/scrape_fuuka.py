    ...
    if thread_id == 0:
        if not post["title"]:
            semantic_url = re.sub(
                r"[^a-z0-9 ]",
                "",
                " ".join(post["comment_sanitized"].split(" ")[:9]).lower(),
            ).replace(" ", "-")
        else:
...
def thread(thread_json):
    ...
    if "posts" in thread_json:
        posts = [
            fuuka_to_4chan(thread_json["posts"][id], op["no"])
            for id in thread_json["posts"]
            if thread_json["posts"][id]["subnum"] == "0"
        ]
...
cli.add_argument(
    "-o", "--output", required=True, help="Path to folder with output JSON"
)
...
while True:
    ...
    try:
        ...
        print(
            "RequestException while requesting page %i: %s"
    ...
    if page_html.status_code != 200:
        print(
            "HTTP error %i while scraping page %i, retrying in 10 seconds"
            % (page_html.status_code, page)
        )
        time.sleep(10)
        continue
    ...
    try:
        ...
        print(
            "JSON decode error while trying to decode page %i, dumped as error.json"
            % page
        )
    ...
    while True:
        ...
        thread_data = None
        thread_url = args.url + "/_/api/chan/thread/?board=%s&num=%s" % (
            args.board,
            thread_id,
        )

        try:
            thread_html = requests.get(
                thread_url,
                headers={
                    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:61.0) Gecko/20100101 Firefox/61.0"
                },
            ...
            print(
                "RequestException while requesting thread %s: %s"
        ...
        if thread_html.status_code > 399 and thread_html.status_code < 500:
            print(
                "HTTP error %i while scraping thread %s, skipping"
                % (thread_html.status_code, thread_id)
            )
            continue
        if thread_html.status_code != 200:
            print(
                "HTTP error %i while scraping thread %s, waiting 10 seconds before retrying"
                % (thread_html.status_code, thread_id)
            )
            time.sleep(10)
            retry = True
        ...
        try:
            ...
            print(
                "JSON decode error while trying to decode thread %s, dumped as error.json"
                % thread_id
            )
            with open("error.json", "wb") as output:
