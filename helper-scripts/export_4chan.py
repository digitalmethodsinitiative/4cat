...
to be exported to a NDJSON or CSV file.

NDJSON dumps can be directly imported to Webjutter.
...
cli.add_argument("-o", "--output", required=True, help="NDJSON or csv file to write to.")
...
if args.output.lower().endswith(".ndjson"):
    filetype = "ndjson"
elif args.output.lower().endswith(".csv"):
    filetype = "csv"
else:
    print("Please provide a valid file type (.csv or .ndjson).")
    sys.exit(1)
        ...
        for row in posts:
            ...
            if row["id"] == row["thread_id"]:
                ...
                if thread_data:
                    # Wrap in posts dict
                    if filetype == "ndjson":
                        out_file.write('{"posts": [')
                    for i_p, p in enumerate(thread_data):
                        if filetype == "ndjson":
                            suffix = "," if i_p < len(thread_data) - 1 else ""
                            out_file.write(json.dumps(p) + suffix)
                        if filetype == "csv":
                            writer.writerow(p)
                    if filetype == "ndjson":
                        out_file.write("]}\n")
                ...
                op_db = db.fetchone(
                    "SELECT * FROM threads_4chan WHERE id_seq = %i" % row["id_seq"]
                )
                ...
                post = {
                    ...
                    if row["semantic_url"] is not None
                    else "",
                }
                ...
                if op_db:
                    ...
                    if (
                        row["timestamp_deleted"] and row["timestamp_deleted"] > 0
                    ) and not post.get("is_sticky"):
                        post["deleted"] = True
                        post["timestamp_deleted"] = row["timestamp_deleted"]
                        thread_deleted = True
            ...
            deleted = (
                True
                if (row["timestamp_deleted"] and row["timestamp_deleted"] > 0)
                and not post.get("op")
                else False
            )
            if (deleted or thread_deleted) and row["id"] != row["thread_id"]:
                ...
                if not skip_deleted:  # Deleted OP data might have been skipped, so we maybe can't get data from `thread_data`
                    post["timestamp_deleted"] = (
                        row["timestamp_deleted"]
                        if (row["timestamp_deleted"] and row["timestamp_deleted"] > 0)
                        else thread_data[0]["deleted_on"]
                    )
            ...
            # Time strings are in EDT/EDT in the 4chan API--adhere to this.
            post["now"] = datetime.fromtimestamp(
            ...
            # These are saved as 'board_flag' and 'flag_name' in the API.
            if "t_" in row.get("country_code", ""):
                row["board_flag"] = row["country_code"].replace("t_", "")
                row["flag_name"] = row["country_name"]
