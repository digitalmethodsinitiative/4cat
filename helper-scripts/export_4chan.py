...
cli.add_argument(
    ...
    type=str,
    required=False,
    help="Board name(s) (separate with comma). If not given, export all available boards.",
)
cli.add_argument(
    "-c",
    "--itersize",
    ...
    required=False,
    default=100000,
    help="Iteration size with which to retrieve data from the database.",
)
cli.add_argument(
    "-d",
    "--skip_deleted",
    ...
    required=False,
    default=False,
    help="Whether to skip deleted posts.",
)
...
min_thread_id = db.fetchone("SELECT min(id) FROM threads_4chan WHERE %s " % board_sql)[
    "min"
]
max_thread_id = db.fetchone("SELECT max(id) FROM threads_4chan WHERE %s " % board_sql)[
    "max"
]
...
with open(args.output, "w", newline="", encoding="utf-8") as out_file:
    ...
    while True:
        ...
        for row in posts:
            ...
            post["time_utc"] = datetime.strftime(
                datetime.utcfromtimestamp(row["timestamp"]), "%Y-%m-%d %H:%M:%S"
            )
