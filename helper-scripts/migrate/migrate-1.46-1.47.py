...
if has_column["num"] == 0:
    print("  ...No, adding.")
    db.execute(
        "ALTER TABLE datasets ADD COLUMN timestamp_finished INTEGER DEFAULT NULL"
    )
    ...
    for dataset in dataset_ids:
        ...
        if log_exists:
            try:
                timestamp_finished = datetime.datetime.strptime(
                    get_last_line(log_path)[:24], "%c"
                )
                update_data.append((key, int(timestamp_finished.timestamp())))
            except ValueError as e:
                # Unable to parse datetime from last line
                print(
                    f" ...Unable to parse datetime from last line for dataset {key}: {e}"
                )
                unable_to_update.append(key)
        else:
            # No log file; unable to determine timestamp_finished
            print(
                f" ...Unable to determine timestamp_finished for dataset {key}; no log file."
            )
            unable_to_update.append(key)

    if update_data:
        db.execute_many(
            "UPDATE datasets SET timestamp_finished = data.timestamp_finished FROM (VALUES %s) AS data (key, timestamp_finished) WHERE datasets.key = data.key",
            replacements=update_data,
        )
