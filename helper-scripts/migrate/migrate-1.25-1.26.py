...
def set_or_create_setting(
    attribute_name, value, connection, cursor, keep_connection_open=False
):
    """
    ...
    try:
        ...
        print(
            "Error transfering setting %s with value %s: %s"
            % (attribute_name, str(value), str(error))
        )
        updated_rows = None
    finally:
...
try:
    ...
    connection = psycopg2.connect(
        dbname=old_config.DB_NAME,
        user=old_config.DB_USER,
        password=old_config.DB_PASSWORD,
        host=old_config.DB_HOST,
        port=old_config.DB_PORT,
        application_name="4cat-migrate",
    )
...
if transfer_settings:
    ...
    # Check if Docker
    old_config_filepath = getattr(
        old_config, "DOCKER_CONFIG_FILE", getattr(old_config, "CONFIG_FILE", None)
    )
    ...
    if old_config.ANONYMISATION_SALT:
        config_reader["GENERATE"]["anonymisation_salt"] = str(
            old_config.ANONYMISATION_SALT
        )
    ...
    old_settings = [
        ...
        (
            "mail.admin_email",
            getattr(old_config, "ADMIN_EMAILS", [None])[0]
            if getattr(old_config, "ADMIN_EMAILS", [None])
            else None,
        ),
        ...
        (
            ...
            getattr(old_config, "MAX_NUMBER_IMAGES", None),
        ),
        (
        ...
        (
            ...
            getattr(old_config, "TUMBLR_CONSUMER_SECRET_KEY", None),
        ),
        ...
        (
            ...
            getattr(old_config.FlaskConfig, "HOSTNAME_WHITELIST", None),
        ),
        (
            "flask.autologin.api",
            getattr(old_config.FlaskConfig, "HOSTNAME_WHITELIST_API", None),
        ),
        (
            "flask.autologin.name",
            getattr(old_config.FlaskConfig, "HOSTNAME_WHITELIST_NAME", None),
        ),
    ]

    for name, setting in old_settings:
        if setting is not None:
            set_or_create_setting(
                name,
                setting,
