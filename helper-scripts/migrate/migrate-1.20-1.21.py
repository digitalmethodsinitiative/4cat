...
for datasource in ("4chan", "8kun", "8chan"):
    ...
    thread_constraints = [
        ...
        for row in db.fetchall(
            "SELECT c.column_name, c.constraint_name FROM information_schema.key_column_usage AS c LEFT JOIN information_schema.table_constraints AS t ON t.constraint_name = c.constraint_name WHERE t.table_name = 'threads_%s' AND t.constraint_type = 'PRIMARY KEY';"
            % datasource
        )
    ]
    ...
    if not thread_constraints:
        print(
            "  No unique key detected for the threads table, adding a unique key constraint to the `id_seq` column."
        )
        ...
        if constraint_col == "id_seq":
            ...
            print(
                " changing primary threads key from %s to id_seq (this might take a while)."
                % constraint_col
            )
            make_primary_threads_key = True
            db.execute(
                "ALTER TABLE threads_%s DROP CONSTRAINT %s"
                % (datasource, constraint_name)
            )
    ...
    if make_primary_threads_key:
        try:
            ...
            db.execute(
                "ALTER TABLE threads_%s ADD COLUMN id_seq SERIAL PRIMARY KEY"
                % datasource
            )
    ...
    post_constraints = [
        (row["column_name"], row["constraint_name"])
        for row in db.fetchall(
            "SELECT c.column_name, c.constraint_name FROM information_schema.key_column_usage AS c LEFT JOIN information_schema.table_constraints AS t ON t.constraint_name = c.constraint_name WHERE t.table_name = 'posts_%s' AND t.constraint_type = 'PRIMARY KEY';"
            % datasource
        )
    ]
    ...
    if not post_constraints:
        print(
            "  No unique key detected for the posts table, adding a unique key constraint to the `id_seq` column."
        )
        ...
        if constraint_col == "id_seq":
            ...
            print(
                " changing primary posts key from %s to id_seq (this might take a while)."
                % constraint_col
            )
            make_primary_posts_key = True
            db.execute(
                "ALTER TABLE posts_%s DROP CONSTRAINT %s"
                % (datasource, constraint_name)
            )
    ...
    if make_primary_posts_key:
        try:
            ...
            db.execute(
                "ALTER TABLE posts_%s ADD COLUMN id_seq SERIAL PRIMARY KEY" % datasource
            )
    ...
    db.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS posts_%s_idboard ON posts_%s ( id, board )"
    ...
    db.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS threads_%s_idboard ON threads_%s ( id, board )"
        % (datasource, datasource)
    )
