    ...
    def process(self):
        ...
        if self.source_dataset.num_rows == 0:
            self.dataset.update_status(
                "No videos from which to extract audio.", is_final=True
            )
        ...
        total_possible_videos = max(
            max_files if max_files != 0 else self.source_dataset.num_rows - 1, 1
        ...
        for path in self.iterate_archive_contents(self.source_file, staging_area):
            ...
            if ffmpeg_output:
                with open(
                    str(output_dir.joinpath(f"{vid_name}_stdout.log")), "w"
                ) as outfile:
            ...
            if ffmpeg_error:
                ...
                with open(
                    str(output_dir.joinpath(f"{vid_name}_stderr.log")), "w"
                ) as outfile:
                    outfile.write(ffmpeg_error)

            if result.returncode != 0:
                error = "Error Return Code with video %s: %s" % (
                    vid_name,
            ...
            processed_videos += 1
            self.dataset.update_status(
                "Extracted audio from %i of %i videos"
