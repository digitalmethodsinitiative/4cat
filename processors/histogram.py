...
class HistogramRenderer(BasicProcessor):
	"""
	...
	def process(self):
		...
		* or " in the query
		...
		fontfile = Path(config.PATH_ROOT, fontfile)
		...
		for year in months:
			for month in months[year]:
				...
				self.dataset.update_status("%i-%i: %i posts" % (year, month, posts))
		...
		if fontfile.exists():
			...
			headerfont = ImageFont.truetype(str(fontfile), int(header_rect_height / 2))
			...
			self.log.warning("Font file missing: %s" % str(fontfile))

		histogram.save(str(self.dataset.get_results_path()))

		self.dataset.update_status("Finished")
		self.dataset.finish(num_months)