...
class ViewMetadata(BasicProcessor):
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        """
        return module.type.startswith("video-downloader") or module.type.startswith(
            "image-downloader"
        )
    ...
    def process(self):
        ...
        with zipfile.ZipFile(self.source_file, "r") as archive_file:
            ...
            if ".metadata.json" not in archive_contents:
                self.dataset.update_status(
                    "Unable to identify metadata file", is_final=True
                )
        ...
        if parent_processor is None or not hasattr(parent_processor, "map_metadata"):
            if parent_processor is not None:
                self.log.warning(
                    f"Metadata formatter processor cannot run on {parent_processor.type}; map_metadata method not implemented"
                )
            self.dataset.update_status(
                "Cannot reformat metadata for this dataset", is_final=True
            )
            self.dataset.finish(0)
            return
