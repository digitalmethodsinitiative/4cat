...
class FourcatToDmiTcatUploader(BasicProcessor):
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        """
        return (
            module.type == "convert-ndjson-for-tcat"
    ...
    def process(self):
        ...
        bin_name = "".join(
            e if e.isalnum() else "_" for e in self.dataset.top_parent().get_label()
        ).lower()
        ...
        auth = (
            self.config.get("tcat-auto-upload.username"),
            self.config.get("tcat-auto-upload.password"),
        )

        # find server URL
        server_choice = self.parameters.get("server", "random")
        if type(self.config.get("tcat-auto-upload.server_url")) in (list, set, tuple):
            if server_choice == "random" or server_choice not in self.config.get(
                "tcat-auto-upload.server_url"
            ):
                server_choice = random.choice(
                    self.config.get("tcat-auto-upload.server_url")
                )
        else:
        ...
        response = self.send_request_to_TCAT(
            server_choice, auth, url_to_file, bin_name, query
        )
        ...
        if response.status_code == 504:
            ...
            self.dataset.update_status(
                "TCAT server currently busy; please try again later"
            )
        ...
        try:
            ...
            if "The query bin" in response.text and "already exists" in response.text:
                ...
                return self.dataset.finish_with_error(
                    "DMI-TCAT bin already exists; unable to add to existing bin."
                )
            else:
                # Something else is wrong...
                self.log.error(
                    "DMI-TCAT Unexpected response: %s - %s - %s"
        ...
        if "success" not in resp_content:
            ...
            return self.dataset.finish_with_error(
                "Error while importing to DMI-TCAT: %s" % resp_content["error"]
            )
        ...
        tcat_result_url = (
            server_choice.replace("/api/import-from-4cat.php", "").rstrip("/")
            + "/analysis/index.php?dataset="
            + bin_name
            + "&startdate="
            + start_date
            + "&enddate="
