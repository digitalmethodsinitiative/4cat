...
class ExtractURLs(BasicProcessor):
    ...
    description = (
        "Extract any URLs from selected column(s) and, optionally, expand any shortened URLs. This will create"
        " a new dataset."
    )
    extension = "csv"
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        if parent_dataset and parent_dataset.get_columns():
            ...
            options["columns"]["default"] = (
                "body"
                if "body" in columns
                else sorted(columns, key=lambda k: "text" in k).pop()
            )
    ...
    def process(self):
        ...
        # Create fieldnames
        fieldnames = (
            self.source_dataset.get_columns()
            + ["4CAT_number_unique_urls", "4CAT_extracted_urls"]
            + ["4CAT_extracted_from_" + column for column in columns]
        )
        ...
        with self.dataset.get_results_path().open(
            ...
            for item in self.source_dataset.iterate_items(self):
                ...
                for column in columns:
                    ...
                    if type(value) is not str:
                        self.dataset.update_status(
                            f'Column "{column}" is not text and will be ignored.'
                        )
                        # Remove from future
                    ...
                    identified_urls = self.identify_links(
                        value, self.parameters.get("split-comma", True)
                    )
                ...
                if (
                    return_matches_only and row["4CAT_extracted_urls"]
                ) or not return_matches_only:
                ...
                if processed_items % progress_interval_size == 0:
                    self.dataset.update_status(
                        f"Processed {processed_items}/{total_items} items; {url_matches_found} items with url(s)"
                    )
    ...
    def resolve_redirect(url, redirect_domains=None, cache={}, depth=0):
        ...
        if 200 <= head_request.status_code < 400:
            ...
            if redirected_to != url:
                ...
                return ExtractURLs.resolve_redirect(
                    redirected_to, redirect_domains, cache, depth
                )
