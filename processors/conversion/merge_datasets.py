...
class DatasetMerger(BasicProcessor):
    ...
    description = (
        "Merge this dataset with other datasets of the same format. A new dataset is "
        "created containing a combination of items from the original datasets."
    ...
    options = {
        ...
        "merge": {
            ...
            "help": "Merge strategy",
            "options": {"remove": "Remove duplicates", "keep": "Keep duplicates"},
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        """
        return module.get_extension() in ("csv", "ndjson") and (
            module.is_from_collector()
        )
    ...
    def process(self):
        ...
        for source_dataset in (
            self.parameters.get("source").strip().replace("\n", ",").split(",")
        ):
            ...
            try:
                source_dataset = self.get_dataset_from_url(
                    source_dataset_url, self.db, modules=self.modules
                )
            except DataSetException:
                return self.dataset.finish_with_error(
                    f"Dataset URL '{source_dataset_url} not found - cannot perform "
                    f"merge."
                )

            if not source_dataset.is_finished():
                return self.dataset.finish_with_error(
                    f"Dataset with URL {source_dataset_url} is unfinished - finish "
            ...
            if not set(source_dataset.get_owners_users("owner")).intersection(
                set(self.source_dataset.get_owners_users("owner"))
            ) and (source_dataset.is_private or self.source_dataset.is_private):
                return self.dataset.finish_with_error(
                    "Cannot merge datasets - all need to be public or have "
                    "overlapping ownership."
                )

            if source_dataset.key in [d.key for d in source_datasets]:
                self.dataset.update_status(
                    f"Skipping dataset with URL {source_dataset_url} - already in list of "
                    f"datasets to merge"
                )
            else:
        ...
        if len(source_datasets) <= 1:
            return self.dataset.finish_with_error(
                "You need to provide at least one valid URL for a source dataset."
            )

        # clean up parameters
        self.dataset.parameters = {
            **self.dataset.parameters,
            "source": ", ".join([d.key for d in source_datasets]),
        }
            ...
            for dataset in source_datasets:
                ...
                try:
                    for mapped_item in dataset.iterate_items():
                        if self.interrupted:
                            raise ProcessorInterruptedException(
                                "Interrupted while mapping duplicates"
                            )
                        ...
                        if not canonical_fieldnames:
                            ...
                            if item_fieldnames != canonical_fieldnames:
                                return self.dataset.finish_with_error(
                                    "Cannot merge datasets - not the same set of "
                        ...
                        processed += 1
                        if (
                            self.parameters["merge"] != "keep"
                            and mapped_item.get("id") in seen_ids
                        ):
                            duplicates += 1
                            continue
                        ...
                        if dataset.get_extension() == "csv":
                            if not writer:
                                writer = csv.DictWriter(
                                    outfile, fieldnames=sorted_canonical_fieldnames
                                )
                                writer.writeheader()
                    ...
                    self.dataset.finish_with_error(
                        f"Datasets comprising {dataset.get_extension()} files cannot be merged. You can only merge NDJSON or CSV datasets."
                    )
        ...
        if num_warnings > 0:
            for dataset, dataset_warnings in warnings.items():
                ...
                self.dataset.log(
                    f"The following warning(s) were raised while processing items from dataset {dataset}:"
                )
        ...
        # phew, finally done
        self.dataset.update_status(
            f"Merged {processed:,} items ({merged:,} merged, {duplicates:,} skipped, {num_warnings:,} warnings). See dataset log for details.",
            is_final=True,
        )
