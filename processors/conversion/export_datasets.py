    ...
    def process(self):
        ...
        if not primary_dataset.is_finished():
            ...
            self.dataset.finish_with_error(
                "You cannot export unfinished datasets; please wait until dataset is finished to export."
            )
            return
        ...
        while keys:
            ...
            try:
                ...
                self.dataset.update_status(
                    f"Dataset {dataset_key} not found: it may have been deleted prior to export; skipping."
                )
                failed_exports.append(dataset_key)
                continue
            if not dataset.is_finished():
                self.dataset.update_status(
                    f"Dataset {dataset_key} not finished: cannot export unfinished datasets; skipping."
                )
                failed_exports.append(dataset_key)
                continue
            ...
            if metadata["num_rows"] == 0:
                self.dataset.update_status(
                    f"Dataset {dataset_key} has no results; skipping."
                )
                failed_exports.append(dataset_key)
                continue
            ...
            if not data_file.exists():
                self.dataset.update_status(
                    f"Dataset {dataset_key} has no data file; skipping."
                )
                failed_exports.append(dataset_key)
                continue
            ...
            # All good, add to ZIP
            with results_path.joinpath(f"{dataset_key}_metadata.json").open(
                "w", encoding="utf-8"
            ) as outfile:
