    ...
    def process(self):
        ...
        if self.source_dataset.parameters["datasource"] == "4chan":
                ...
                if self.parameters.get("overwrite"):
                    ...
                    if board in boards_4plebs:
                        post_img_links.append(
                            "https://archive.4plebs.org/_/search/image/"
                            + post["image_md5"].replace("/", "_")
                        )

                    # Fireden boards
                    elif board in boards_fireden:
                        post_img_links.append(
                            "https://boards.fireden.net/_/search/image/"
                            + post["image_md5"].replace("/", "_")
                        )
                    ...
                    # Else, assume archived.moe has it
                        ...
                        post_img_links.append(
                            "https://archived.moe/_/search/image/"
                            + post["image_md5"].replace("/", "_")
                        )
            ...
            top_images = {
                id: images[id]
                for id in sorted(
                    images, key=lambda id: images[id]["count"], reverse=True
                )
            }
            ...
            results = [
                {
                    ...
                    "url_4cat": ("https" if config.get("flask.https") else "http")
                    + "://"
                    + config.get("flask.server_name")
                    + "/api/image/"
                    + images[id]["md5"]
                    + "."
                    + images[id]["filename"].split(".")[-1],
                    "url_4plebs": "https://archive.4plebs.org/_/search/image/"
                    + images[id]["hash"].replace("/", "_"),
                    "url_fireden": "https://boards.fireden.net/_/search/image/"
                    + images[id]["hash"].replace("/", "_"),
                    "url_archivedmoe": "https://archived.moe/_/search/image/"
                    + images[id]["hash"].replace("/", "_"),
                }
