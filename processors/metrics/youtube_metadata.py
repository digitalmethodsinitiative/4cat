...
class YouTubeMetadata(BasicProcessor):
    ...
    def process(self):
        ...
        for post in self.source_dataset.iterate_items(self):
            ...
            if datasource == "reddit":
                if "youtu.be" in post.get("domain") or "youtube.com" in post.get(
                    "domain"
                ):
    ...
    def get_youtube_metadata(self, di_urls):
        ...
        video_data = self.request_youtube_api(
            videos_to_fetch, custom_key=custom_key, object_type="video"
        )
        channel_data = self.request_youtube_api(
            channels_to_fetch, custom_key=custom_key, object_type="channel"
        )
        ...
        for youtube_item in urls_metadata:
            ...
            metadata["referenced_by"] = ",".join(
                [str(n) for n in youtube_item["referenced_by"]]
            )
            ...
            if counter % 10 == 0:
                self.dataset.update_status(
                    "Extracted metadata " + str(counter) + "/" + str(len(urls_metadata))
                )
        ...
        for i, ids_string in enumerate(ids_list):
            ...
            while retries < self.max_retries:
                ...
                # Check rate limits
                    ...
                    if (
                        status_code == 403
                    ):  # "Forbidden", what Google returns with rate limits
                        retries += 1
                ...
                # Google API's also throws other weird errors that might be resolved by retrying, like SSLEOFError
                    ...
                    retries += 1
                    self.dataset.update_status(
                        "Error encoutered, sleeping for " + str(self.sleep_time)
                    )
                    time.sleep(self.sleep_time)  # Wait a bit before trying again
                    pass
            ...
            if retries >= self.max_retries:
                ...
                for metadata in response["items"]:
                    ...
                    if object_type == "video":
                        ...
                        result["video_duration"] = metadata.get("contentDetails").get(
                            "duration"
                        )
                        result["video_view_count"] = metadata["statistics"].get(
                            "viewCount"
                        )
                        result["video_comment_count"] = metadata["statistics"].get(
                            "commentCount"
                        )
                        result["video_likes_count"] = metadata["statistics"].get(
                            "likeCount"
                        )
                        result["video_dislikes_count"] = metadata["statistics"].get(
                            "dislikeCount"
                        )
                        ...
                        result["channel_description"] = metadata["snippet"].get(
                            "description"
                        )
                        result["channel_default_language"] = metadata["snippet"].get(
                            "defaultLanguage"
                        )
                        result["channel_country"] = metadata["snippet"].get("country")
                        result["channel_viewcount"] = metadata["statistics"].get(
                            "viewCount"
                        )
                        result["channel_commentcount"] = metadata["statistics"].get(
                            "commentCount"
                        )
                        result["channel_subscribercount"] = metadata["statistics"].get(
                            "subscriberCount"
                        )
                        result["channel_videocount"] = metadata["statistics"].get(
                            "videoCount"
                        )
                        # This one sometimes fails for some reason
                        if "topicDetails" in metadata:
                            result["channel_topic_ids"] = metadata["topicDetails"].get(
                                "topicIds"
                            )
                            result["channel_topic_categories"] = metadata[
                                "topicDetails"
                            ].get("topicCategories")
                        result["channel_branding_keywords"] = (
                            metadata.get("brandingSettings")
                            .get("channel")
                            .get("keywords")
                        )
    ...
    def after_process(self):
        ...
        if self.api_limit_reached:
            self.dataset.update_status(
                "YouTube API quota limit exceeded. Saving the results retreived thus far. To get all data, use your own API key, or try to split up the dataset and get the YouTube data over the course of a few days."
            )
        elif self.invalid_api_key:
            self.dataset.update_status(
                "Invalid API key. Extracted YouTube links but did not retreive any video information."
            )
        else:
