...
class CountPosts(BasicProcessor):
    ...
    def process(self):
        ...
        unknown_dates = (
            0  # seperate counter as padding will not interpret this correctly
        )
        ...
        with self.dataset.get_results_path().open("w"):
            ...
            if add_relative:
                ...
                if timeframe != "all":
                    ...
                    time_sql = (
                        "to_char(to_date(date, 'YYYY-MM-DD'), '%s') AS date_str,"
                        % time_format
                    )
                else:
                ...
                for interval in list(intervals.keys()):
                    ...
                    if interval in added_dates:
                        intervals[interval]["relative"] = (
                            intervals[interval]["absolute"] / total_counts[interval]
                        )
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        if parent_dataset.parameters.get("datasource") in (
            "fourchan",
            "eightchan",
            "eightkun",
        ):
