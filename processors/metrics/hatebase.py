...
class HatebaseAnalyser(BasicProcessor):
    ...
    references = ["[Hatebase.org](https://hatebase.org)"]

    options = {
        "language": {
            "type": UserInput.OPTION_CHOICE,
            "default": "en",
            "options": {"en": "English", "it": "Italian"},
            "help": "Language",
        },
        "columns": {
    ...
    def process(self):
        ...
        if not columns:
            self.dataset.update_status(
                "No columns selected; no data analysed.", is_final=True
            )
        ...
        with (
            ...
            .joinpath(f"common/assets/hatebase/hatebase-{language}.json")
            .open() as hatebasedata
        ):
        ...
        if not hatebase or not hatebase_regex:
            self.dataset.update_status(
                "No hatebase data found for the selected language.", is_final=True
            )
            self.dataset.finish(0)
            return
        ...
        with self.dataset.get_results_path().open("w") as output:
            ...
            fieldnames += (
                "hatebase_num",
                "hatebase_num_ambiguous",
                "hatebase_num_unambiguous",
                "hatebase_terms",
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        if parent_dataset and parent_dataset.get_columns():
            ...
            options["columns"]["default"] = (
                ["body"] if "body" in columns else [sorted(columns).pop()]
            )
