...
class AttributeRanker(BasicProcessor):
    ...
    options = {
        ...
        "top-style": {
            ...
            "default": "per-item",
            "options": {
                "per-item": "per timeframe (separate ranking per timeframe)",
    ...
    def process(self):
        ...
        if rank_style == "overall":
            ...
            for post in self.source_dataset.iterate_items(
                self,
                map_missing=missing_value_placeholder
                if self.include_missing_data
                else "default",
            ):
            ...
            overall_top = sorted(
                overall_top, key=lambda item: overall_top[item], reverse=True
            )
            if cutoff:
        ...
        for post in self.source_dataset.iterate_items(
            ...
            try:
                ...
                self.dataset.update_status(
                    "%s, cannot count items per %s" % (str(e), timeframe), is_final=True
                )
        ...
        for time_unit in sorted_items:
            sorted_unit = OrderedDict(
                (item, sorted_items[time_unit][item])
                for item in sorted(
    ...
    def get_values(self, post, attributes, filter, split_comma, extract):
        ...
        for attribute in attributes:
            if split_comma:
                item_values = [
                    v.strip()
                    for v in str(post.get(attribute, "")).split(",")
                    if v.strip() or self.include_missing_data
                ]
            else:
                item_values = (
                    [post.get(attribute, "")]
                    if post.get(attribute, "") or self.include_missing_data
                    else []
                )

            if extract:
                item_values = list(
                    chain(*[self.extract(v, extract) for v in item_values])
                )
