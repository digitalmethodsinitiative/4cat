...
class OvertimeAnalysis(BasicProcessor):
    ...
    def process(self):
        ...
        for vocabulary_id in self.parameters.get("vocabulary", []):
            vocabulary_file = self.config.get('PATH_ROOT').joinpath(f"common/assets/wordlists/{vocabulary_id}.txt")
        ...
        custom_vocabulary = set(
            [word.strip() for word in self.parameters.get("vocabulary-custom", "").split(",") if word.strip()])
