...
class OvertimeAnalysis(BasicProcessor):
    ...
    def process(self):
        ...
        for vocabulary_id in self.parameters.get("vocabulary", []):
            vocabulary_file = config.get("PATH_ROOT").joinpath(
                f"common/assets/wordlists/{vocabulary_id}.txt"
            )
            ...
            with open(vocabulary_file, encoding="utf-8") as vocabulary_handle:
                vocabularies[vocabulary_id] |= set(
                    vocabulary_handle.read().splitlines()
                )
        ...
        custom_vocabulary = set(
            [
                word.strip()
                for word in self.parameters.get("vocabulary-custom", "").split(",")
                if word.strip()
            ]
        )
        ...
        for vocabulary_id in vocabularies:
            ...
            vocabulary_regexes[vocabulary_id] = re.compile(
                r"\b("
                + "|".join(
                    [re.escape(term) for term in vocabularies[vocabulary_id] if term]
                )
                + r")\b"
            )
