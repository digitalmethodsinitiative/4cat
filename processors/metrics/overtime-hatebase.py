...
class OvertimeHatefulAnalysis(BasicProcessor):
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        """
        return module.parameters.get("datasource") in (
            "telegram",
            "instagram",
            "reddit",
            "upload",
    ...
    def process(self):
        ...
        hatebase_regex = re.compile(
            ...
            + "|".join(
                [
                    re.escape(term)
                    for term in hatebase
                    if not min_offensive
                    or (
            ...
            if timeframe != "all" and not post.get("timestamp"):
                ...
                try:
                    ...
                    self.dataset.update_status(
                        "%s, cannot count items per %s" % (str(e), timeframe),
                        is_final=True,
                    )
        ...
        for interval in sorted(intervals):
            ...
            rows.append(
                {"date": interval, "item": "messages", "value": activity[interval]}
            )
            if engagement_field:
                rows.append(
                    {
