    ...
    def filter_items(self):
        ...
        if not dataset_size:
            self.dataset.finish_with_error(
                "Could not retrieve the amount of rows for the parent dataset"
            )
        ...
        try:
            ...
            self.dataset.update_status(
                "Use a valid integer as a sample size", is_final=True
            )
            self.dataset.finish(0)
            return
        if not sample_size:
            self.dataset.update_status(
                "Invalid sample size %s" % sample_size, is_final=True
            )
            self.dataset.finish(0)
            return
        elif sample_size > dataset_size:
            self.dataset.update_status(
                "The sample size can't be larger than the dataset size", is_final=True
            )
            self.dataset.finish(0)
            return
