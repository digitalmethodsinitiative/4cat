...
class ColumnFilter(BaseFilter):
    ...
    description = (
        "A generic filter that checks whether a value in a selected column matches a custom requirement. "
        "This will create a new dataset."
    )

    options = {
        "column": {"type": UserInput.OPTION_CHOICE, "help": "Column"},
        "match-style": {
    ...
    def filter_items(self):
        ...
        match_values = [
            value.strip().lower() if force_lowercase else value.strip()
            for value in self.parameters.get("match-value").split(",")
        ]
        ...
        if match_style in ("less-than", "greater-than"):
            try:
                ...
                self.dataset.update_status(
                    "Cannot do '%s' comparison with non-numeric value(s)", is_final=True
                )
                self.dataset.finish(0)
                return

        elif match_style in ("top", "bottom"):
            for item in self.filter_top(
                column, match_values[0], (match_style == "bottom")
            ):
                yield item
        ...
        # pre-process dates to compare to
            ...
            if not ok_format:
                try:
                    ...
                    self.dataset.update_status(
                        "Cannot do '%s' comparison with value(s) that are not dates"
                        % match_style,
                        is_final=True,
                    )
                    self.dataset.finish(0)
        ...
        for mapped_item in self.source_dataset.iterate_items(processor=self):
            ...
            if processed_items % 500 == 0:
                self.dataset.update_status(
                    "Processed %i items (%i matching)"
                    % (processed_items, matching_items)
                )
            ...
            if match_style in ("before", "after"):
                ...
                if re.match(
                    ...
                    mapped_item.get(column),
                ):
                    date_compare = datetime.datetime.strptime(
                        mapped_item.get(column), "%Y-%m-%d %H:%M:%S"
                    ).timestamp()
                else:
                    try:
                        ...
                        self.dataset.update_status(
                            "Invalid date value '%s', cannot determine if before or after"
                            % mapped_item.get(column),
                            is_final=True,
                        )
                        self.dataset.finish(0)
                ...
                if type(mapped_item.get(column)) is str:
                    # Text
                    column_value = (
                        mapped_item.get(column).lower()
                        if force_lowercase
                        else mapped_item.get(column)
                    )
            ...
            matches = False
            if match_style == "exact" and match_function(
                [column_value == value for value in match_values]
            ):
                matches = True
            elif match_style == "exact-not" and match_function(
                [column_value != value for value in match_values]
            ):
                matches = True
            elif match_style == "contains" and match_function(
                [value in column_value for value in match_values]
            ):
                matches = True
                ...
                matches = True
            elif match_style == "after" and match_function(
                [value <= date_compare for value in match_values]
            ):
                matches = True
            elif match_style == "before" and match_function(
                [value >= date_compare for value in match_values]
            ):
                matches = True
            else:
                ...
                try:
                    if match_style == "greater-than" and match_function(
                        [
                            float(value) < float(mapped_item.get(column))
                            for value in match_values
                        ]
                    ):
                        matches = True
...
class ColumnProcessorFilter(ColumnFilter):
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        """
        return not module.is_top_dataset() and module.get_extension() in (
            "csv",
            "ndjson",
        )
