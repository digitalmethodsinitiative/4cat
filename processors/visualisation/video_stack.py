...
class VideoStack(BasicProcessor):
    ...
    description = (
        "Create a video stack from the videos in the dataset. Videos are layered on top of each other "
        "transparently to help visualise similarities. Does not work well with more than a dozen or so "
        "videos. Videos are stacked by length, i.e. the longest video is at the 'bottom' of the stack."
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        if not (
            ...
            ffmpeg_path = shutil.which(
                config.get("video-downloader.ffmpeg_path", user=user)
            )
            ffprobe_path = (
    ...
    def process(self):
        ...
        if self.source_dataset.num_rows < 3:
            self.dataset.update_status(
                "Not enough videos to stack (need at least 2)", is_final=True
            )
        ...
        if not video_dataset:
            ...
            return self.dataset.finish_with_error(
                "Video data missing. Cannot stack videos."
            )
        ...
        max_videos = min(
            amount, self.source_dataset.num_rows - 1
        )  # minus 1, because .metadata.json
        lengths = {}
        videos = []

        # unpack videos and determine length of the video (for sorting)
        for video in self.iterate_archive_contents(
            video_dataset.get_results_path(),
            staging_area=video_staging_area,
            immediately_delete=False,
        ):
            if self.interrupted:
                shutil.rmtree(video_staging_area, ignore_errors=True)
                return ProcessorInterruptedException(
                    "Interrupted while unpacking videos"
                )
            ...
            if length_error:
                shutil.rmtree(video_staging_area, ignore_errors=True)
                return self.dataset.finish_with_error(
                    "Cannot determine length of video {video.name}. Cannot stack "
        ...
        for video in videos:
            ...
            if index == 0:
                ...
                if index < last_index - 1:
                    merge_filter.append(
                        f"[stage{index}][output{index + 1}]{overlay}[stage{index + 1}]"
                    )
                else:
                    # second to last video overlays last video and marks merge filter as final
                    merge_filter.append(
                        f"[stage{index}][output{index + 1}]{overlay}[final]"
                    )
            else:
        ...
        ffmpeg_filter = oslex.quote(
            ";".join(transparency_filter) + ";" + ";".join(merge_filter)
        )[1:-1]
        ...
        self.dataset.update_status(
            "Merging video files with ffmpeg (this can take a while)"
        )
        ...
        if with_errors:
            self.dataset.update_status(
                "Stack created, but with errors. Check dataset log for details.",
                is_final=True,
            )
