...
class ImageCategoryWallGenerator(BasicProcessor):
    ...
    def get_options(cls, parent_dataset=None, config=None):
        ...
        if max_number_images == 0:
            options['width_amount']['tooltip'] = options['width_amount'][
                                                     'tooltip'] + ";'0' will use all available images"
        ...
        if parent_columns:
            ...
            default_options = [default for default in ["top_categories", "impression_count", "category", "type"] if
                               default in parent_columns]
            if default_options:
    ...
    def identity_dataset_types(source_dataset):
        ...
        if any([source_dataset.type.startswith(dataset_prefix) for dataset_prefix in
    ...
    def process(self):
        ...
        if special_case and category_column == "top_categories":
            ...
            if category_type is int:
                ranges = [(math.floor(min_value + range_size * i), math.ceil(min_value + range_size * (i + 1))) for i in
                          range(self.number_of_ranges)]
            else:
                ranges = [(min_value + range_size * i, min_value + range_size * (i + 1)) for i in
                          range(self.number_of_ranges)]
            ranges.reverse()
        ...
        for category, images in categories.items():
            ...
            for i, image in enumerate(images):
                ...
                image_filenames = filename_map.get(image.get("id")) if images_per_item == "all" else [
                    filename_map.get(image.get("id"), [None])[0]]
                    ...
                    if category_type in [float, int]:
                        if category_type is int:
                            ...
                            image_score = f"{image.get('value'):.2f}" + (
                                "%" if special_case and (category_column == "top_categories") else "")
        ...
        canvas = get_4cat_canvas(self.dataset.get_results_path(), canvas_width,
