...
class ImageCategoryWallGenerator(BasicProcessor):
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        return (
            ...
            and not module.type not in ["image-downloader-screenshots-search"]
        )
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        max_number_images = int(
            config.get("image-visuals.max_per_cat", 1000, user=user)
        )
        max_pixels = int(
            config.get("image-visuals.max_pixels_per_image", 300, user=user)
        )
        options = {
            "category": {"type": UserInput.OPTION_TEXT, "help": "Category column"},
            "width_amount": {
                "type": UserInput.OPTION_TEXT,
                "help": "Max images per category"
                + (f" (max {max_number_images:,})" if max_number_images != 0 else ""),
        ...
        if max_number_images == 0:
            options["width_amount"]["tooltip"] = (
                options["width_amount"]["tooltip"]
                + ";'0' will use all available images"
            )
    ...
    def identity_dataset_types(source_dataset):
        ...
        if any(
            [
                source_dataset.type.startswith(dataset_prefix)
                for dataset_prefix in ImageCategoryWallGenerator.image_datasets
            ]
        ):
        ...
        # Or is the parent dataset an image dataset?
            ...
            [
                source_dataset.get_parent().type.startswith(dataset_prefix)
                for dataset_prefix in ImageCategoryWallGenerator.image_datasets
            ]
    ...
    def process(self):
        ...
        Process the job
        """
        image_dataset, category_dataset = self.identity_dataset_types(
            self.source_dataset
        )
        if image_dataset is None or category_dataset is None:
            self.dataset.finish_with_error(
                "Unable to indentify image and category datasets"
            )
            return
        ...
        if image_dataset.num_rows == 0 or category_dataset == 0:
            self.dataset.finish_with_error(
                "No images/categories available to render to image wall."
            )
            return
        self.dataset.log(
            f"Found {image_dataset.type} w/ {image_dataset.num_rows} images and {category_dataset.type} w/ {category_dataset.num_rows} items"
        )
        ...
        if image_dataset.type == "video-hasher-1":
            ...
            filename_map = {
                filename.stem + ".mp4": [filename]
                for filename in staging_area.iterdir()
            }
        elif special_case:
            # We know the post ID is the filename.stem as this dataset is derived from the image dataset
            filename_map = {
                filename.stem: [filename] for filename in staging_area.iterdir()
            }
        else:
                ...
                if image.get("success"):
                    for post_id in image.get("post_ids"):
                        if post_id not in filename_map:
                            filename_map[post_id] = [
                                staging_area.joinpath(image.get("filename"))
                            ]
                        else:
                            filename_map[post_id].append(
                                staging_area.joinpath(image.get("filename"))
                            )
                ...
                for i, post in enumerate(category_dataset.iterate_items(self)):
                    if self.interrupted:
                        raise ProcessorInterruptedException(
                            "Interrupted while collecting categories"
                        )
                    ...
                    if special_case and category_column == "top_categories":
                        ...
                        if not top_cats:
                            self.dataset.log(
                                f"Post {post.get('id')} has no top categories; skipping"
                            )
                            continue
                        ...
                        if category_type is str:
                            ...
                            if post.get(category_column) is None:
                                self.dataset.log(
                                    f"Post {post.get('id')} has no data; skipping"
                                )
                                continue
                            try:
        ...
        if special_case and category_column == "top_categories":
            ...
            for cat in categories:
                categories[cat] = sorted(
                    categories[cat], key=lambda x: x.get("value"), reverse=True
                )
        elif category_type is float:
            ...
            if category_type is int:
                ranges = [
                    (
                        math.floor(min_value + range_size * i),
                        math.ceil(min_value + range_size * (i + 1)),
                    )
                    for i in range(self.number_of_ranges)
                ]
            else:
                ranges = [
                    (min_value + range_size * i, min_value + range_size * (i + 1))
                    for i in range(self.number_of_ranges)
                ]
            ranges.reverse()
            ...
            for value, post_id in post_values:
                for min_value, max_value in ranges:
                    ...
                    ):  # inclusive as an image is the exact min and exact max; we break so images are stored in only first matching range
                        if category_type is int:
                            categories[f"{min_value} - {max_value}"].append(
                                {"id": post_id, "value": value}
                            )
                        else:
                            categories[f"{min_value:.2f} - {max_value:.2f}"].append(
                                {"id": post_id, "value": value}
                            )
                        break
            ...
            categories = {
                cat[0]: cat[1]
                for cat in sorted(
                    categories.items(), key=lambda x: len(x), reverse=True
                )
            }
        ...
        for category, images in categories.items():
            ...
            for i, image in enumerate(images):
                if images_per_category != 0 and i >= images_per_category:
                    ...
                    footersize = (
                        fontsize * (len(remaining) + 2) * 0.5925,
                        fontsize * 2,
                    )
                    footer_shape = SVG(
                        insert=(offset_w, base_height / 2 - footersize[1]),
                        size=footersize,
                    )
                    footer_shape.add(
                ...
                image_filenames = (
                    filename_map.get(image.get("id"))
                    if images_per_item == "all"
                    else [filename_map.get(image.get("id"), [None])[0]]
                )
                if not image_filenames:
                    self.dataset.log(
                        f"Image {image.get('id')} not found in image archive"
                    )
                    continue
                for image_filename in image_filenames:
                    ...
                    frame_element = ImageElement(
                        href=frame_data,
                        insert=(category_widths[category], 0),
                    ...
                    if category_type in [float, int]:
                        if category_type is int:
                            ...
                            image_score = f"{image.get('value'):.2f}" + (
                                "%"
                                if special_case
                                and (category_column == "top_categories")
                                else ""
                            )
                        footersize = (
                            fontsize * (len(image_score) + 2) * 0.5925,
                            fontsize * 2,
                        )
                        footer_shape = SVG(
                            insert=(offset_w, base_height - footersize[1]),
                            size=footersize,
                        )
                        footer_shape.add(
                            Rect(insert=(0, 0), size=("100%", "100%"), fill="#000")
                    ...
                    self.dataset.log(
                        f"Added image {image.get('id')} to category {category}; width {category_widths[category]} height {offset_y}"
                    )
        ...
        canvas = get_4cat_canvas(
            ...
            canvas_width,
            row_height * len(category_widths) + fontsize * 4,
            header=f"Images sorted by {category_column}",
