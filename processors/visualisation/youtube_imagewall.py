...
class YouTubeImageWall(BasicProcessor):
    ...
    def process(self):
        ...
        # Build that wall!
        self.make_imagewall(
            root_csv, max_amount=max_amount, category_overlay=category_overlay
        )
    ...
    def make_imagewall(self, path_to_yt_metadata, max_amount=0, category_overlay=False):
        ...
        for file in files:
            ...
            if counter % 50 == 0:
                self.dataset.update_status(
                    "Placing image " + str(counter) + "/" + str(len(files))
                )
            ...
            image = ImageOps.fit(
                image, (tile_width, tile_height), method=Image.BILINEAR
            )
            if delete_after_use:
                temp_path.unlink()
        ...
        if category_overlay:
            ...
            font = ImageFont.truetype(
                str(
                    config.get("PATH_ROOT").joinpath(
                        "common/assets/Inconsolata-Bold.ttf"
                    )
                ),
                50,
            )
            ...
            for i, category in enumerate(categories_legend):
                ...
                string = (
                    ...
                    + " ("
                    + str(categories[str(category)]["count"])
                    + ")"
                )
                ...
                # Draw the text
                draw.text(
                    (text_width, text_height), string, font=font, fill=(255, 255, 255)
                )
