       ...
       """
        ...
        for file in files:
            ...
            if category_overlay:
                ...
                category = df.loc[df[id_col] == file, "video_category_id"].iloc[0]

                # If the video category is not null
                if not math.isnan(category) and file in image_ids:
                    category = int(category)
                    rgb_colour = categories[str(category)]["colour"]

                    # Add opacity to the colour (150/255) for the overlay
                    # This keeps it distinguishable but visible
                    overlay_colour = rgb_colour + (150,)

                    # Ensure base image is RGBA
                    image = image.convert("RGBA")

                    # Create the semi-transparent overlay
                    overlay = Image.new("RGBA", image.size, overlay_colour)
            ...
            # If there is no category, we just keep the original image (do nothing)
            # rather than creating a transparent (invisible) block.
        ...
        if category_overlay:
            ...
            # Get a font
            font = ImageFont.truetype(str(self.config.get('PATH_ROOT').joinpath("common/assets/Inconsolata-Bold.ttf")),
                                      50)
            ...
            for i, category in enumerate(categories_legend):
                # Get category colour
                # This uses the original opaque RGB value from the dict
                ...
                draw.rectangle((text_width - 15, text_height, text_width + (len(string) * 27), text_height + 70),
                               fill=colour)
                # Draw the text
