...
class VideoHashSimilarities(BasicProcessor):
    ...
    description = ProcessorDescription(
        ...
        description="Compare video hashes and assign each video to a similarity group in a CSV table. Videos are grouped when their hashes are at least the chosen percentage similar, and groups can chain together when videos overlap. Run this on the output of 'Create video collages'.",
        references=[
            "[Video Hash](https://github.com/akamhy/videohash#readme)",
        ],
    )
