...
class VideoHasherPreset(ProcessorAdvancedPreset):
    ...
    title = (
        "Create Video hashes to identify near duplicate videos"  # title displayed in UI
    )
...
class VideoHasher(BasicProcessor):
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        """
        return module.get_media_type() == "video" or module.type.startswith(
            "video-downloader"
        )
    ...
    def process(self):
        ...
        video_hashes = {}
        video_metadata = None
        total_possible_videos = max(
            (
                min(self.source_dataset.num_rows - 1, max_videos)
                if max_videos != 0
        ...
        for path in self.iterate_archive_contents(self.source_file, staging_area):
            if self.interrupted:
                raise ProcessorInterruptedException(
                    "Interrupted while creating video hashes"
                )
            ...
            try:
                videohash = VideoHash(
                    path=str(path),
                    storage_path=str(staging_area),
                    frame_interval=frame_interval,
                    do_not_copy=True,
                )
            except FFmpegNotFound:
                self.log.error(
                    "ffmpeg must be installed for video_hash.py processor to be used."
                )
                self.dataset.update_status(
                    "FFmpeg software not found. Please contact 4CAT maintainers.",
                    is_final=True,
                )
                self.dataset.finish(0)
                return
                ...
                self.dataset.update_status(
                    f"Unable to extract frame for {path.name} (see log for details)"
                )
        ...
        if processed_videos == 0:
            self.dataset.finish_with_error(
                "Unable to create video hashes for any videos"
            )
            return
        ...
        if video_metadata is None:
            ...
            try:
                metadata_path = self.extract_archived_file_by_name(
                    ".metadata.json", self.source_file, output_dir
                )
            except FileNotFoundError:
                metadata_path = None
            if metadata_path:
            ...
            for url, data in video_metadata.items():
                ...
                for file in files:
                    ...
                    if file.get("filename") not in video_hashes:
                        self.dataset.log(
                            f"Metadata Error: {file.get('filename')} with {url} - {data}"
                        )
                        continue
                    ...
                    rows.append(
                        {
                            "id": file.get(
                                "filename"
                            ),  # best if all datasets have unique identifier
                            "url": url,
                            ...
                            "post_ids": ",".join(
                                [str(post_id) for post_id in data.get("post_ids", [])]
                            ),
                            "video_collage_filename": video_hashes[
                                file.get("filename")
                            ].get("video_collage_filename"),
                        }
                    )
                    num_posts += 1

        writer = None
        with output_dir.joinpath("video_hashes.csv").open(
        ...
        # Finish up
        self.dataset.update_status(
            f"Created {num_posts} video hashes and stored video collages"
        )
    ...
    def process(self):
        ...
        with zipfile.ZipFile(self.source_file, "r") as archive_file:
            ...
            if "video_hashes.csv" not in archive_contents:
                self.dataset.update_status(
                    "Unable to obtain hashes from video colleges.", is_final=True
                )
                self.dataset.finish(0)
                return
        ...
        self.dataset.update_status(
            f"Collecting video hashes for {percent_similar * 100}% similar network"
        )
        hashes = []
        identifiers = []
        bit_length = None
        with open(
            staging_area.joinpath("video_hashes.csv"), "r", encoding="utf-8", newline=""
        ) as infile:
        ...
        self.dataset.update_status(
            f"Calculating video hash similarities {percent_similar * 100}% similar"
        )
...
class VideoHashSimilarities(BasicProcessor):
    ...
    def process(self):
        ...
        # Read hash file
        self.dataset.update_status(
            f"Collecting video hashes for {self.parameters.get('percent', 90)}% similar network"
        )
        hashes = []
        identifiers = {}
        bit_length = None
        with staging_area.joinpath("video_hashes.csv").open(
        ...
        self.dataset.update_status(
            f"Calculating video hash similarities {self.parameters.get('percent', 90)}% similar"
        )
        ...
        for vid_hash in hashes:
            ...
            similarity_matrix.append(
                [
                    xor_comparison.sum() <= bits_threshhold
                    for xor_comparison in xor_result
                ]
            )
        ...
        for i, vid in enumerate(all_video_hashes):
            ...
            if sum(similarity_matrix[i]) > 1:
                ...
                group_key_match = set(
                    video_group_key.get(match)
                    for match in group
                    if video_group_key.get(match)
                )
                if len(group_key_match) == 1:
                    ...
                    if len(group_key_match) > 1:
                        self.dataset.log(
                            f"Merging groups to new group for: {group}"
                        )  # this is not all yet
        ...
        with self.dataset.get_results_path().open(
            ...
            with staging_area.joinpath("video_hashes.csv").open(
                ...
                writer = csv.DictWriter(
                    outfile, fieldnames=reader.fieldnames + ["group_id"]
                )
                writer.writeheader()
                for row in reader:
                    video_id = row.get("id")
