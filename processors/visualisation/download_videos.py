...
def url_to_filename(url, extension, directory):
    # Create filename
    filename = re.sub(r"[^0-9a-z]+", "_", url.lower())[
        :100
    ]  # [:100] is to avoid folder length shenanigans
...
class VideoDownloaderPlus(BasicProcessor):
    ...
    description = (
        "Download videos from URLs and store in a zip file. May take a while to complete as videos are "
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        if max_video_size == 0:
            ...
            options["max_video_size"]["tooltip"] = (
                "Set to 0 if all sizes are to be downloaded."
            )
            options["max_video_size"]["min"] = 0
        else:
            # Limit video size
            options["max_video_size"]["max"] = max_video_size
            options["max_video_size"]["default"] = (
                options["max_video_size"]["default"]
                if options["max_video_size"]["default"] <= max_video_size
                else max_video_size
            )
            options["max_video_size"]["tooltip"] = (
                f"Cannot be more than {max_video_size}MB."
            )
        ...
        if parent_dataset and parent_dataset.get_columns():
            ...
            columns.sort(
                key=lambda col: next(
                    (i for i, p in enumerate(priority) if p in col.lower()),
                    len(priority),
                )
            )
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        """
        return (
            (module.type.endswith("-search") or module.is_from_collector())
            and module.type
            not in ["tiktok-search", "tiktok-urls-search", "telegram-search"]
    ...
    def process(self):
        ...
        if self.source_dataset.num_rows == 0:
            self.dataset.update_status(
                "No data from which to extract video URLs.", is_final=True
            )
        ...
        def dmi_match_filter(vid_info, *, incomplete):
            ...
            if vid_info.get("is_live"):
                raise LiveVideoException(
                    "4CAT settings do not allow downloading live videos with this processor"
                )
        ...
        if self.max_videos_per_url == 0:
            ...
            self.max_videos_per_url = (
                1  # Ensure unknown channels only end up with one video downloaded
            )
        ...
        if max_video_size > 0 or max_video_res > 0:
            ...
            if filesize_filter:
                ...
                ydl_opts["format"] = (
                    f"{res_filter}{filesize_filter}/bestvideo{res_filter}{filesize_filter}+bestaudio{filesize_filter}/{res_filter}{filesize_approx_filter}/bestvideo{res_filter}{filesize_approx_filter}+bestaudio{filesize_approx_filter}"
                )
            else:
        ...
        not_a_video = 0
        last_domains = []
        self.total_possible_videos = (
            min(len(urls), amount) if amount != 0 else len(urls)
        )
        yt_dlp_archive_map = {}
        for url in urls:
            if self.interrupted:
            ...
            if url in vid_lib.library:
                ...
                if previous_vid_metadata.get("success", False):
                    ...
                    try:
                        self.dataset.log(
                            f"Copying previously downloaded video for url: {url}"
                        )
                        num_copied = self.copy_previous_video(
                            previous_vid_metadata,
                            results_path,
                            vid_lib.previous_downloaders,
                        )
                        urls[url] = previous_vid_metadata
                        self.dataset.update_status(
                            "Copied previously downloaded video to current dataset."
                        )
                    ...
                    urls[url] = previous_vid_metadata
                    self.dataset.log(
                        f"Skipping; previously identified url as not a video: {url}"
                    )
                    continue
            ...
            if consecutive_errors >= 5:
                if use_yt_dlp:
                    message = (
                        "Downloaded %i videos. Errors %i consecutive times; try "
                        "deselecting the non-direct videos setting"
                        % (self.downloaded_videos, consecutive_errors)
                    )
                else:
                    message = (
                        "Downloaded %i videos. Errors %i consecutive times; check logs to ensure "
                        "video URLs are working links and you are not being blocked."
                        % (self.downloaded_videos, consecutive_errors)
                    )
            ...
            if not_a_video >= 10 and last_domains.count(urlparse(url).netloc) == 5:
                ...
                self.dataset.update_status(
                    f"Too many consecutive non-video URLs encountered; {'try again with Non-direct videos option selected' if self.config.get('video-downloader.allow-indirect') else 'try extracting URLs and filtering dataset first'}.",
                    is_final=True,
                )
                if self.downloaded_videos == 0:
                    self.dataset.finish(0)
            ...
            if not download_channels and any(
                [sub_url in url for sub_url in self.known_channels]
            ):
            ...
            try:
                filename = self.download_video_with_requests(
                    url, results_path, max_video_size
                )
                urls[url]["downloader"] = "direct_link"
                urls[url]["files"] = [
                    {"filename": filename, "metadata": {}, "success": True}
                ]
                success = True
                ...
                if use_yt_dlp:
                    ...
                    ydl_opts["outtmpl"] = (
                        str(results_path)
                        + "/"
                        + re.sub(r"[^0-9a-z]+", "_", url.lower())[:100]
                        + "_%(autonumber)s.%(ext)s"
                    )
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        ...
                        try:
                            ...
                            self.dataset.log(
                                "Already downloaded video associated with: %s" % url
                            )
                            ...
                            with yt_dlp.YoutubeDL({"socket_timeout": 30}) as ydl2:
                                ...
                                if info2:
                                    self.url_files[info2.get("_filename", {})] = (
                                        yt_dlp_archive_map[
                                            info2.get("extractor") + info2.get("id")
                                        ]
                                    )
                                    self.dataset.log(
                                        "Already downloaded video associated with: %s"
                                        % url
                                    )
                                else:
                            ...
                            self.dataset.log(
                                f"YT-DLP raised unexpected error: {str(e)}"
                            )
                    ...
                    for file in self.url_files.values():
                        yt_dlp_archive_map[
                            file.get("metadata").get("extractor")
                            + file.get("metadata").get("id")
                        ] = file
            ...
            self.dataset.update_status(
                f"Downloaded {self.downloaded_videos}/{self.total_possible_videos} videos"
                + (
                    f"; videos copied from {copied_videos} previous downloads"
                    if copied_videos > 0
                    else ""
                )
            ...
            self.dataset.update_progress(
                self.downloaded_videos / self.total_possible_videos
            )
        ...
        self.write_archive_and_finish(
            results_path, self.downloaded_videos + copied_videos
        )
        self.dataset.update_status(
            f"Downloaded {self.downloaded_videos} videos"
            + (
                f"; videos copied from {copied_videos} previous downloads"
                if copied_videos > 0
                else ""
    ...
    def yt_dlp_monitor(self, d):
        ...
        if (
            len(self.videos_downloaded_from_url) != 0
            and len(self.videos_downloaded_from_url) >= self.max_videos_per_url
        ):
    ...
    def yt_dlp_post_monitor(self, d):
        ...
        if d["status"] == "finished":  # "downloading", "error", or "finished"
            self.videos_downloaded_from_url.add(
                d.get("info_dict", {}).get("_filename", {})
            )
    ...
    def download_video_with_requests(
        self, url, results_path, max_video_size, retries=0
    ):
        """
        ...
        try:
            with requests.get(
                ...
                if 400 <= response.status_code < 500:
                    ...
                    raise FailedDownload(
                        f"Unable to obtain URL (Code {response.status_code} / Reason {response.reason}): {url}"
                    )
                ...
                if not content_type:
                    raise VideoStreamUnavailable(
                        f"Unable to verify video; no Content-Type provided: {url}"
                    )
                elif "image" in content_type.lower():
                    raise NotAVideo(
                        "Not a Video (%s): %s" % (response.headers["Content-Type"], url)
                    )
                elif "video" not in content_type.lower():
                    raise VideoStreamUnavailable(
                        f"Does not appear to be a direct to video link: {url}; "
                ...
                if extension not in ["mp4", "mp3"]:
                    self.dataset.log(
                        f"DEBUG: Odd extension type {extension}; Notify 4CAT maintainers if video. "
                ...
                if not max_video_size == 0:
                    if response.headers.get("Content-Length", False):
                        if int(response.headers.get("Content-Length")) > (
                    ...
                    # Size unknown
                        ...
                        "video-downloader.allow-unknown-size", False
                    ):
                        raise FilesizeException(
                            "Video size unknown; not allowed to download per 4CAT settings"
                        )

                # Download video
                self.dataset.update_status(
                    "Downloading %i/%i via requests: %s"
                    % (self.downloaded_videos + 1, self.total_possible_videos, url)
                )
                with open(results_path.joinpath(save_location), "wb") as f:
                    try:
                        for chunk in response.iter_content(chunk_size=1024 * 1024):
                            if not max_video_size == 0 and f.tell() > (
                                max_video_size * 1000000
                            ):
                                # File size too large; stop download and remove file
                                os.remove(f.name)
                                raise FilesizeException(
                                    "Video size larger than maximum allowed per 4CAT"
                                )
                            if chunk:
                                f.write(chunk)
            ...
            return self.download_video_with_requests(
                "https://" + url.lstrip(" :/"),
                results_path,
                max_video_size,
                retries=retries + 1,
    ...
    def collect_video_urls(self):
        ...
        for index, post in enumerate(self.source_dataset.iterate_items(self)):
            ...
            if index + 1 % 250 == 0:
                self.dataset.update_status(
                    "Extracting video links from item %i/%i"
                    % (index + 1, self.source_dataset.num_rows)
                )
    ...
    def copy_previous_video(
        self, previous_vid_metadata, staging_area, previous_downloaders
    ):
        """
        ...
        dataset = [
            dataset for dataset in previous_downloaders if dataset.key == dataset_key
        ]
        ...
        if "files" in previous_vid_metadata:
            ...
            files = [
                {"filename": previous_vid_metadata.get("filename"), "success": True}
            ]
        else:
        ...
        with zipfile.ZipFile(dataset.get_results_path(), "r") as archive_file:
            ...
            for file in files:
                if file.get("filename") not in archive_contents:
                    raise FailedToCopy(
                        f"Previously downloaded video {file.get('filename')} not found"
                    )

                self.dataset.log(
                    f"Copying previously downloaded video {file.get('filename')} to new staging area"
                )
...
class DatasetVideoLibrary:
    def __init__(self, current_dataset):
        ...
        self.current_dataset.log(
            f"Previously video downloaders: {[downloader.key for downloader in self.previous_downloaders]}"
        )
        ...
        for metadata_file in metadata_files:
            for url, data in metadata_file[1].items():
                if data.get("success", False):
                    ...
                    library[url] = {**data, "file_dataset_key": metadata_file[0]}
    ...
    def collect_previous_downloaders(self):
        ...
        previous_downloaders = [
            child
            for child in parent_dataset.get_children()
            if (
                child.type == "video-downloader"
                and child.key != self.current_dataset.key
            )
        ]

        # Check to see if filtered dataset
        if (
            "copied_from" in parent_dataset.parameters
            and parent_dataset.is_top_dataset()
        ):
            try:
                original_dataset = DataSet(
                    key=parent_dataset.parameters["copied_from"],
                    db=self.current_dataset.db,
                )
                previous_downloaders += [
                    child
                    for child in original_dataset.top_parent().get_children()
                    if (
                        child.type == "video-downloader"
                        and child.key != self.current_dataset.key
                    )
    ...
    def collect_all_metadata_files(self):
        ...
        metadata_files = [
            (
                downloader.key,
                self.collect_metadata_file(downloader, metadata_staging_area),
            )
            for downloader in self.previous_downloaders
        ]
        metadata_files = [file for file in metadata_files if file[1] is not None]
        self.current_dataset.log(
            f"Metadata files collected: {len(metadata_files)}; with {[len(urls[0]) for urls in metadata_files]}"
        )
