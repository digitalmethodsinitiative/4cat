...
class ImageWallGenerator(BasicProcessor):
    ...
    def process(self):
        ...
        for path in self.iterate_archive_contents(
            self.source_file, staging_area, immediately_delete=False
        ):
            if self.interrupted:
            ...
            try:
                ...
                self.dataset.update_status(
                    f"File {path.name} could not be parsed. Skipping."
                )
                continue

            self.dataset.update_status(
                f"Analysing {path.name} ({len(dimensions):,}/{self.source_dataset.num_rows:,})"
            )
            self.dataset.update_progress(
                len(dimensions) / self.source_dataset.num_rows / 2
            )
            ...
            if sort_mode not in ("", "random") and (
                picture.height > sample_max or picture.width > sample_max
            ):
                sample_width = int(
                    sample_max * picture.width / max(picture.width, picture.height)
                )
                sample_height = int(
                    sample_max * picture.height / max(picture.width, picture.height)
                )
                try:
                    ...
                    picture = ImageOps.fit(
                        picture, (sample_width, sample_height), method=Image.NEAREST
                    )
            ...
            if sort_mode == "random":
                ...
                if sort_mode == "kmeans-dominant":
                    ...
                    for index in range(0, len(clusters.cluster_centers_)):
                        ranked_centroids[
                            numpy_to_rgb(clusters.cluster_centers_[index])
        ...
        if max_images == 0:
            # Use all images
            sorted_image_files = [
                path for path in sorted(image_colours, key=lambda k: image_colours[k])
            ]
        else:
            sorted_image_files = [
                path
                for path in sorted(image_colours, key=lambda k: image_colours[k])[
                    :max_images
                ]
            ]
        ...
        if not dimensions:
            self.dataset.update_status(
                "No images left after sorting; cannot make image wall", is_final=True
            )
            self.dataset.finish(0)
            return
        ...
        if sizing_mode == "fit-height":
            ...
            ideal_height = math.sqrt(
                fitted_pixels / (self.TARGET_WIDTH / self.TARGET_HEIGHT)
            )
            ...
            ideal_width = math.sqrt(
                fitted_pixels / (self.TARGET_HEIGHT / self.TARGET_WIDTH)
            )
        ...
        for path in sorted_image_files:
            ...
            counter += 1
            self.dataset.update_status(
                f"Rendering {path.name} ({counter:,}/{len(sorted_image_files):,}) to image wall"
            )
            ...
            try:
                picture = ImageOps.fit(
                    picture, (picture_x, tile_y), method=Image.BILINEAR
                )
            except ValueError:
                # BILINEAR may also fail
                picture = ImageOps.fit(
                    picture, (picture_x, tile_y), method=Image.NEAREST
                )
