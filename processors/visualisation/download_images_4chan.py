...
class ImageDownloader(BasicProcessor):
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        if type(module) is DataSet:
            return module.type == "fourchan-search" and "board" in module.parameters
        else:
            return module.type == "fourchan-search"
    ...
    def process(self):
        ...
        if not columns:
            self.dataset.update_status(
                "No columns selected; no images extracted.", is_final=True
            )
            self.dataset.finish(0)
            return
        ...
        external = None
        if datasource in ["4chan", "fourchan"] and (
            "image_md5" in columns or "image_file" in columns
        ):
            external = (
                "boards.fireden.net"
                if top_parent.parameters.get("board") == "v"
                else "archive.4plebs.org"
            )
        ...
        for item in self.source_dataset.iterate_items(self):
            ...
            if item_index % 50 == 0:
                self.dataset.update_status(
                    "Extracting image links from item %i/%i"
                    % (item_index, self.source_dataset.num_rows)
                )
            ...
            if external:
                ...
                local_path = Path(
                    config.get("PATH_IMAGES"), md5.hexdigest() + "." + extension
                )
                if local_path.exists():
                    ...
                    remote_path = "https://%s/_/search/image/%s" % (
                        external,
                        item["image_md5"].replace("/", "_"),
                    )
        ...
        for url in urls:
            ...
            if self.interrupted:
                raise ProcessorInterruptedException(
                    "Interrupted while downloading images."
                )

            processed_urls += 1
            self.dataset.update_status(
                "Downloaded %i/%i images; downloading from %s"
            ...
            if not image_suffix or image_suffix not in (
                ".png",
                ".gif",
                ".jpeg",
                ".jpg",
            ...
            save_location = results_path.joinpath(image_filename).with_suffix(
                image_suffix
            )
            while save_location.exists():
                save_location = results_path.joinpath(
                    image_stem + "-" + str(index) + image_suffix
                )
                index += 1
            ...
            try:
                ...
                self.dataset.log(
                    "ERROR: OSError when saving image %s: %s" % (save_location, e)
                )
                try:
        ...
        metadata = {
            url: {
                ...
                "success": url_file_map.get(url) is not None
                and url not in failures,  # skipped and fails are NOT success
        ...
        self.write_archive_and_finish(
            results_path, len([x for x in metadata.values() if x.get("success")])
        )
    ...
    def get_image(self, url):
        ...
        if domain in ("www.imgur.com", "imgur.com"):
            ...
            # Two formats identified: https://imgur.com/a/randomid, https://imgur.com/randomid
                ...
                try:
                    ...
                    self.dataset.log(
                        "Error: IndexError may be 404 for image %s, skipping"
                        % image_url
                    )
                    raise FileNotFoundError()
                    ...
                    try:
                        self.dataset.log(
                            "Debug: UnicodeDecodeError detected for image %s"
                            % image_url
                        )
                        # Use requests chardet to detect encoding
                        page.encoding = page.apparent_encoding
                        image_url = (
                            page.text.split('<meta property="og:image"')[1]
                            .split('content="')[1]
                            .split('?fb">')[0]
                        )
                    except IndexError:
        ...
        elif domain.endswith("gfycat.com") and url_ext not in (
            "png",
            "jpg",
            "jpeg",
            "gif",
        ...
        # Get the image!
        if (
            domain.endswith("4plebs.org")
            or domain.endswith("fireden.net")
            and image_url
        ):
            image, image_name = self.get_4chan_image(url)
        elif image_url:
            image = self.request_get_w_error_handling(
                image_url,
                stream=True,
                timeout=20,
                headers={
            ...
            if (
                ...
                or image.headers.get("content-type", "")[:5] != "image"
            ):
                raise FileNotFoundError()
            try:
    ...
    def get_4chan_image(self, url):
        ...
        while rate_limited:
            self.log.debug(
                "Rate-limited by external source. Waiting %s seconds." % rate_limited[1]
            )
        ...
        if page.status_code != 200:
            self.dataset.log(
                "ERROR: status from url %s: %i %s"
                % (
                    url,
                    page.status_code,
                    page.reason if hasattr(page, "reason") else "",
                )
            )
        ...
        try:
            ...
            if css("div.alert")(tree):
                if "No results found" in " ".join(
                    [i for i in css("div.alert")(tree)[0].itertext()]
                ):
                    self.dataset.log("ERROR: Image not found: %s" % url)
                    raise FileNotFoundError()
            # This could indicate a change in page layout
            self.dataset.log(
                "ERROR: IndexError while trying to download 4chan image %s: %s"
                % (url, e)
            )
            raise FileNotFoundError()
        except UnicodeDecodeError:
            self.dataset.log(
                "ERROR: 4chan image search could not be completed for image %s, skipping"
                % url
            )
            raise FileNotFoundError()
        ...
        if image.status_code != 200:
            self.dataset.log(
                "DEBUG: 4chan image %s status code: %i" % (image_url, image.status_code)
            )
            thumbnail_url = (
                ".".join(image_url.split(".")[:-1]) + "s." + image_url.split(".")[-1]
            )
            image = self.request_get_w_error_handling(
                thumbnail_url, stream=True, headers=headers
            )

        if image.status_code != 200:
            self.dataset.log(
                "DEBUG: 4chan image thumbnail %s status code: %i"
                % (thumbnail_url, image.status_code)
            )
            raise FileNotFoundError()
    ...
    def request_get_w_error_handling(self, url, retries=0, **kwargs):
        ...
        try:
            ...
            self.dataset.log(
                "Error: Timeout while trying to download image %s: %s" % (url, e)
            )
            raise FileNotFoundError()
        except requests.exceptions.SSLError as e:
            self.dataset.log(
                "Error: SSLError while trying to download image %s: %s" % (url, e)
            )
            raise FileNotFoundError()
        except requests.exceptions.TooManyRedirects as e:
            self.dataset.log(
                "Error: TooManyRedirects while trying to download image %s: %s"
                % (url, e)
            )
            raise FileNotFoundError()
            ...
            if retries < 3:
                ...
                self.dataset.log(
                    "Error: ConnectionError while trying to download image %s: %s"
                    % (url, e)
                )
                raise FileNotFoundError()
            ...
            self.dataset.log(
                "Error: LocationParseError while trying to download image %s: %s"
                % (url, e)
            )
            raise FileNotFoundError()
