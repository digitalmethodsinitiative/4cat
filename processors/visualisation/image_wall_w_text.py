...
class ImageTextWallGenerator(BasicProcessor):
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        max_pixels = int(
            config.get("image-visuals.max_pixels_per_image", 500, user=user)
        )
        options = {
            "amount": {
                ...
                else min(max_number_images, 100),
    ...
    def identity_dataset_types(source_dataset):
        ...
        if source_dataset.type in ImageTextWallGenerator.combined_dataset:
            ...
            [
                ...
                for dataset_prefix in ImageTextWallGenerator.caption_datasets
            ]
        ):
            ...
            if not any(
                [
                    image_dataset.type.startswith(dataset_prefix)
                    for dataset_prefix in ImageTextWallGenerator.image_datasets
                ]
                + [image_dataset.get_media_type() == "image"]
            ):
    ...
    def process(self):
        ...
        if image_dataset.num_rows == 0 or text_dataset.num_rows == 0:
            self.dataset.finish_with_error(
                "No images/captions available to render to image wall."
            )
            return
        self.dataset.log(
        ...
        if text_dataset.type in ImageTextWallGenerator.combined_dataset:
            ...
            metadata_file = self.extract_archived_file_by_name(
                ".metadata.json", self.source_file
            )
            ...
            for item in metadata.values():
                if self.interrupted:
                    raise ProcessorInterruptedException(
                        "Interrupted while collecting text"
                    )
                if "filename" in item:
            ...
            for item in text_dataset.iterate_items(self):
                ...
                if item.get("image_filename", item.get("filename")) is not None:
                    ...
                    image_text = (
                        item.get("text") if item.get("text") is not None else ""
                    )
                    max_text_len = max(max_text_len, len(image_text))
                    filename_to_text_mapping[
                        item.get("image_filename", item.get("filename"))
                    ] = image_text
        ...
        characters_per_line = math.ceil(
            base_height / (fontsize * fontsize_to_pixels_multiplier)
        )  # this is a rough estimate as width can be longer than height (works for square formats)
        rows_of_text = min(
            math.ceil(max_text_len / characters_per_line), 6
        )  # max of 6 rows of text
        ...
        self.dataset.log(
            f"Creating image wall with {max_images} images, size {base_height} and tile type {tile_type}"
        )
        for image_path in self.iterate_archive_contents(
            image_dataset.get_results_path()
        ):
            ...
            if tile_type == "square":
                ...
                frame_width = base_height
                frame = ImageOps.fit(
                    frame, (frame_width, base_height), method=Image.BILINEAR
                )
            else:
                # resize to height
            ...
            frame_element = ImageElement(
                href=frame_data,
                insert=(row_widths[current_row], 0),
            ...
            if filename in filename_to_text_mapping:
                image_text = textwrap.wrap(
                    filename_to_text_mapping[filename],
        ...
        canvas = get_4cat_canvas(
            ...
            row_height * len(row_widths) + fontsize * 4,
            header="Images with captions",
        ...
        if load_errors:
            self.dataset.log(
                f"Unable to load {len(load_errors)} images: {', '.join(load_errors)}"
            )
            self.dataset.update_status(
                f"Created image wall; Unable to load {len(load_errors)} images of {total_images_collected}",
                is_final=True,
            )
