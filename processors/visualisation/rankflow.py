...
class RankFlowRenderer(BasicProcessor):
    ...
    def process(self):
        ...
        only_adjacent_flows = self.parameters.get("only_adjacent_flows", False)
        include_value = self.parameters.get("show_value", False)
        normal_size = self.parameters.get("normalise_size", False)
        filter_incomplete = self.parameters.get("filter_incomplete", False)
        ...
        if filter_incomplete:
            ...
            ignore = [
                label
                for label in known_periods
                if len(known_periods[label]) != len(all_periods)
            ]
        ...
        for row in self.source_dataset.iterate_items(self):
            ...
            if processed % 250 == 0:
                self.dataset.update_status(
                    "Determining RankFlow parameters, item %i/%i"
        ...
        if not items:
            return self.dataset.finish_with_error(
                "No items remain after filtering. Try disabling 'Remove items that "
                "do not occur...'."
            )
        ...
        for period in items:
            items[period] = dict(
                sorted(items[period].items(), key=lambda x: x[1], reverse=True)
            )
            ...
            for item, value in items[period].items():
                if self.interrupted:
                    raise ProcessorInterruptedException(
                        "Interrupted while aggregating items"
                    )
                ...
                label = self.get_label(
                    {"item": item, "value": items[period][item]},
                    with_value=include_value,
                    as_pct=normal_size,
                )
        ...
        box_max_height = box_height * (
            3 if normal_size else 6
        )  # how dramatic are the size changes?
        ...
        for period in items:
            ...
            for item in items[period]:
                ...
                if self.interrupted:
                    raise ProcessorInterruptedException(
                        "Interrupted while rendering items"
                    )
                ...
                height = (
                    int(max(box_height, box_max_height * weight_factor))
                    if size_property != "none" and weighted
                    else box_height
                )
                ...
                if colour_property == "item":
                    ...
                    change_factor = (
                        0
                        if not weighted or change <= 0
                        else (changes[period][item] / max_change)
                    )
                    colour = base_colour.copy()
                    colour[0] += (1 - base_colour[0]) * (
                        weight_factor if colour_property == "weight" else change_factor
                    )
                    opacity = "0.35"
                    text_colour = "black"
                ...
                # period as well
                if (item in previous and not only_adjacent_flows) or item in previous_adjacent:

                    # Skip flows if not in the directly previous time slice
                    if only_adjacent_flows and item not in previous_adjacent:
                        continue
                    ...
                    # steep
                    top_offset = (
                        box["x"] - previous_box["x"] + previous_box["width"]
                    ) / 2
                    control_top_left = (
                        previous_box["x"] + previous_box["width"] + top_offset,
                        previous_box["y"],
                    )
                    ...
                    flow.push(
                        "C %f %f %f %f %f %f"
                        % (*control_top_left, *control_top_right, box["x"], box["y"])
                    )  # top bezier
                    flow.push(
                        "L %f %f" % (box["x"], box["y"] + box["height"])
                    )  # right boundary
                    flow.push(
                        "C %f %f %f %f %f %f"
                        % (
                ...
                previous_adjacent.append(item)
                previous_boxes[item] = box

                box_start_y += height + box_gap_y

            previous_adjacent = previous
    ...
    def get_label(self, row, with_value=False, as_pct=False):
        ...
        if with_value and row["value"] != 1:
            if row["value"] != int(row["value"]):
                label_weight = (
                    "{0:.2g}".format(row["value"])
                    if not as_pct
                    else str(int(row["value"] * 100)) + "%"
                )
            else:
