...
class VideoFrames(BasicProcessor):
    ...
    def process(self):
        ...
        for i, path in enumerate(
            self.iterate_archive_contents(self.source_file, staging_area)
        ):
            if self.interrupted:
            ...
            if result.returncode != 0:
                self.dataset.update_status(
                    f"Unable to extract frames from video {vid_name} (see logs for details)"
                )
                self.dataset.log(
                    "Error Return Code (%s) with video %s: %s"
                    % (
                ...
                processed_videos += 1
                self.dataset.update_status(
                    "Created frames for %i of %i videos"
        ...
        make_archive(
            self.dataset.get_results_path().with_suffix(""), "zip", output_directory
        )
