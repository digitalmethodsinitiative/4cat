    ...
    def process(self):
        ...
        if len(graphs) > top:
            selected_graphs = {
                ...
                for graph in sorted(
                    graphs,
                    key=lambda x: sum([graphs[x][interval] for interval in graphs[x]]),
                    reverse=True,
                )[0:top]
            }
        ...
        for graph in graphs:
            missing[graph], graphs[graph] = pad_interval(
                graphs[graph], first_interval=first_date, last_interval=last_date
            )
        ...
        if completeness > 0:
            ...
            graphs = {
                graph: graphs[graph] for graph in graphs if graph not in disqualified
            }
        ...
        limits = {
            limit: limits[limit] for limit in sorted(limits, key=lambda i: limits[i])
        }
        ...
        for i in range(0, len(intervals)):
            if (
                re.match(r"^[0-9]{4}-[0-9]{2}", intervals[i])
                and int(intervals[i].split("-")[1]) > 12
            ):
                is_week = True
                break
        ...
        for i in range(0, len(intervals)):
            if i % skip == 0:
                canvas.add(
                    Line(
                        start=(gridline_x, gridline_y),
                        end=(gridline_x - y_axis_width, gridline_y - y_axis_height),
                        stroke="grey",
                        stroke_width=0.25,
                    )
                )
        ...
        for graph in reversed(list(graphs)):
            ...
            for interval in graphs[graph]:
                ...
                if previous_value is None:
                    ...
                    area_graph.push(
                        "C %f %f %f %f %f %f"
                        % (*control_left, *control_right, graph_x, graph_y - value)
                    )
            ...
            area_graph.push(
                "L %f %f" % (graph_x - step_x_horizontal, graph_y + step_y_horizontal)
            )
        ...
        for graph in graphs:
            ...
            canvas.add(
                Line(
                    start=(gridline_x, gridline_y),
                    end=(gridline_x + x_axis_width, gridline_y - x_axis_height),
                    stroke="black",
                    stroke_width=1,
                )
            )
