...
class VideoSceneDetector(BasicProcessor):
    ...
    description = (
        "Detect distinct 'scenes' in videos based on various parameters (e.g. change in color and "
        "intensity or cuts and fades to black) and extract the scene metadata."
    ...
    def process(self):
        ...
        if self.source_dataset.num_rows <= 1:
            ...
            self.dataset.update_status(
                "No videos from which to extract scenes.", is_final=True
            )
            self.dataset.finish(0)
            return

        self.dataset.update_status("Detecting video scenes")
        total_possible_videos = (
            self.source_dataset.num_rows - 1
        )  # for the metadata file that is included in archives
        processed_videos = 0
        video_metadata = None
        collected_scenes = {}
        for path in self.iterate_archive_contents(self.source_file):
            if self.interrupted:
                raise ProcessorInterruptedException(
                    "Interrupted while detecting video scenes"
                )
            ...
            try:
                ...
                self.dataset.update_status(
                    f"Skipping video; Unable to open {str(path.name)}: {str(e)}"
                )
                continue
            ...
            processed_videos += 1
            self.dataset.update_status(
                "Detected scenes for %i of %i videos"
                % (processed_videos, total_possible_videos)
            )
        ...
        if video_metadata is None:
                    ...
                    rows.append(
                        {
                            **{
                                "id": filename
                                + "_scene_"
                                + str(i),  # best if all datasets have unique identifier
                                "filename": filename,
                            },
                            **scene,
                        }
            ...
            for url, video_data in video_metadata.items():
                if video_data.get("success"):
                    files = (
                        video_data.get("files")
                        if "files" in video_data
                        else [{"filename": video_data.get("filename"), "success": True}]
                    )
                    for file in files:
                        if (
                            not file.get("success")
                            or file.get("filename") not in collected_scenes
                        ):
                            continue

                        # List types are not super fun for CSV
                        if "post_ids" in video_data:
                            video_data["post_ids"] = ",".join(
                                [str(i) for i in video_data["post_ids"]]
                            )

                        for i, scene in enumerate(
                            collected_scenes[file.get("filename")]
                        ):
                            rows.append(
                                {
        ...
        if rows:
            ...
            return self.dataset.finish_with_error(
                "No distinct scenes could be detected in the videos. The videos may "
    ...
    def get_new_scene_manager(self):
        ...
        if self.parameters.get("detector_type") == "content_detector":
            ...
            scene_manager.add_detector(
                ContentDetector(
                    threshold=self.parameters.get("cd_threshold"),
            ...
            scene_manager.add_detector(
                AdaptiveDetector(
                    adaptive_threshold=self.parameters.get("ad_adaptive_threshold"),
                    min_scene_len=self.parameters.get("min_scene_len"),
                    luma_only=self.parameters.get("luma_only"),
            ...
            scene_manager.add_detector(
                ThresholdDetector(
                    threshold=self.parameters.get("td_threshold"),
                    min_scene_len=self.parameters.get("min_scene_len"),
