...
class TextClassifier(BasicProcessor):
    ...
    def get_progress_callback(num_items):
        ...
        :param int num_items:  Total number of items that will be processed.
        :return: Callback
        """

        def callback(manager):
            if manager.local_or_remote == "local":
                current_completed = manager.count_local_lines(
                    manager.processor.config.get("PATH_DATA").joinpath(manager.path_to_results))
            elif manager.local_or_remote == "remote":
                current_completed = manager.request_file_lines(manager.server_file_collection_name)

            manager.processor.dataset.update_status(
                f"Classified {current_completed:,} of {num_items:,} item(s)")
    ...
    def process(self):
        ...
        results_folder_name = f"text_classifier_{self.dataset.key}"
        ...
        try:
            dmi_service_manager.send_request_and_wait_for_results(api_endpoint, data, wait_period=5, check_process=None,
                                                                  callback=self.get_progress_callback(self.source_dataset.num_rows))
