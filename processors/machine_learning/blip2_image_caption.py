...
class CategorizeImagesCLIP(BasicProcessor):
    ...
    title = (
        "Generate image captions using OpenAI's BLIP2 model"  # title displayed in UI
    )
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        """
        return (
            config.get("dmi-service-manager.fc_blip2_enabled", False, user=user)
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        max_number_images = int(
            config.get("dmi-service-manager.fd_blip2_num_files", 100, user=user)
        )
        if max_number_images == 0:  # Unlimited allowed
            ...
            options["amount"]["tooltip"] = (
                "Use '0' to categorize all images (this can take a very long time)"
            )
        else:
    ...
    def process(self):
        ...
        for file in self.iterate_archive_contents(
            self.source_file, staging_area=staging_area, immediately_delete=False
        ):
            if self.interrupted:
                raise ProcessorInterruptedException(
                    "Interrupted while unpacking archive"
                )
            ...
            if self.parameters.get(
                "amount", 100
            ) != 0 and total_image_files >= self.parameters.get("amount", 100):
                break
        ...
        if (
            gpu_response
            and int(gpu_response.get("memory", {}).get("gpu_free_mem", 0)) < 1000000
        ):
        ...
        data = {
            "args": [
                ...
                "--image-folder",
                f"data/{path_to_files}",
                "--max_new_tokens",
                str(self.parameters.get("max_new_tokens", 20)),
                "--dataset-name",
                f"{self.dataset.key}",
            ],
        ...
        try:
            dmi_service_manager.send_request_and_wait_for_results(
                api_endpoint, data, wait_period=30
            )
            ...
            self.log.warning(
                f"DMI Service Manager connection error ({self.dataset.key}): {e}"
            )
            self.dataset.finish_with_error(
                "DMI Service Manager connection error; please contact 4CAT admins."
            )
            return
        except DmiServiceManagerException as e:
            self.dataset.log(str(e))
            ...
            self.dataset.finish_with_error(
                "Error with BLIP2 model; please contact 4CAT admins."
            )
            return
        ...
        try:
            metadata_file = self.extract_archived_file_by_name(
                ".metadata.json", self.source_file, staging_area
            )
            ...
            with open(metadata_file) as file:
                ...
                for url, data in image_data.items():
                    if data.get("success"):
                        ...
                        image_metadata[".".join(data["filename"].split(".")[:-1])] = (
                            data
                        )
        else:
        ...
        with self.dataset.get_results_path().open(
            ...
            with open(
                output_dir.joinpath(f"{self.dataset.key}.ndjson"), "r"
            ) as result_file:
                for line in result_file:
                    ...
                    for filename, result_data in data.items():
                        ...
                        data = {
                            ...
                            **result_data,
                            "image_metadata": image_metadata.get(
        ...
        self.dataset.update_status(
            f"Generated captions/responses in {processed} of {total_image_files} images"
        )
        ...
        return MappedItem(
            {
                ...
                if image_metadata
                else item.get("id"),  # fallback to id which is filename
                "original_url": image_metadata.get("url", "N/A"),
                "post_ids": ", ".join(
                    [str(post_id) for post_id in image_metadata.get("post_ids", [])]
                ),
