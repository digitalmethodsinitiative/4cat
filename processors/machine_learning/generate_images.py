...
class StableDiffusionImageGenerator(BasicProcessor):
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        if max_prompts == 0:
            ...
            options["amount"]["help"] = (
                f"Number of images to generate (max {max_prompts})"
            )
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        """
        return (
            config.get("dmi-service-manager.sd_enabled", False, user=user)
        ...
        def callback(manager):
            if manager.local_or_remote == "local":
                current_completed = manager.count_local_files(
                    manager.processor.config.get("PATH_DATA").joinpath(
                        manager.path_to_results
                    )
                )
            elif manager.local_or_remote == "remote":
                existing_files = manager.request_folder_files(
                    manager.server_file_collection_name
                )
                current_completed = len(
                    existing_files.get(manager.server_results_folder_name, [])
                )
    ...
    def process(self):
        ...
        try:
            gpu_response = dmi_service_manager.check_gpu_memory_available(
                "stable_diffusion"
            )
        ...
        with output_dir.joinpath(".metadata.json").open("w") as outfile:
            metadata = {
                prompt_id: {
                    ...
                    "success": output_dir.joinpath(
                        make_filename(prompt_id, data["prompt"])
                    ).exists(),
                    "post_ids": [prompt_id],
                    "prompt": data["prompt"],
        ...
        self.write_archive_and_finish(
            output_dir, num_items=len([r for r in metadata.values() if r["success"]])
        )
