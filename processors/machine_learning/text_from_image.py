...
class ImageTextDetector(BasicProcessor):
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        """
        return (
            config.get("dmi-service-manager.eb_ocr_enabled", False, user=user)
    ...
    def process(self):
        ...
        skipped_images = 0
        metadata_file = None
        for image in self.iterate_archive_contents(
            self.source_file, staging_area=staging_area, immediately_delete=False
        ):
            if self.interrupted:
                raise ProcessorInterruptedException(
                    "Interrupted while unzipping images"
                )
        ...
        try:
            ...
            self.dataset.finish_with_error(
                f"Unable to connect to DMI Service Manager: {e}"
            )
            return
        ...
        try:
            dmi_service_manager.send_request_and_wait_for_results(
                api_endpoint, data, wait_period=30, check_process=True
            )
            ...
            self.dataset.finish_with_error(
                f"Error with {model_type} model; please contact 4CAT admins."
            )
            return
        ...
        if metadata_file is None:
            try:
                self.extract_archived_file_by_name(
        ...
        if update_original:
            if not metadata_exists:
                self.dataset.update_status(
                    "No metadata file found, cannot update original dataset"
                )
            ...
            for result_filename in os.listdir(output_dir):
                ...
                with open(output_dir.joinpath(result_filename), "r") as result_file:
                    ...
                    if update_original:
                        ...
                        detected_text = '%s:"""%s"""' % (
                            image_name,
                            result_data.get("simplified_text", {}).get("raw_text", ""),
                        )
        ...
        if update_original:
            ...
            try:
                self.add_field_to_parent(
                    field_name="4CAT_detexted_text",
        ...
        if self.parameters.get("update_original", False) and not update_original:
            self.dataset.update_status(
                f"{detected_message} No metadata file found, unable to update original dataset.",
                is_final=True,
            )
        else:
        ...
        return MappedItem(
            {
                ...
                "post_ids": ", ".join(
                    [
                        str(post_id)
                        for post_id in item.get("image_metadata", {}).get(
                            "post_ids", []
                        )
                    ]
                ),
