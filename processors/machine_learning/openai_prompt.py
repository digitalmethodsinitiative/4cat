...
class OpenAI(BasicProcessor):
    ...
    description = (
        "Use OpenAI's LLMs or local models via LM Studio to generate text based on the parent dataset. "
        "This can be used for tasks like classification or entity extraction."
    ...
    def process(self):
        ...
        if not consent and not model == "local-lmstudio":
            self.dataset.finish_with_error(
                "You must consent to your data being sent to OpenAI first"
            )
            return
        ...
        if model == "local-lmstudio":
            client = openai.OpenAI(
                api_key="lm-studio", base_url=self.parameters.get("lmstudio_server")
            )
        else:
        ...
        for item in self.source_dataset.iterate_items():
            ...
            for replacement in replacements:
                try:
                    ...
                    self.dataset.finish_with_error(
                        "Field %s could not be found in the parent dataset" % str(e)
                    )
                    return

            try:
                response = self.prompt_gpt(
                    prompt,
                    client,
                    model=model,
            ...
            results.append(
                {"id": item_id, "prompt": prompt, model + " output": response}
            )
