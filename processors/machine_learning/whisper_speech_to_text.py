...
class AudioToText(BasicProcessor):
    ...
    description = (
        "Detect speech in audion and convert to text."  # description displayed in UI
    )
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        """
        return (
            config.get("dmi-service-manager.bc_whisper_enabled", False, user=user)
            and config.get("dmi-service-manager.ab_server_address", False, user=user)
            and (
                module.get_media_type() == "audio"
                or module.type.startswith("audio-extractor")
            )
        )
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        max_number_audio_files = int(
            config.get("dmi-service-manager.bd_whisper_num_files", 100, user=user)
        )
        if max_number_audio_files == 0:  # Unlimited allowed
            ...
            options["amount"]["tooltip"] = (
                "Use '0' to convert all audio (this can take a very long time)"
            )
        else:
            options["amount"]["help"] = (
                f"Number of audio files (max {max_number_audio_files})"
            )
    ...
    def process(self):
        ...
        if advanced_settings:
            try:
                ...
                self.dataset.finish_with_error(
                    "Unable to parse Advanced settings. Please format as JSON."
                )
                return
        ...
        try:
            dmi_service_manager.send_request_and_wait_for_results(
                whisper_endpoint, data, wait_period=30
            )
            ...
            for result_filename in os.listdir(output_dir):
                ...
                with open(output_dir.joinpath(result_filename), "r") as result_file:
                    ...
                    fourcat_metadata = {
                        ...
                        "audio_metadata": video_metadata.get(audio_name, {})
                        if video_metadata
                        else {},
                    }
        ...
        self.dataset.update_status(
            f"Detected speech in {processed} of {total_audio_files} audio files"
        )
        ...
        return MappedItem(
            {
                ...
                "segments": ",\n".join(
                    [
                        f"text: {segment['text']} (start: {segment['start']}; end: {segment['end']}; )"
