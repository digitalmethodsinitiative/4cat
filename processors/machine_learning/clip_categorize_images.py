...
class CategorizeImagesCLIP(BasicProcessor):
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        """
        return (
            config.get("dmi-service-manager.cc_clip_enabled", False, user=user)
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        """
        options = {
            "amount": {"type": UserInput.OPTION_TEXT},
            # CLIP model options
        ...
        max_number_images = int(
            config.get("dmi-service-manager.cd_clip_num_files", 100, user=user)
        )
    ...
    def process(self):
        ...
        if self.source_dataset.num_rows <= 1:
            ...
            return
        categories = [
            cat.strip() for cat in self.parameters.get("categories").split(",")
        ]
        ...
        for filename in os.listdir(staging_area):
            ...
            if filename.split(".")[-1] in ["svg"]:
                skipped_images += 1
                self.dataset.log(
                    f"Skipping {filename} (SVG files cannot be processed currently)"
                )
        ...
        data["args"].extend(
            [
                f"data/{path_to_files.joinpath(dmi_service_manager.sanitize_filenames(filename))}"
        ...
        try:
            ...
            self.dataset.finish_with_error(
                "Error with CLIP model; please contact 4CAT admins."
            )
            return
            ...
            for result_filename in os.listdir(output_dir):
                ...
                with open(output_dir.joinpath(result_filename), "r") as result_file:
                    if not result_filename.endswith(".json"):
                        self.dataset.log(
                            f"Skipping {result_filename} (not a JSON results file)"
                        )
                        continue
                    ...
                    data = {
                        ...
                        "image_metadata": image_metadata.get(image_name, {})
                        if image_metadata
                        else {},
                    }
        ...
        self.dataset.update_status(
            f"Detected speech in {processed} of {total_image_files} images{'' if skipped_images == 0 else f' (skipped {skipped_images} SVG files)'}",
            is_final=True,
        )
        ...
        return MappedItem(
            {
                ...
                "top_categories": ", ".join(
                    [f"{cat[0]}: {100 * cat[1]:.2f}%" for cat in top_cats]
                ),
