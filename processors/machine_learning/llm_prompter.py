...
class LLMPrompter(BasicProcessor):
    ...
    description = (
        "Use LLMs for analysis, via APIs or locally. This can be used for tasks like classification or "
    ...
    def process(self):
        ...
        if not use_local_model and not api_consent:
            self.dataset.finish_with_error(
                "You must consent to your data being sent to the LLM provider first"
            )
            return
        ...
        batches = max(
            1, min(self.parameters.get("batches", 1), self.source_dataset.num_rows)
        )
        ...
        if use_local_model:
            ...
            if provider == "lmstudio":
                ...
                if not model:
                    self.dataset.finish_with_error(
                        "You need to provide a model name for Ollama (e.g. 'llama3.2')"
                    )
                    return
                if not base_url:
                ...
                self.dataset.finish_with_error(
                    "Local provider not supported, choose either lmstudio or ollama"
                )
                return
        else:
            ...
            if api_model == "custom":
                ...
                if not model:
                    self.dataset.finish_with_error(
                        "You must provide a valid API model name/ID"
                    )
                    return
                if not provider:
                    self.dataset.finish_with_error(
                        "You must provide a valid API model provider"
                    )
                    return
            else:
            ...
            api_key = self.parameters.get("api_key") or self.config.get(
                f"api.{provider}.api_key", ""
            )
            if not api_key:
        ...
        if structured_output:
            if not json_schema:
                self.dataset.finish_with_error(
                    "You need to provide a JSON schema for structured outputs."
                )
                return
            try:
                ...
                self.dataset.finish_with_error(
                    "Your JSON schema is not valid JSON (copy/paste to jsonlint.com to check"
                    " what's wrong)."
                )
                return
        ...
        json_schema_original = (
            json_schema or None
        )  # We may manipulate the schema later, save a copy
        ...
        if use_batches:
            # Get a JSON schema
            json_schema = self.get_json_schema_for_batch(
                batches, custom_schema=json_schema
            )
        if json_schema:
            try:
                ...
                self.dataset.finish_with_error(
                    f"Could not use JSON schema for structured output{batch_warning}. "
            ...
            self.dataset.update_status(
                f"Set output structure with the following JSON schema: {json_schema}"
            )
            ...
            for item in self.source_dataset.iterate_items():
                ...
                if self.interrupted:
                    raise ProcessorInterruptedException(
                        "Interrupted while generating text through LLMs"
                    )
                ...
                for column_to_use in columns_to_use:
                    ...
                    try:
                        ...
                        self.dataset.finish_with_error(
                            f"Column(s) '{column_to_use}' not in the parent dataset"
                        )
                        return
                    ...
                    if not item_value and use_batches:
                        item_values = {}
                        self.dataset.update_status(
                            f"Skipping row {row} because of empty value(s) in {column_to_use}"
                        )
                        break
                    else:
                ...
                if not any(v for v in item_values.values()):
                    if item_values.keys():
                        missing_columns = (
                            andify(columns_to_use)
                            if len(columns_to_use) > 1
                            else columns_to_use[0]
                        )
                        self.dataset.update_status(
                            f"Skipping row {row} because of empty value(s) in {missing_columns}"
                        )
                    skipped += 1
                ...
                # Else add the values to the batch
                    ...
                    batched_ids.append(
                        item_id
                    )  # Also store IDs, so we can match them to the output

                i += 1
                # Generate text when we've reached 1) the batch length (which can be 1) or 2) the end of the dataset.
                if n_batched % batches == 0 or (
                    n_batched and row == self.source_dataset.num_rows
                ):
                    ...
                    for column_to_use in columns_to_use:
                        ...
                        prompt_values = (
                            ...
                            else f"```{json.dumps(prompt_values)}```"
                        )
                    ...
                    if row == self.source_dataset.num_rows and use_batches:
                        ...
                        if n_batched != batches and json_schema:
                            json_schema = self.get_json_schema_for_batch(
                                n_batched, custom_schema=json_schema_original
                            )
                    ...
                    batch_str = (
                        f" and {n_batched} items batched into the prompt"
                        if use_batches
                        else ""
                    )
                    self.dataset.update_status(
                        f"Generating text at row {row}/"
                    ...
                    if not response:
                        structured_warning = (
                            " with your specified JSON schema"
                            if structured_output
                            else ""
                        )
                        self.dataset.finish_with_error(
                            f"{model} could not return text{structured_warning}. Consider "
                    ...
                    if use_batches or structured_output:
                        ...
                        if use_batches:
                            ...
                            if len(output) != n_batched:
                                self.dataset.update_status(
                                    f"Output did not result in {n_batched} item(s).\nInput:\n"
                                    f"{prompt}\nOutput:\n{response}"
                                )
                                self.dataset.finish_with_error(
                                    "Model could not output as many values as the batch. See log "
                        ...
                        try:
                            ...
                            self.dataset.finish_with_error(
                                f"Invalid JSON schema and/or LLM output: `{e}`"
                            )
                            return
                    ...
                    for n, output_item in enumerate(output):
                        ...
                        result = {
                            ...
                            "prompt": prompt
                            if not use_batches
                            else base_prompt,  # Insert dataset values if not batching
                            "temperature": temperature,
                            "max_tokens": max_tokens,
                            "model": model,
                        ...
                        if save_annotations:
                            annotation = {
                                ...
                                "value": remove_nuls(json.dumps(output_item))
        ...
        skipped_str = (
            "" if not skipped else f" Skipped {skipped} rows because of empty values."
        )
        self.dataset.update_status(
            f"Finished, {model} generated text in {time_progressed}.{skipped_str}",
            is_final=True,
        )
        ...
        if isinstance(parsed_response, dict):
            ...
            if integer_keys:
                parsed_response = dict(
                    sorted(parsed_response.items(), key=lambda item: int(item[0]))
                )
