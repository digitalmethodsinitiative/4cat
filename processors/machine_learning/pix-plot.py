...
class PixPlotGenerator(BasicProcessor):
    ...
    description = (
        "Put all images from an archive into a PixPlot visualisation: an explorable map of images "
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        max_number_images = int(
            config.get("dmi-service-manager.dc_pixplot_num_files", 10000, user=user)
        )
        if max_number_images == 0:
            options["amount"]["help"] = (
                options["amount"]["help"] + " (max: all available)"
            )
            options["amount"]["min"] = 0
            options["amount"]["tooltip"] = (
                options["amount"]["tooltip"] + " 0 allows as many images as available."
            )
        else:
            options["amount"]["help"] = (
                options["amount"]["help"] + f" (max: {max_number_images})"
            )
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        """
        return (
            config.get("dmi-service-manager.db_pixplot_enabled", False, user=user)
    ...
    def process(self):
        ...
        if self.source_dataset.num_rows == 0:
            self.dataset.update_status(
                "No images available to render to PixPlot.", is_final=True
            )
        ...
        if total_image_files < self.min_photos_needed:
            self.dataset.update_status(
                "Minimum of %i photos needed for a PixPlot to be created"
                % self.min_photos_needed,
                is_final=True,
            )
            self.dataset.finish(0)
            return
        ...
        path_to_files, path_to_results = dmi_service_manager.process_files(
            staging_area,
            image_filenames + ([metadata_file_path] if metadata_file_path else []),
            output_dir,
        ...
        try:
            dmi_service_manager.send_request_and_wait_for_results(
                api_endpoint, data, wait_period=30, check_process=False
            )
        ...
        plot_url = (
            ...
            + "/result/"
            + f"{os.path.relpath(self.dataset.get_results_folder_path(), self.dataset.folder)}/index.html"
        )
    ...
    def format_metadata(self, temp_path):
        ...
        for url, data in image_data.items():
            ...
            if (
                data.get("success")
                and data.get("filename") is not None
                and data.get("post_ids")
            ):
                ...
                images[url] = {
                    "filename": filename,
                    "permalink": url,
                    "description": "<b>Number of posts with this image:</b> "
                    + str(len(data.get("post_ids"))),
                    "tags": "",
                    "number_of_posts": 0,
                }

        self.dataset.log(
            f"Metadata for {successful_image_count} images collected from {len(post_id_image_dictionary)} posts"
        )
            ...
            if post["id"] in post_id_image_dictionary.keys():
                ...
                for img_name in post_id_image_dictionary[post["id"]]:
                    ...
                    for detail in ordered_descriptions:
                        if post.get(detail):
                            image["description"] += (
                                '<p><span class="fieldname">'
                                + detail
                                + "</span>"
                                + self.make_nice_link(post.get(detail))
                                + "</p>"
                            )
                    ...
                    if "timestamp" in post.keys():
                        if not post["timestamp"]:
                            ...
                            image["year"] = datetime.fromtimestamp(
                                post["timestamp"]
                            ).year
                        elif type(post["timestamp"]) is str:
                            try:
                                image["year"] = datetime.strptime(
