...
class TopHashtags(ProcessorPreset):
    ...
    description = (
        "Count how often each hashtag occurs in the dataset and sort by this value"
    )
    extension = "csv"
    ...
    def is_compatible_with(cls, module=None, user=None):
        ...
        return columns and any(
            [tag in columns for tag in CoTaggerPreset.possible_tag_columns]
        )
    ...
    def get_processor_pipeline(self):
        ...
        tag_column = next(
            (col for col in columns if col in CoTaggerPreset.possible_tag_columns), None
        )
