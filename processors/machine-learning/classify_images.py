...
class TextClassifier(BasicProcessor):
    """
    Classify images using image recognition models
    """
    type = "image-classification"  # job type ID
    category = "Visual analysis"  # category
    title = "Classify text using image classification models"  # title displayed in UI
    description = ("Uses a configurable set of image classification models to assign labels to images. Make sure to "
                   "read the model cards for each model to understand the scope of the model and the type of features "
                   "it is able to detect!")  # description displayed in UI
    extension = "ndjson"  # extension of result file, used internally and in UI

    config = {
        "dmi-service-manager.image-classification_intro": {
            "type": UserInput.OPTION_INFO,
            "help": "Image classification",
        },
        "dmi-service-manager.image-classification_enabled": {
            "type": UserInput.OPTION_TOGGLE,
            "default": False,
            "help": "Enable ML-based image classification",
        }
    }
    ...
    options = {
        "features": {
            ...
            "help": "Features",
            "options": {
                "with-labels": "Object presence Detection (google/vit-base-patch16-224)",
                "with-celebrities": "Celebrity Detection (tonyassi/celebrity-classifier)",
                "with-nsfw": "NSFW Detection (Falconsai/nsfw_image_detection)",
            },
            "default": ["with-labels"]
        },
        "confidence-threshold": {
            ...
            "tooltip": "Only include labels if the model is at least this confident about its accuracy, as a number "
                       "between 0 and 1"
        }
    }
    ...
    def get_progress_callback(num_items):
        ...
        :param int num_items:  Total number of images that will be processed.
        ...
        def callback(manager):
            ...
            manager.processor.dataset.update_status(
                f"Annotated {current_completed:,} of {num_items:,} image(s)")
            manager.processor.dataset.update_progress(current_completed / num_items)
