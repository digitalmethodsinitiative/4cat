    ...
    def process(self):
        ...
        if bool(self.dataset.top_parent().parameters.get("pseudonymise", None)):
            self.dataset.update_status(
                "Dataset previously was pseudonymised; not all metrics could be calculated.",
                is_final=True,
            )
        ...
        for post in self.source_dataset.iterate_items(self):
            ...
            if category == "user":
                ...
                if post_category == "REDACTED":
                    raise ProcessorException(
                        "Author information has been removed; cannot calculate user stats"
                    )
            elif category == "type":
                # Identify tweet type(s)
                tweet_types = [
                    ref.get("type") for ref in post.get("referenced_tweets", [])
                ]
                type_results = [
                    "retweet" if "retweeted" in tweet_types else "",
                    "quote" if "quoted" in tweet_types else "",
                    "reply" if "replied_to" in tweet_types else "",
                ]
                type_results.sort()
                post_category = (
                    "-".join([r for r in type_results if r])
                    if any(type_results)
                    else "tweet"
                )
                ...
                post_category = (
                    post.get("geo", {}).get("place", {}).get("full_name", "Unknown")
                )
            ...
            post_values = {
                ...
                "Total Retweets of Tweets": post.get("public_metrics").get(
                    "retweet_count"
                ),
                "Total Replies of Tweets": post.get("public_metrics").get(
                    "reply_count"
                ),
                ...
                "Number of URLs": len(
                    [
                        tag["expanded_url"]
                        for tag in post.get("entities", {}).get("urls", [])
                    ]
                ),
                "Number of Hashtags": len(
...
class TwitterAggregatedStatsVis(TwitterAggregatedStats):
    ...
    def process(self):
        ...
        vis_data = {
            data_type: {"intervals": [], "data": []}
            for data_type in data_types
            if data_type not in ["Created at Timestamp"]
        }
        counter = 0
        for interval, data in intervals.items():
            for data_type in data_types:
        ...
        fig, axs = plt.subplots(
            len(vis_data.keys()),
            1,
            figsize=(max(len(intervals) * 1.5, 10), len(vis_data.keys()) * 10),
            constrained_layout=True,
        )

        for index, (data_type, data) in enumerate(vis_data.items()):
            axs[index].boxplot(
                data["data"], sym="+" if self.parameters.get("show_outliers") else ""
            )
            axs[index].set_title(
                "%s per %s"
                % (data_type.replace(f" per {category}", ""), header_category)
            )
            ...
            if self.parameters.get("label_median"):
                ...
                lower_labels = [
                    str(np.median(plot_values)) for plot_values in data["data"]
                ]
                weights = ["bold", "semibold"]
                for tick, label in zip(
                    range(len(data["data"])), axs[index].get_xticklabels()
                ):
                    k = tick % 2
                    axs[index].text(
                        pos[tick],
