...
class TwitterHashtagStats(TwitterStatsBase):
    ...
    def map_data(self, post):
        ...
        sum_map = {
            ...
            "Number of Retweets of Tweets w/ Hashtag": post.get("public_metrics").get(
                "retweet_count"
            ),
            "Number of Replies to Tweets w/ Hashtag": post.get("public_metrics").get(
                "reply_count"
            ),
            "Number of Likes of Tweets w/ Hashtag": post.get("public_metrics").get(
                "like_count"
            ),
            "Number of Quotes of Tweets w/ Hashtag": post.get("public_metrics").get(
                "quote_count"
            ),
        }
    ...
    def modify_intervals(self, key, data):
        ...
        data["Number of Unique Users w/ Tweets using Hashtag"] = len(
            data["Users of Hashtag"]
        )
        data["Number of Unique Mentions used in Tweets w/ Hashtag"] = len(
            data["Mentions"]
        )
        data["Number of Other Unique Hashtags used in Tweets w/ Hashtag"] = len(
            data["Hashtags"]
        )

        # Rename and format lists
        data["Users w/ Tweets containing Hashtag"] = ", ".join(
            data.pop("Users of Hashtag")
        )
        data["Mentions in Tweets containing Hashtag"] = ", ".join(data.pop("Mentions"))
        data["Other Hashtags in Tweets containing Hashtag"] = ", ".join(
            data.pop("Hashtags")
        )
