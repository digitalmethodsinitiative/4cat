    ...
    def process(self):
        ...
        with self.dataset.get_results_path().open(
            ...
            for post in self.source_dataset.iterate_items(self):
                ...
                # Check for Mentions
                mentions = [
                    {"username": tag["username"], "id": tag["id"]}
                ...
                if item["from_username"] == "REDACTED":
                    raise ProcessorException(
                        "Author information has been removed; cannot proceed"
                    )
                ...
                for ref_tweet in post.get("referenced_tweets", []):
                    ...
                    if ref_tweet.get("author_user", {}).get("username"):
                        ...
                        item["to_username"] = ref_tweet.get("author_user").get(
                            "username"
                        )
                    else:
                        ...
                        if ref_tweet.get("author_id"):
                            ...
                            error_message = (
                                'Unable to match referenced tweet type %s to specific mention (all mentions listed as mention_type "mention" for this tweet) due to: %s'
                                % (
                                    item["mention_type"],
                                    ref_tweet.get("error").get("detail"),
                                )
                            )
                    ...
                    if ref_tweet.get("error"):
                        item["errors"] = (
                            "Tweet %s " % item["tweet_id"] + error_message
                            if error_message
                            else "Tweet %s had error for mention type %s: "
                            % (item["tweet_id"], item["mention_type"])
                            + "%s" % str(ref_tweet.get("error").get("detail"))
                        )
                    ...
                    if item["to_username"] not in captured_mentions:
                        ...
                        if counter % 2500 == 0:
                            self.dataset.update_status(
                                "Processed through " + str(counter) + " mentions."
                            )
