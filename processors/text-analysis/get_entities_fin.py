...
class ExtractNouns(BasicProcessor):  # TEMPORARILY DISABLED
    ...
    """
    type = "get-entities-fin"  # job type ID
    category = "Text analysis"  # category
    title = "Extract named entities in Finnish text"  # title displayed in UI
    description = "Retrieve named entities in Finnish text detected by SpaCy, ranked on frequency. Be sure to have selected " \
    ...
    def process(self):
        ...
        if "ner" not in self.source_dataset.parameters["enable"]:
            ...
            li_entities = []
            nlp = spacy.load("fi_core_news_sm")  # Load model
