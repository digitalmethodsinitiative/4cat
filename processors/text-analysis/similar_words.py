    ...
    def process(self):
                ...
                for similar_word in similar_words:
                    ...
                    try:
                        ...
                        if word not in excluded_words:
                            ...
                            self.dataset.log(
                                f"'{word}' outside thresholds used for embedding model {model_file.name}; no occurrences"
                            )
                        input_occurrences = 0
                        self.flawless = False
        ...
        if not result:
            ...
            if not self.flawless:
                self.dataset.update_status(
                    "Dataset complete, but some input words were not found in the embedding models (0 occurances in model due to chosen thresholds). Similar words are still identified; check log for specifics.",
                    is_final=True,
                )
