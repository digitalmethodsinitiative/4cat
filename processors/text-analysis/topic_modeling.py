...
class TopicModeler(BasicProcessor):
    ...
    description = (
        "Creates topic models per tokenset using Latent Dirichlet Allocation (LDA). "
        "For a given number of topics, tokens are assigned a relevance weight per topic, "
        "which can be used to find clusters of related words."
    ...
    options = {
        "vectoriser": {
            ...
            "options": {"count": "Frequency", "tf-idf": "Tf-idf"},
            "default": "count",
    ...
    def process(self):
        ...
        vectoriser_class = (
            TfidfVectorizer
            if self.parameters.get("vectoriser") == "tf-idf"
        ...
        for token_file in self.iterate_archive_contents(self.source_file):
            ...
            if token_file.name == ".token_metadata.json":
                ...
                shutil.copyfile(
                    token_file, staging_area.joinpath(".token_metadata.json")
                )
                continue

            index += 1
            self.dataset.update_status(
                "Processing token set %i (%s)" % (index, token_file.stem)
            )
            ...
            vectoriser = vectoriser_class(
                tokenizer=token_helper, lowercase=False, min_df=min_df, max_df=max_df
            )
            ...
            self.dataset.update_status(
                "Fitting token clusters for token set '%s'" % token_file.stem
            )
            if self.interrupted:
                raise ProcessorInterruptedException(
                    "Interrupted while fitting LDA model"
                )

            model = LatentDirichletAllocation(
                n_components=self.parameters.get("topics"), random_state=0
            )
            model.fit(vectors)
            ...
            self.dataset.update_status(
                "Storing model for token set '%s'" % token_file.stem
            )
            with staging_area.joinpath("%s.features" % token_file.stem).open(
                "wb"
            ) as outfile:
                pickle.dump(features, outfile)

            with staging_area.joinpath("%s.model" % token_file.stem).open(
                "wb"
            ) as outfile:
            ...
            self.dataset.update_status(
                "Storing vectors and vectoriser for token set '%s'" % token_file.stem
            )
            with staging_area.joinpath("%s.vectors" % token_file.stem).open(
                "wb"
            ) as outfile:
                pickle.dump(vectors, outfile)

            with staging_area.joinpath("%s.vectoriser" % token_file.stem).open(
                "wb"
            ) as outfile:
            ...
            for topic_index, topic in enumerate(model.components_):
                ...
                top_five_features = {
                    f: model_features[f]
                    for f in sorted(
                        model_features, key=lambda k: model_features[k], reverse=True
                    )[:5]
                }
            ...
            model_metadata[token_file.name]["predictions"] = {
                i: {
                    topic: unnormalized_distribution
                    for topic, unnormalized_distribution in enumerate(doc_predictions)
                }
                for i, doc_predictions in enumerate(predicted_topics)
            }

        # Save the model metadata in our staging area
        with staging_area.joinpath(".model_metadata.json").open(
            "w", encoding="utf-8"
        ) as outfile:
