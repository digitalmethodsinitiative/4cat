...
class Tokenise(BasicProcessor):
    ...
    description = (
        "Splits the post body texts in separate words (tokens). This data can then be used for text analysis. "
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        with (
            ...
            .joinpath("common/assets/stopwords-languages.json")
            .open() as infile
        ):
        ...
        options = {
            ...
            "grouping-per": {
                ...
                "options": {"item": "Item", "sentence": "Sentence in item"},
    ...
    def process(self):
        ...
        with open(
            config.get("PATH_ROOT").joinpath("common/assets/stopwords-iso.json"),
            encoding="utf-8",
        ) as infile:
        ...
        for wordlist in self.parameters.get("filter", []):
            ...
            if wordlist.startswith("stopwords-iso"):
                word_filter = set.union(
                    word_filter, stopwords_iso[wordlist.replace("stopwords-iso-", "")]
                )
            else:
                wordlist_path = config.get("PATH_ROOT").joinpath(
                    f"common/assets/wordlists/{wordlist}.txt"
                )
        ...
        if self.parameters.get("accept_words"):
            accept_words = [
                str(word).strip() for word in self.parameters["accept_words"].split(",")
            ]
        ...
        if self.parameters.get("reject_words"):
            reject_words = [
                ...
                for word in self.parameters["reject_words"].split(",")
            ]
        ...
        grouping = (
            "item" if self.parameters.get("grouping-per", "") == "item" else "sentence"
        )
        ...
        sentence_method, sentence_error = self.get_sentence_method(
            language=language, grouping=grouping, dataset=self.dataset
        )
        ...
        for post in self.source_dataset.iterate_items(self):
            ...
            if docs_per != "thread":
                try:
                    ...
                    self.dataset.update_status(
                        "%s, cannot count items per %s" % (str(e), docs_per),
                        is_final=True,
                    )
                    self.dataset.update_status(0)
                    return
                ...
                document_descriptor = (
                    post["thread_id"] if post["thread_id"] else "undefined"
                )
            ...
            if post_id in metadata:
                ...
                self.dataset.log(
                    f"Note: duplicate post ID {post_id} found in dataset; posts will be processed multiple times"
                )
            else:
                metadata[post_id] = {}
            ...
            for column in columns:
                ...
                if column_value is not None and type(column_value) is not str:
                    self.dataset.update_status(
                        "Column %s contains non text values and cannot be tokenized"
                        % column,
                        is_final=True,
                    )
                    self.dataset.update_status(0)
                ...
                value = [
                    Tokenise.normalise_segment(v)
                    for v in sentence_method(post[column], language)
                    if v is not None
                ]
                if value:
            ...
            for i, document in enumerate(groupings):
                ...
                if post_tokens:
                    ...
                    if current_output_path != output_path:
                        self.dataset.update_status(
                            "Processing items (%s)" % document_descriptor
                        )
                    ...
                    metadata[post_id][document_descriptor]["document_numbers"].append(
        ...
        with staging_area.joinpath(".token_metadata.json").open(
            "w", encoding="utf-8"
        ) as outfile:
        ...
        if sentence_error:
            self.dataset.update_status(
                f"Finished tokenizing; Unable to group by sentence ({language} not supported), instead grouped by item.",
                is_final=True,
            )
