...
class GetCollocations(BasicProcessor):
    ...
    options = {
        "n_size": {
            ...
            "default": 2,
            "options": {"2": "2 (bigrams)", "3": "3 (trigrams)"},
    ...
    def process(self):
        ...
        if self.parameters.get("forbidden_words", None):
            forbidden_words = (
                self.parameters["forbidden_words"].replace(" ", "").lower().split(",")
            )
        else:
        ...
        for token_file in self.iterate_archive_contents(self.source_file):
            ...
            for post_tokens in tokens:
                post_collocations = self.get_collocations(
                    post_tokens,
                    window_size,
                    n_size,
                    ...
                    if query_string:
                        for qs in query_string:
                            if qs in collocation:
                                ...
                                collocation.insert(
                                    0, collocation.pop(collocation.index(qs))
                                )
            ...
            if tokenset_results:
                sorted_results = sorted(
                    tokenset_results.items(), key=operator.itemgetter(1), reverse=True
                )

                # Filter out word pairs that appear less than the min frequency, if given.
                if min_frequency:
                    sorted_results = [
                        tpl for tpl in sorted_results if tpl[1] >= min_frequency
                    ]
    ...
    def get_collocations(
        ...
        collocations = sorted(
            finder.ngram_fd.items(), key=operator.itemgetter(1), reverse=True
        )
