            ...
            for interval, token_data in post_intervals.items():
                ...
                for document_number in token_data["document_numbers"]:
                    ...
                    top_topics = {
                        ...
                        for f in sorted(
                            doc_predictions,
                            key=lambda k: doc_predictions[k],
                            reverse=True,
                        )[:2]
                    }
                    test_top_two = [
                        weight for topic_number, weight in top_topics.items()
                    ]
                    if test_top_two[0] == test_top_two[1]:
                        self.dataset.log(
                            "Document %s-%s equally probable in two or more topics; skipping"
                            % (post_id, str(document_number))
                        )

                    topic_number = str(
                        [topic_number for topic_number, weight in top_topics.items()][0]
                    )
                ...
                topics.append(
                    {
                        ...
                        + 1,  # Adding 1 to conform with other processors
                        "top_five_features": ", ".join(
                            [
                                f + ": " + str(w)
                                for f, w in topic["top_five_features"].items()
                            ]
                        ),
                        "number_of_documents": topics_count[
                            token_filename.rstrip(".json") + str(topic["topic_index"])
                        ],
                    }
                )
