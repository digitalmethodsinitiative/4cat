...
class TfIdf(BasicProcessor):
    ...
    def process(self):
        ...
        try:
            if library == "gensim":
                results = self.get_tfidf_gensim(
                    tokens, dates, top_n=max_output, smartirs=smartirs
                )
            elif library == "scikit-learn":
                results = self.get_tfidf_sklearn(
                    tokens,
                    dates,
                    ngram_range=n_size,
            ...
            self.dataset.update_status(
                "Out of memory - dataset too large to run tf-idf analysis."
            )
            self.dataset.finish(0)
    ...
    def get_tfidf_sklearn(
        ...
        try:
            ...
            self.dataset.update_status(
                "No tokens remain with these parameters. Set less strict constraints and try again.",
                is_final=True,
            )
            self.dataset.finish(0)
            return
        ...
        df_weights = pd.DataFrame(
            {"term": tfidf_vectorizer.get_feature_names_out(), "weight": weights}
        )
