...
class VectoriseByCategory(BasicProcessor):
    ...
    description = (
        "Counts all tokens and categorizes them so they are transformed into category => token => frequency counts. "
        "This is also known as a bag of words."
    ...
    def process(self):
        ...
        try:
            metadata_file = self.extract_archived_file_by_name(
                ".token_metadata.json", self.source_file
            )
        except FileNotFoundError:
            self.dataset.finish_with_error(
                "Metadata file not found; cannot match categories to tokens"
            )
            return
        ...
        for interval, token_data in metadata[first_key].items():
            if any(
                [
                    ...
                    for required_keys in ["filename", "document_numbers"]
                ]
            ):
                self.dataset.finish_with_error(
        ...
        if not category_dataset:
            self.dataset.finish_with_error(
                "No top dataset found; unable to identify categories"
            )
            return
        ...
        for row in category_dataset.iterate_items():
            ...
            for interval, token_data in metadata[item_id].items():
                ...
                for document_number in document_numbers:
                    file_to_category_mapping[(filename, document_number)] = (
                        category_values
                    )
        ...
        word_filter_list = [
            word.strip().lower()
            for word in self.parameters.get("word_filter_list", "").split(",")
        ]
            ...
            with token_file.open("rb") as binary_tokens:
                ...
                for i, document in enumerate(documents):
                    if (token_file.name, i) not in file_to_category_mapping:
                        ...
                        self.dataset.log(
                            "No category found for document %s-%s"
                            % (token_file.name, i)
                        )
                        continue
        ...
        with open(self.dataset.get_results_path(), "w", encoding="utf-8") as output:
            writer = csv.DictWriter(
                output, fieldnames=("date", "category", "item", "value")
            )
            writer.writeheader()
            for interval, category_data in vector_sets.items():
                for category, tokens in category_data.items():
                    ...
                    for token, frequency in token_list:
                        ...
                        if (
                            use_word_filter == "include"
                            and token not in word_filter_list
                        ):
                        ...
                        if use_word_filter == "exclude" and token in word_filter_list:
                            ...
                            continue
                        if threshold_type == "true" and (
                            frequency < threshold_value
                            or (0 < top_n <= num_per_category)
                        ):
    ...
    def exclude_followup_processors(cls, processor_type):
        ...
        if processor_type in [
            ...
            "sentence-split",
            "tokenise-posts",
            "image-downloader-stable-diffusion",
            "word-trees",
            "histogram",
            "extract-urls-filter",
        ]:
