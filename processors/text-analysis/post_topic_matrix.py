...
class TopicModelWordExtractor(BasicProcessor):
    ...
    def process(self):
        ...
        with zipfile.ZipFile(self.source_file, "r") as archive_file:
            ...
            if any(
                [
                    filename not in zip_filenames
                    for filename in [".token_metadata.json", ".model_metadata.json"]
                ]
            ):
        ...
        for interval, token_data in token_metadata[first_key].items():
            if any(
                [
                    required_keys not in token_data
                    for required_keys in ["interval", "document_numbers", "filename"]
                ]
            ):
        ...
        post_column_names = list(
            set(
                ["id", "thread_id", "timestamp", "author", "body"]
                + self.parameters.get("columns", [])
                + token_metadata_parameters.get("columns")
            )
        )
        ...
        if (
            token_metadata_parameters.get("grouped_by") != "item"
            or len(token_metadata_parameters.get("columns")) > 1
        ):
            ...
            if self.parameters.get("include_top_features"):
                model_column_names += [
                    ...
                    + "-".join(
                        [
                            f
                            for f in model_metadata[interval + ".json"]["model_topics"][
                                str(i)
                            ]["top_five_features"]
                        ]
                    )
        ...
        with self.dataset.get_results_path().open(
            ...
            writer = csv.DictWriter(
                results, fieldnames=post_column_names + model_column_names
            )
            writer.writeheader()
            ...
            for post in self.dataset.top_parent().iterate_items(self):
                ...
                for interval, token_data in post_intervals.items():
                    ...
                    if self.parameters.get("include_top_features"):
                        related_topic_columns = [
                            ...
                            + "-".join(
                                [
                                    f
                                    for f in model_metadata[interval + ".json"][
                                        "model_topics"
                                    ][str(i)]["top_five_features"]
                                ]
                            )
                            for i in range(model_metadata_parameters.get("topics"))
                        ]
                    ...
                    for document_number in token_data["document_numbers"]:
                        combined_data["id"] = (
                            str(post.get("id")) + "-" + str(document_number)
                        )
                        ...
                        if multiple_docs_per_post:
                            combined_data["original_document_split"] = token_data[
                                "multiple_docs"
                            ]

                        doc_predictions = model_data["predictions"][
                            str(document_number)
                        ]
                        combined_data["top_topic(s)"] = ", ".join(
                            [
                                str(int(key) + 1)
