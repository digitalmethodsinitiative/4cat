...
class URLCoLinker(BasicProcessor):
    ...
    description = (
        "Create a GEXF network file comprised of URLs appearing together (in a post or thread). "
        "Edges are weighted by amount of co-links."
    ...
    options = {
        "detail": {
            ...
            "options": {"url": "Full URL", "domain": "Domain name"},
            "default": "url",
        },
        "level": {
            "type": UserInput.OPTION_CHOICE,
        ...
        * or " in the query
        ...
        for post in self.source_dataset.iterate_items(self):
            if self.interrupted:
                raise ProcessorInterruptedException(
                    "Interrupted while collecting links"
                )
            ...
            if processed % update_on_item == 0:
                ...
                self.dataset.update_progress(
                    processed / (self.source_dataset.num_rows * 2)
                )
            ...
            if self.parameters.get("detail") == "domain":
                try:
                    post_links = [
                        www_regex.sub("", link.split("/")[2]) for link in post_links
                    ]
                except IndexError:
        ...
        for i, post_links in enumerate(links.values()):
            if i % update_on_item == 0:
                ...
                self.dataset.update_progress(
                    (self.source_dataset.num_rows + i)
                    / (self.source_dataset.num_rows * 2)
                )
            ...
            while post_links:
                ...
                for to_link in post_links:
                    if self.interrupted:
                        raise ProcessorInterruptedException(
                            "Interrupted while creating network edges"
                        )

                    network.add_edge(from_link, to_link)

        self.dataset.update_status(
            f"Network has {len(network.nodes)} and {len(network.edges)} edges"
        )
        self.dataset.log(f"time elapsed: {time.time() - start_time:.2f} seconds")
        self.dataset.update_status("Writing network file")
        if psutil.virtual_memory().percent > 95:
            self.dataset.update_status(
                "WARNING: memory usage about 95%; network may fail..."
            )
            self.log.warning(
                f"Memory usage above 95%: network dataset {self.dataset.key}"
            )
