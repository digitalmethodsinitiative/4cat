        ...
        * or " in the query
        ...
        def stringify_children(node):
            ...
            parts = (
                [node.text]
                + list(
                    chain(*([c.text, tostring(c), c.tail] for c in node.getchildren()))
                )
                + [node.tail]
            )
        ...
        for category in all_categories:
            network.add_node(
                ...
                **({"weight": all_categories[category], "type": "category"}),
            )
        ...
        for page in page_categories:
            for category in page_categories[page]:
                network.add_edge(
                    ids[page.replace("_", " ")],
                    ids[category.replace("_", " ")],
                    **({"weight": all_categories[category]}),
                )
