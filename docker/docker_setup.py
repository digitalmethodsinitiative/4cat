...
def update_config_from_environment(CONFIG_FILE, config_parser):
    ...
    config_parser["API"]["api_host"] = os.environ[
        "API_HOST"
    ]  # set in .env; should be backend container_name in docker-compose.py unless frontend and backend are running together in one container
...
if __name__ == "__main__":
    ...
    if not os.path.exists(CONFIG_FILE):
        ...
        config_parser["API"]["api_port"] = (
            "4444"  # backend internal port set in docker-compose.py; NOT API_PUBLIC_PORT as that is what port Docker exposes to host network
        )

        # File paths
        config_parser.add_section("PATHS")
        config_parser["PATHS"]["path_images"] = (
            "data"  # shared volume defined in docker-compose.yml
        )
        config_parser["PATHS"]["path_data"] = (
            "data"  # shared volume defined in docker-compose.yml
        )
        config_parser["PATHS"]["path_lockfile"] = (
            "backend"  # docker-entrypoint.sh looks for pid file here (in event Docker shutdown was not clean)
        )
        config_parser["PATHS"]["path_sessions"] = (
            "config/sessions"  # shared volume defined in docker-compose.yml
        )
        config_parser["PATHS"]["path_logs"] = (
            "logs/"  # shared volume defined in docker-compose.yml
        )

        # Generated variables
        config_parser.add_section("GENERATE")
        config_parser["GENERATE"]["anonymisation_salt"] = bcrypt.gensalt().decode(
            "utf-8"
        )
    ...
    # Config file already exists; Update .env variables if they changed
        ...
        public_port = int(config_parser["SERVER"]["public_port"])
        frontend_port = (
            int(config.get("flask.server_name").split(":")[-1])
            if ":" in config.get("flask.server_name")
            else 80
        )
        ...
        if frontend_port != public_port:
            ...
            print(
                f"You can do so by running the following command if you do not have access to the 4CAT frontend Control Panel:\n"
