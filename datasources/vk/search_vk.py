...
class SearchVK(Search):
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        intro_text = (
            "This data source uses VK's [API](https://vk.com/dev/first_guide) and a python "
    ...
    def get_items(self, query):
        ...
        """
        if (
            self.parameters.get("username") is None
            or self.parameters.get("password") is None
        ):
        ...
        try:
            ...
            self.dataset.update_status(
                f"VK unable to authorize user: {e}", is_final=True
            )
            return []
        ...
        if query_type == "newsfeed":
            ...
            for i, result_batch in enumerate(
                self.search_newsfeed(vk_helper, **query_parameters)
            ):
                if self.interrupted:
                    raise ProcessorInterruptedException(
                        "Interrupted while fetching newsfeed data from the VK API"
                    )
                ...
                for result in result_batch:
                    ...
                    if include_comments:
                        for comment in self.collect_all_comments(
                            vk_helper,
                    ...
                    self.dataset.update_status(
                        f"Received {num_results} results of max {self.parameters.get('amount')} from the VK API"
                    )
    ...
    def login(self, username, password):
        ...
        vk_session = vk_api.VkApi(
            username,
            password,
            config_filename=Path(config.get("PATH_ROOT")).joinpath(
                config.get("PATH_SESSIONS"), username + "-vk_config.json"
            ),
        )
        vk_session.auth()
    ...
    def search_newsfeed(
        ...
        **kwargs,
        ...
        [
            result.update(
                {"author_profile": author_profiles.get(result.get("from_id"), {})}
            )
        ...
        if response.get("next_from") and num_collected < max_amount:
            ...
            for additional_results in self.search_newsfeed(
                vk_helper, query, max_amount, num_collected=num_collected, **parameters
            ):
        ...
        for comment in comments:
            ...
            if reply_count > 10 and len(replies) == 10:
                ...
                replies += self.get_comments(
                    ...
                    last_collected_id=replies[-1].get("id"),
                )[1:]
            ...
            for reply in replies:
                ...
                if reply.get("thread"):
                    self.log.warning(
                        "VK Datasource issue with replies: additional depth needs to be handled; contact 4CAT devs"
                    )
    ...
    def get_comments(
        ...
        **kwargs,
        ...
        if self.interrupted:
            raise ProcessorInterruptedException(
                "Interrupted while fetching comments from the VK API"
            )

        if post_id is None and comment_id is None:
            raise ProcessorException(
                "Must provide either post_id or comment_id to collect comments from VK"
            )
        ...
        try:
            ...
            self.dataset.log(
                f"Unable to collect comments for owner_id {owner_id} and {'post_id' if post_id is not None else 'comment_id'} {post_id if post_id is not None else comment_id}: {e}"
            )
            return []
        ...
        [
            comment.update(
                {"author_profile": author_profiles.get(comment.get("from_id"), {})}
            )
            for comment in comments
        ]
        # Also expand replies
        [
            reply.update(
                {"author_profile": author_profiles.get(reply.get("from_id"), {})}
            )
            for replies in [
                comment.get("thread", {}).get("items", [])
                for comment in comments
                if comment.get("thread")
            ]
            for reply in replies
        ]
        ...
        for profile_type, profiles in dict_of_profile_types.items():
            for profile in profiles:
                if "id" not in profile:
                    raise ProcessorException(
                        "Profile missing id field; VK data format incorrect/changed"
                    )
                elif profile.get("id") in author_types:
                    raise ProcessorException(
                        "Profile id duplicated across profile types; unable to combine profiles"
                    )
    ...
    def map_item(item):
        ...
        for attachment in item.get("attachments", []):
            ...
            if attachment_type == "photo":
                ...
                videos.append(
                    f"https://vk.com/video{attachment.get('owner_id')}_{attachment.get('id')}"
                )
        ...
        profile_source = (
            "user"
            if author_profile.get("4CAT_author_profile_type") == "profile"
            else "community"
            if author_profile.get("4CAT_author_profile_type") == "group"
            else "N/A"
        )
        ...
        return MappedItem(
            {
                ...
                "author_name": author_profile.get(
                    "name",
                    " ".join(
                        [
                            author_profile.get("first_name", ""),
                            author_profile.get("last_name", ""),
                        ]
                    ),
                ),
                "author_sex": "F"
                if author_profile.get("sex") == 1
                else "M"
                if author_profile.get("sex") == 2
                else "Not Specified"
                if author_profile.get("sex") == 0
                else author_profile.get("sex", "N/A"),
                "author_city": author_profile.get("city", {}).get("title", ""),
                "author_country": author_profile.get("country", {}).get("title", ""),
                "author_photo": author_profile.get(
                    "photo_200",
                    author_profile.get("photo_100", author_profile.get("photo_50", "")),
                ),
                "author_is_admin": True
                if author_profile.get("is_admin") == 1
                else False
                if author_profile.get("is_admin") == 0
                else author_profile.get("is_admin", "N/A"),
                "author_is_advertiser": True
                if author_profile.get("is_advertiser") == 1
                else False
                if author_profile.get("is_advertiser") == 0
                else author_profile.get("is_advertiser", "N/A"),
                "author_deactivated": author_profile.get("is_deactivated", False),
                "author_privacy_is_closed": "closed"
                if author_profile.get("is_closed") == 1
                else "open"
                if author_profile.get("is_closed") == 0
                else "private"
                if author_profile.get("is_closed") == 2
                else author_profile.get("is_closed", "N/A"),
                "author_followers": author_profile.get(
                    "followers_count", author_profile.get("members_count", "N/A")
                ),
                ...
                "edited": datetime.datetime.fromtimestamp(item.get("edited")).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
