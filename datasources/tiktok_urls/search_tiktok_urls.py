...
class SearchTikTokByID(Search):
    ...
    description = (
        "Retrieve metadata for TikTok post URLs."  # description displayed in UI
    )
    ...
    def get_items(self, query):
        ...
        return loop.run_until_complete(
            tiktok_scraper.request_metadata(query["urls"].split(","))
        )
    ...
    def validate_query(query, request, user):
        ...
        if not query.get("urls"):
            ...
            raise QueryParametersException(
                'Missing "Post URLs" ("urls" parameter). Please provide a list of TikTok video URLs.'
            )
        ...
        for item in str(items).split(","):
            ...
            if (
                not re.match(r"https?://www\.tiktokv\.com/share/video/[0-9]+/", item)
                and not re.match(
                    r"https?://www\.tiktok\.com/@[^/]+/video/[0-9]+.*", item
                )
                and not re.match(r"https?://tiktok\.com/@[^/]+/video/[0-9]+.*", item)
            ):
                raise QueryParametersException(
                    "'%s' is not a valid TikTok video URL" % item
                )
        ...
        if not sanitized_items:
            raise QueryParametersException(
                "You must provide at least one valid TikTok video URL."
            )

        # simple!
        return {"urls": ",".join(sanitized_items)}
...
class TikTokScraper:
    ...
    def update_proxies(self):
        ...
        self.proxy_sleep = self.processor.config.get(
            "tiktok-urls-search.proxies.wait", self.proxy_sleep
        )
        if not all_proxies:
        ...
        for proxy in all_proxies:
            if proxy in self.proxy_map:
                ...
                self.proxy_map[proxy] = {"busy": False, "url": None, "next_request": 0}
        ...
        if not available_proxies:
            ...
            if self.last_time_proxy_available is None:
                ...
                self.processor.dataset.log(
                    "No available proxy found at start of request_metadata"
                )
                self.last_time_proxy_available = time.time()

            if (
                self.last_time_proxy_available + self.no_available_proxy_timeout
                < time.time()
            ):
                # No available proxies in timeout period
                raise ProcessorException(
                    f"Error: No proxy found available after {self.no_available_proxy_timeout}"
                )
        else:
            self.last_time_proxy_available = time.time()
    ...
    def release_proxy(self, url):
        ...
        # Release proxy
        used_proxy = [
            proxy for proxy in self.proxy_map if self.proxy_map[proxy]["url"] == url
        ]
        if used_proxy:
            used_proxy = used_proxy[0]
            self.proxy_map[used_proxy].update(
                {"busy": False, "next_request": time.time() + self.proxy_sleep}
            )
        else:
            # TODO: why are we releasing a proxy without a URL?
            self.processor.dataset.log(
                f"Unable to find and release proxy associated with {url}"
            )
            pass
    ...
    async def request_metadata(self, urls):
        ...
        while urls or tiktok_requests:
            ...
            for available_proxy in available_proxies:
                ...
                while urls and url is None:
                    ...
                    url = url.replace(
                        "https://", "http://"
                    )  # https is finicky, lots of blocks
                    ...
                    proxy = (
                        {"http": available_proxy, "https": available_proxy}
                        if available_proxy != "__localhost__"
                        else None
                    )
                    tiktok_requests[url] = session.get(url, proxies=proxy, timeout=30)
                    seen_urls.add(url)
                    self.proxy_map[available_proxy].update({"busy": True, "url": url})
            ...
            if self.processor.interrupted:
                ...
                while (
                    not all([r for r in tiktok_requests.values() if r.done()])
                    and time.time() < max_timeout
                ):
                    await asyncio.sleep(0.5)

                raise WorkerInterruptedException(
                    "Interrupted while fetching TikTok metadata"
                )
            ...
            for url in list(tiktok_requests.keys()):
                ...
                if exception:
                    ...
                    if isinstance(exception, requests.exceptions.RequestException):
                        self.processor.dataset.update_status(
                            "Video at %s could not be retrieved (%s: %s)"
                ...
                if response.status_code == 404:
                    failed += 1
                    self.processor.dataset.log(
                        "Video at %s no longer exists (404), skipping" % response.url
                    )
                    continue
                ...
                if not sigil:
                    if url not in retries or retries[url] < 3:
                        ...
                        urls.append(url)
                        self.processor.dataset.log(
                            "No embedded metadata found for video %s, retrying" % url
                        )
                    else:
                        failed += 1
                        self.processor.dataset.log(
                            "No embedded metadata found for video %s, skipping" % url
                        )
                    continue
                ...
                try:
                    if sigil.text:
                        ...
                        failed += 1
                        self.processor.dataset.log(
                            "Embedded metadata was found for video %s, but it could not be parsed, skipping"
                            % url
                        )
                        continue
                ...
                for video in self.reformat_metadata(metadata):
                    if video == self.VIDEO_NOT_FOUND:
                        failed += 1
                        self.processor.dataset.log(
                            f"Video for {url} not found, may have been removed, skipping"
                        )
                        continue
        ...
        if failed:
            notes.append(
                "%s URL(s) failed or did not exist anymore" % "{:,}".format(failed)
            )
        if dupes:
        ...
        if notes:
            self.processor.dataset.update_status(
                "Dataset completed, but not all URLs were collected (%s). See "
    ...
    def reformat_metadata(self, metadata):
        ...
        if (
            "__DEFAULT_SCOPE__" in metadata
            and "webapp.video-detail" in metadata["__DEFAULT_SCOPE__"]
        ):
            try:
                video = metadata["__DEFAULT_SCOPE__"]["webapp.video-detail"][
                    "itemInfo"
                ]["itemStruct"]
            ...
            metadata = {"ItemModule": {video["id"]: video}}
        ...
        while video_ids or video_download_urls or video_requests:
            ...
            for available_proxy in available_proxies:
                ...
                if video_download_urls:
                    ...
                    video_requests[video_download_url] = {
                        "request": session.get(
                            video_download_url, proxies=proxy, timeout=30
                        ),  # not using stream=True here, as it seems slower; possibly due to short video lengths
                        "video_id": video_id,
                        "type": "download",
                    }
                    self.proxy_map[available_proxy].update(
                        {"busy": True, "url": video_download_url}
                    )
            ...
            if self.processor.interrupted:
                ...
                while (
                    not all(
                        [
                            r["request"]
                            for r in video_requests.values()
                            if r["request"].done()
                        ]
                    )
                    and time.time() < max_timeout
                ...
                raise WorkerInterruptedException(
                    "Interrupted while downloading TikTok videos"
                )
            ...
            for url in list(video_requests.keys()):
                ...
                try:
                    ...
                    error_message = (
                        f"URL {url} could not be retrieved ({type(e).__name__}: {e})"
                    )
                ...
                if request_type == "metadata":
                    ...
                    try:
                        ...
                        self.processor.dataset.log(
                            f"JSONDecodeError for video {video_id} metadata: {e}\n{json_source}"
                        )
                    ...
                    try:
                        url = list(video_metadata["source"]["data"].values())[0][
                            "videoData"
                        ]["itemInfos"]["video"]["urls"][0]
                    except (KeyError, IndexError):
                        error_message = (
                            f"vid: {video_id} - failed to find video download URL"
                        )
                        request_metadata["error"] = error_message
                        download_results[video_id] = request_metadata
                        self.processor.dataset.log(error_message)
                        self.processor.dataset.log(
                            video_metadata["source"]["data"].values()
                        )
                        continue
                    ...
                    self.processor.dataset.update_status(
                        "Collected metadata for %i/%i videos"
                    ...
                    # Download video
                    with open(
                        staging_area.joinpath(video_id).with_suffix(".mp4"), "wb"
                    ) as f:
                        for chunk in response.iter_content(chunk_size=1024 * 1024):
                            if chunk:
                                f.write(chunk)
                    request_metadata["success"] = True
                    request_metadata["files"] = [
                        {"filename": video_id + ".mp4", "success": True}
                    ]
                    download_results[video_id] = request_metadata

                    downloaded_videos += 1
                    self.processor.dataset.update_status(
                        "Downloaded %i/%i videos" % (downloaded_videos, max_videos)
                    )
                    self.processor.dataset.update_progress(
