...
class SearchInstagram(Search):
    ...
    def get_items(self, query):
        ...
        raise NotImplementedError(
            "Instagram datasets can only be created by importing data from elsewhere"
        )
    ...
    def map_item(item):
        ...
        if (item.get("product_type", "") == "ad") or (
            ...
            # These are ads
            raise MapItemException(
                "appears to be Instagram ad, check raw data to confirm and ensure Zeeschuimer is up to date."
            )

        is_graph_response = "__typename" in item and item["__typename"] not in (
            "XDTMediaDict",
        )
    ...
    def parse_graph_item(node):
        ...
        num_media = (
            ...
            else len(node["edge_sidecar_to_children"]["edges"])
        )
        ...
        if media_node["__typename"] == "GraphVideo":
            ...
            resources = media_node.get(
                "display_resources", media_node.get("thumbnail_resources")
            )
            try:
        ...
        if node["__typename"] != "GraphSidecar":
            ...
            media_types = set(
                [
                    s["node"]["__typename"]
                    for s in node["edge_sidecar_to_children"]["edges"]
                ]
            )
            media_type = (
                "mixed"
                if len(media_types) > 1
                else type_map.get(media_types.pop(), "unknown")
            )
        ...
        if node.get("location"):
            ...
            location["latlong"] = (
                str(node["location"]["lat"]) + "," + str(node["location"]["lng"])
                if node["location"].get("lat")
                else ""
            )
        ...
        mapped_item = {
            # Post data
            "id": node["shortcode"],
            "post_source_domain": node.get("__import_meta", {}).get(
                "source_platform_url"
            ...
            # Author data
            "author": user.get(
                "username", owner.get("username", MissingMappedField(""))
            ),
            "author_fullname": user.get(
                "full_name", owner.get("full_name", MissingMappedField(""))
            ),
            "is_verified": True if user.get("is_verified") else False,
            "author_avatar_url": user.get(
                "profile_pic_url", owner.get("profile_pic_url", MissingMappedField(""))
            ),
    ...
    def parse_itemlist_item(node):
        ...
        missing_media = None
        type_map = {
            SearchInstagram.MEDIA_TYPE_PHOTO: "photo",
            SearchInstagram.MEDIA_TYPE_VIDEO: "video",
        }
        media_types = set()
        # for carousels, get the first media item, for videos, get the video
        # url, for photos, get the highest resolution
        ...
        for media_node in media_nodes:
            if media_node["media_type"] == SearchInstagram.MEDIA_TYPE_VIDEO:
                ...
                if "image_versions2" in media_node:
                    display_urls.append(
                        media_node["image_versions2"]["candidates"][0]["url"]
                    )
                else:
                ...
                "media_type"
            ] == SearchInstagram.MEDIA_TYPE_PHOTO and media_node.get("image_versions2"):
                # Images
        ...
        if "usertags" in node:
            usertags = (
                ",".join([user["user"]["username"] for user in node["usertags"]["in"]])
                if node["usertags"]
                else ""
            )
        else:
        ...
        mapped_item = {
            ...
            "timestamp": datetime.datetime.fromtimestamp(node["taken_at"]).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
            "thread_id": node["code"],
            "parent_id": node["code"],
