...
class SearchPinterest(Search):
    ...
    def get_items(self, query):
        ...
        raise NotImplementedError(
            "Pinterest datasets can only be created by importing data from elsewhere"
        )
        ...
        return MappedItem(
            {
                ...
                "author_fullname": post["pinner"].get(
                    "fullName", post["pinner"].get("full_name", "")
                ),
                "author_original": post["nativeCreator"]["username"]
                if post.get("nativeCreator")
                else post["pinner"]["username"],
                "body": post["description"].strip(),
                "subject": post["title"].strip(),
                "ai_description": post.get("auto_alt_text", ""),
                "pinner_original": post["originPinner"]["fullName"]
                if post.get("originPinner")
                else "",
                "pinner_via": post["viaPinner"]["fullName"]
                if post.get("viaPinner")
                else "",
                "board": post["board"]["name"],
                "board_pins": post["board"].get(
                    "pinCount", post["board"].get("pin_count")
                ),
                "board_url": f"https://www.pinterest.com{post['board']['url']}",
                "timestamp": str_timestamp,
                "idea_tags": ",".join(post["pinJoin"]["visualAnnotation"])
                if post.get("pinJoin")
                else "",
                ...
                "image_url": image_url,
                "dominant_colour": post.get(
                    "dominantColor", post.get("dominant_color")
                ),
