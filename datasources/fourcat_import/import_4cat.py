...
class SearchImportFromFourcat(BasicProcessor):
    ...
    def process(self):
        ...
        self.created_datasets = (
            set()
        )  # keys of created datasets - may not be successful!
        self.remapped_keys = {}  # changed dataset keys
        self.dataset_owner = self.dataset.get_owners()[
            0
        ]  # at this point it has 1 owner
        try:
    ...
    def after_create(query, dataset, request):
        ...
        if query.get("method") == "zip":
            ...
            file.seek(0)
            with (
                dataset.get_results_path()
                .with_suffix(".importing")
                .open("wb") as outfile
            ):
                while True:
    ...
    def process_zip(self):
        ...
        with zipfile.ZipFile(temp_file, "r") as zip_ref:
            ...
            metadata_files = [
                file for file in zip_contents if file.endswith("_metadata.json")
            ]
            if not metadata_files:
                self.dataset.finish_with_error(
                    "No metadata files found in ZIP file; is this a 4CAT export?"
                )
                return
            ...
            for file in metadata_files:
                with zip_ref.open(file) as f:
                    content = f.read().decode(
                        "utf-8"
                    )  # Decode the binary content using the desired encoding
            ...
            if len(primary_dataset_keys) != 1:
                self.dataset.finish_with_error(
                    "ZIP file contains multiple primary datasets; only one is allowed."
                )
                return
            ...
            while datasets:
                ...
                new_dataset = self.create_dataset(
                    processed_metadata, dataset_key, dataset_key in primary_dataset_keys
                )
                processed_files += 1
                ...
                if log_filename in zip_contents:
                    self.dataset.update_status(
                        f"Transferring log file for dataset {new_dataset.key}"
                    )
                    ...
                    self.dataset.log(
                        f"Log file not found for dataset {new_dataset.key} (original key {dataset_key})."
                    )
                ...
                if results_filename in zip_contents:
                    self.dataset.update_status(
                        f"Transferring data file for dataset {new_dataset.key}"
                    )
                    ...
                    self.dataset.log(
                        f"Results file not found for dataset {new_dataset.key} (original key {dataset_key})."
                    )
                    new_dataset.finish_with_error(
                        f"Results file not found for dataset {new_dataset.key} (original key {dataset_key})."
                    )
                ...
                if dataset_key in parent_child_mapping:
                    ...
                    self.dataset.log(
                        f"Adding ({len(parent_child_mapping[dataset_key])}) child datasets to import queue"
                    )
            ...
            if failed_imports:
                self.dataset.update_status(
                    f"Dataset import finished, but not all data was imported properly. "
                    f"{len(failed_imports)} dataset(s) were not successfully imported. Check the "
                    f"dataset log file for details.",
                    is_final=True,
                )
            elif missed_files:
                self.dataset.log(
                    f"ZIP file contained {len(missed_files)} files that were not processed: {missed_files}"
                )
                self.dataset.update_status(
                    f"Dataset import finished, but not all files were processed. "
                ...
                self.dataset.update_status(
                    f"{len(imported)} dataset(s) succesfully imported.", is_final=True
                )
    ...
    def create_dataset(self, metadata, original_key, primary=False):
        ...
        if primary:
            ...
            try:
                ...
                # generated.
                new_dataset = DataSet(
                    parameters={}, type=self.type, db=self.db, modules=self.modules
                )
                metadata.pop("key")
                self.db.update(
                    "datasets", where={"key": new_dataset.key}, data=metadata
                )
                ...
                new_dataset = DataSet(
                    key=metadata["key"], db=self.db, modules=self.modules
                )
        ...
        if new_dataset.key != original_key:
            ...
            self.dataset.update_status(
                f"Cannot import with same key - already in use on this server. Using key "
        ...
        updated = new_dataset.reserve_result_file(
            parameters=new_dataset.parameters, extension=extension
        )
        if not updated:
    ...
    def process_urls(self):
        ...
        while keys:
            ...
            self.halt_and_catch_fire()
            self.dataset.log(
                f"Importing dataset {dataset_key} from 4CAT server {self.base}."
            )

            # first, metadata!
            try:
                metadata = SearchImportFromFourcat.fetch_from_4cat(
                    self.base, dataset_key, api_key, "metadata"
                )
                metadata = metadata.json()
            except FourcatImportException as e:
                self.dataset.log(
                    f"Error retrieving record for dataset {dataset_key}: {e}"
                )
                continue
            except ValueError:
            ...
            new_dataset = self.create_dataset(
                metadata, dataset_key, primary=True if not imported else False
            )
            ...
            try:
                ...
                log = SearchImportFromFourcat.fetch_from_4cat(
                    self.base, dataset_key, api_key, "log"
                )
                ...
                new_dataset.finish_with_error(
                    f"Error retrieving log for dataset {new_dataset.key}: {e}"
                )
                failed_imports.append(dataset_key)
                continue
            except ValueError:
                new_dataset.finish_with_error(
                    f"Could not read log for dataset {new_dataset.key}: skipping dataset"
                )
                failed_imports.append(dataset_key)
                continue
            ...
            try:
                ...
                SearchImportFromFourcat.fetch_from_4cat(
                    self.base, dataset_key, api_key, "data", datapath
                )
                ...
                self.dataset.log(
                    f"Dataset {new_dataset.key} unable to import: {e}, skipping import"
                )
                if new_dataset.key != self.dataset.key:
                    new_dataset.delete()
                continue
                ...
                new_dataset.finish_with_error(
                    f"Could not read results for dataset {new_dataset.key}"
                )
                failed_imports.append(dataset_key)
                continue
            ...
            try:
                self.dataset.update_status(
                    f"Looking for child datasets to transfer for dataset {new_dataset.key}"
                )
                children = SearchImportFromFourcat.fetch_from_4cat(
                    self.base, dataset_key, api_key, "children"
                )
                children = children.json()
            except FourcatImportException as e:
                self.dataset.update_status(
                    f"Error retrieving children for dataset {new_dataset.key}: {e}"
                )
                failed_imports.append(dataset_key)
                continue
            except ValueError:
                self.dataset.update_status(
                    f"Could not collect children for dataset {new_dataset.key}"
                )
                failed_imports.append(dataset_key)
                continue
        ...
        if failed_imports:
            ...
            self.dataset.update_status(
                f"{len(imported)} dataset(s) succesfully imported from {self.base}.",
                is_final=True,
            )
    ...
    def halt_and_catch_fire(self):
        ...
        if self.interrupted:
            ...
            self.dataset.finish_with_error(
                f"Interrupted while importing datasets{' from ' + self.base if self.base else ''}. Cannot resume - you "
    ...
    def fetch_from_4cat(base, dataset_key, api_key, component, datapath=None):
        ...
        try:
            if component == "data" and datapath:
                # Stream data
                with requests.get(
                    f"{base}/api/export-packed-dataset/{dataset_key}/{component}/",
                    timeout=5,
                    stream=True,
                    headers={"User-Agent": "4cat/import", "Authentication": api_key},
                ) as r:
                    r.raise_for_status()
            ...
            raise FourcatImportException(
                f"The 4CAT server at {base} took too long to respond. Make sure it is "
                f"accessible to external connections and try again."
            )
        except requests.RequestException as e:
            raise FourcatImportException(
                f"Could not connect to the 4CAT server at {base} ({e}). Make sure it is "
                f"accessible to external connections and try again."
            )
    ...
    def validate_query(query, request, user):
        ...
        if query.get("method") == "zip":
            ...
            return {"method": "zip", "filename": filename}
            ...
            if len(keys) != 1:
                ...
                raise QueryParametersException(
                    "You need to provide a single URL to a 4CAT dataset to import."
                )

            if len(bases) != 1:
                raise QueryParametersException(
                    "All URLs need to point to the same 4CAT server. You can only import from "
            ...
            try:
                ...
                test = SearchImportFromFourcat.fetch_from_4cat(
                    base, keys[0], query.get("api-key"), "metadata"
                )
            except FourcatImportException as e:
            ...
            try:
                ...
                raise QueryParametersException(
                    f"Unexpected response when trying to fetch metadata for dataset {keys[0]}."
                )
            ...
            if metadata.get("current_4CAT_version") != version:
                raise QueryParametersException(
                    f"This 4CAT server is running a different version of 4CAT ({version}) than "
            ...
            return {"url": ",".join(urls), "api-key": query.get("api-key")}
        else:
    ...
    def get_keys_from_urls(urls):
        ...
        """
        return [
            url.split("/results/")[-1].split("/")[0].split("#")[0].split("?")[0]
            for url in urls
        ]
