"""
Import Instagram data from URLs
"""
import instaloader
import requests
import asyncio
...
class SearchInstagramByID(Search):
    """
    Collect Instagram post data
    """
    type = "instagram-urls-search"  # job ID
    category = "Search"  # category
    title = "Search Instagram by post URL"  # title displayed in UI
    description = "Retrieve metadata for Instagram post URLs."  # description displayed in UI
    ...
    config = {
        "instagram-urls-search.proxies": {
            "type": UserInput.OPTION_TEXT_JSON,
            "default": [],
            "help": "Proxies for Instagram data collection"
        },
        "instagram-urls-search.proxies.wait": {
    ...
    options = {
        "intro": {
            ...
            "help": "This data source can retrieve metadata for Instagrm posts based on a list of URLs for those "
                    "posts.\n\nEnter a list of Instagram post URLs. Metadata for each post will be extracted from "
                    "each post's page in the browser interface "
                    "([example](https://www.instagram.com/p/C1CKQroyybi/)). This includes a lot of "
    ...
    def get_items(self, query):
        """
        Retrieve metadata for Instagram URLs

        :param dict query:  Search query parameters
        """
        tiktok_scraper = InstagramScraper(processor=self, config=self.config)
    ...
    @staticmethod
    def map_item(item, strict=False):
        """
