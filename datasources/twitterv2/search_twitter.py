...
class SearchWithTwitterAPIv2(Search):
    ...
    def get_items(self, query):
        ...
        bearer_token = (
            self.parameters.get("api_bearer_token")
            if not have_api_key
            else have_api_key
        )
        ...
        if self.parameters.get(
            "query_type", "query"
        ) == "id_lookup" and self.config.get("twitterv2-search.id_lookup"):
            ...
            params["max_results"] = (
                max(10, min(amount, 100)) if amount > 0 else 100
            )  # 100 = upper limit, 10 = lower

            if self.parameters.get("min_date"):
                params["start_time"] = datetime.datetime.fromtimestamp(
                    self.parameters["min_date"]
        ...
        for query in queries:
            if self.parameters.get("query_type", "query") == "id_lookup" and config.get(
                "twitterv2-search.id_lookup"
            ):
                params["ids"] = query
            else:
            ...
            while True:
                if self.interrupted:
                    raise ProcessorInterruptedException(
                        "Interrupted while getting posts from the Twitter API"
                    )
                ...
                while retries > 0:
                    try:
                        api_response = requests.get(
                            endpoint, headers=auth, params=params, timeout=30
                        )
                        break
                ...
                if api_response.status_code == 429:
                    try:
                        ...
                        if structured_response.get("title") == "UsageCapExceeded":
                            self.dataset.update_status(
                                "Hit the monthly post cap. You cannot capture more posts "
                        ...
                        self.dataset.update_status(
                            "Hit X's rate limit, but could not figure out why. Halting "
                            "post collection.",
                            is_final=True,
                        )
                        return
                    ...
                    resume_at = (
                        convert_to_int(api_response.headers["x-rate-limit-reset"]) + 1
                    )
                    resume_at_str = datetime.datetime.fromtimestamp(
                        int(resume_at)
                    ).strftime("%c")
                    self.dataset.update_status(
                        "Hit X's rate limit - waiting until %s to continue."
                        % resume_at_str
                    )
                    while time.time() <= resume_at:
                        if self.interrupted:
                            raise ProcessorInterruptedException(
                                "Interrupted while waiting for rate limit to reset"
                            )
                        time.sleep(0.5)
                    continue
                ...
                # activated properly get a 403
                    ...
                    try:
                        structured_response = api_response.json()
                        self.dataset.update_status(
                            "'Forbidden' error from the X API. Could not connect to X API "
                            "with this API key. %s"
                        ...
                        self.dataset.update_status(
                            "'Forbidden' error from the X API. Your key may not have access to "
                ...
                # reasons - in that case just wait a while and try again
                    ...
                    self.dataset.update_status(
                        "X unavailable (status %i) - waiting until %s to continue."
                ...
                # invalid API key
                    ...
                    self.dataset.update_status(
                        "Invalid API key - could not connect to X API", is_final=True
                    )
                    return
                ...
                # haven't seen one yet, but they probably exist
                    ...
                    self.log.warning(
                        "X API v2 responded with status code %i. Response body: %s"
                    ...
                    self.dataset.update_status(
                        "Could not connect to X. Cancelling.", is_final=True
                    )
                    return
                ...
                for missing_object in api_response.get("errors", {}):
                    ...
                    if parameter_type in missing_objects:
                        missing_objects[parameter_type][
                            missing_object.get("resource_id")
                        ] = missing_object
                    else:
                        missing_objects[parameter_type] = {
                            missing_object.get("resource_id"): missing_object
                        }
                ...
                if num_missing_objects > 0:
                    # Log amount
                    self.dataset.log(
                        "Missing objects collected: "
                        + ", ".join(
                            ["%s: %s" % (k, len(v)) for k, v in missing_objects.items()]
                        )
                    )
                if num_missing_objects > 50:
                    ...
                    error_report.append(
                        "%i missing objects received following post number %i. Possible issue with X API."
                        % (num_missing_objects, tweets)
                    )
                    error_report.append(
                        "Missing objects collected: "
                        + ", ".join(
                ...
                if any(
                    key not in expected_error_types for key in missing_objects.keys()
                ):
                    self.log.warning(
                        "X API v2 returned unknown error types: %s"
                        % str(
                            [
                                key
                                for key in missing_objects.keys()
                                if key not in expected_error_types
                            ]
                        )
                    )
                ...
                for tweet in api_response.get("data", []):
                    ...
                    if tweets % 500 == 0:
                        self.dataset.update_status(
                            "Received %s of ~%s tweets from the X API"
                            % ("{:,}".format(tweets), expected_tweets)
                        )
                ...
                if self.parameters.get(
                    ...
                    for tweet_error in api_response.get("errors", []):
                        ...
                        if (
                            tweet_error.get("resource_type") == "tweet"
                            and tweet_id in tweet_ids
                            and tweet_id not in collected_errors
                        ):
                ...
                # paginate
                if (
                    (amount <= 0 or tweets < amount)
                    and api_response.get("meta")
                    and "next_token" in api_response["meta"]
                ):
        ...
        if not self.import_issues:
            ...
            self.dataset.update_status(
                "Completed with errors; Check log for Error Report.", is_final=True
            )

    def enrich_tweet(
        self, tweet, users, media, polls, places, referenced_tweets, missing_objects
    ):
        """
        ...
        **This is not an efficient way to store things** but it is more
        ...
        if (
            "place_id" in tweet.get("geo", {})
            and tweet.get("geo").get("place_id") in places_by_id
        ):
            tweet["geo"]["place"] = places_by_id.get(tweet.get("geo").get("place_id"))
        elif "place_id" in tweet.get("geo", {}) and tweet.get("geo").get(
            "place_id"
        ) in missing_objects.get("place", {}):
            tweet["geo"]["place"] = missing_objects.get("place", {}).get(
                tweet.get("geo").get("place_id"), {}
            )
        ...
        for index, media_key in enumerate(
            ...
            if media_key in media_by_key:
                ...
                tweet["attachments"]["media_keys"][index] = {
                    "media_key": media_key,
                    "error": missing_objects["media"][media_key],
                }
        ...
        if "in_reply_to_user_id" in tweet:
            if tweet["in_reply_to_user_id"] in users_by_id:
                ...
                tweet["in_reply_to_user"] = {
                    "in_reply_to_user_id": tweet["in_reply_to_user_id"],
                    "error": missing_objects["user"][tweet["in_reply_to_user_id"]],
                }
        ...
        for index, reference in enumerate(tweet.get("referenced_tweets", [])):
            if reference["id"] in tweets_by_id:
                tweet["referenced_tweets"][index] = {
                    **reference,
                    **self.enrich_tweet(
                        tweets_by_id[reference["id"]],
                        users,
                        media,
                        polls,
                        places,
                ...
                tweet["referenced_tweets"][index] = {
                    **reference,
                    **{"error": missing_objects["tweet"][reference["id"]]},
                }
    ...
    def fix_tweet_error(self, tweet_error):
        ...
        modified_tweet["created_at"] = datetime.datetime.now().strftime(
            "%Y-%m-%dT%H:%M:%S.000Z"
        )
        modified_tweet["text"] = ""
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        if have_api_key:
            intro_text = (
                "This data source uses the full-archive search endpoint of the X API (v2) to retrieve "
                "historic tweets that match a given query."
            )

        else:
            intro_text = (
                "This data source uses the full-archive search endpoint of the X/Twitter API, v2. To use the "
        ...
        options = {
            "intro-1": {"type": UserInput.OPTION_INFO, "help": intro_text},
        }
        ...
        options.update(
            {
                "query": {"type": UserInput.OPTION_TEXT_LARGE, "help": "Query"},
                "amount": {
                    "type": UserInput.OPTION_TEXT,
                    "help": "Posts to retrieve",
                    "tooltip": "0 = unlimited (be careful!)"
                    if not max_tweets
                    else ("0 = maximum (%s)" % str(max_tweets)),
                    "min": 0,
                    "max": max_tweets if max_tweets else 10_000_000,
                    "default": 10,
                },
                "divider-2": {"type": UserInput.OPTION_DIVIDER},
                "daterange-info": {
                ...
                "daterange": {"type": UserInput.OPTION_DATERANGE, "help": "Date range"},
            }
        )
    ...
    def validate_query(query, request, user):
        ...
        if (
            len(query.get("query")) > 1024
            and query.get("query_type", "query") != "id_lookup"
        ):
            raise QueryParametersException(
                "X API queries cannot be longer than 1024 characters."
            )

        if query.get("query_type", "query") == "id_lookup" and config.get(
            "twitterv2-search.id_lookup", user=user
        ):
        ...
        # to process
        if params["query_type"] == "query" and (
            params.get("api_type") == "all" or have_api_key
        ):
            ...
            # the right format
            if after:
                count_params["start_time"] = datetime.datetime.fromtimestamp(
                    after
                ).strftime("%Y-%m-%dT%H:%M:%SZ")

            if before:
                count_params["end_time"] = datetime.datetime.fromtimestamp(
                    before
                ).strftime("%Y-%m-%dT%H:%M:%SZ")

            bearer_token = (
                params.get("api_bearer_token") if not have_api_key else have_api_key
            )
            ...
            while True:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        expected_tweets += int(
                            response.json()["meta"]["total_tweet_count"]
                        )
                    except KeyError:
                    ...
                    if "next_token" not in response.json().get("meta", {}):
                        ...
                        count_params["next_token"] = response.json()["meta"][
                            "next_token"
                        ]

                elif response.status_code == 401:
                    raise QueryParametersException(
                        "Your bearer token seems to be invalid. Please make sure it is valid "
                        "for the Research track of the X API."
                    )

                elif response.status_code == 400:
                    raise QueryParametersException(
                        "Your query is invalid. Please make sure the date range does not "
            ...
            if expected_tweets:
                ...
                if collectible_tweets > 0:
                    if collectible_tweets < expected_tweets:
                        warning += ", but only %s will be collected. " % "{:,}".format(
                            collectible_tweets
                        )
                ...
                if expected_seconds > 900:
                    warning += (
                        ". Collection will take approximately %s." % expected_time
                    )

            if warning and not query.get("frontend-confirm"):
                warning = "This query matches approximately %s tweets%s" % (
                    "{:,}".format(expected_tweets),
                    warning,
                )
    ...
    def map_item(item):
        ...
        tweet_time = datetime.datetime.strptime(
            item["created_at"], "%Y-%m-%dT%H:%M:%S.000Z"
        )

        # For backward compatibility
        author_username = (
            item["author_user"]["username"]
            if item.get("author_user")
            else item["author_username"]
        )
        author_fullname = (
            item["author_user"]["name"]
            if item.get("author_user")
            else item["author_fullname"]
        )
        author_followers = (
            item["author_user"]["public_metrics"]["followers_count"]
            if item.get("author_user")
            else ""
        )
        ...
        mentions = [
            tag["username"] for tag in item.get("entities", {}).get("mentions", [])
        ]
        urls = [
            tag.get("expanded_url", tag.get("display_url"))
            for tag in item.get("entities", {}).get("urls", [])
            if ("display_url" in tag or "expanded_url" in tag)
        ]
        images = [
            attachment["url"]
            for attachment in item.get("attachments", {}).get("media_keys", [])
            if type(attachment) is dict and attachment.get("type") == "photo"
        ]
        video_items = [
            attachment
            for attachment in item.get("attachments", {}).get("media_keys", [])
            if type(attachment) is dict and attachment.get("type") == "video"
        ]
        ...
        if is_retweet:
            retweeted_tweet = [
                t for t in item["referenced_tweets"] if t.get("type") == "retweeted"
            ][0]
            if retweeted_tweet.get("text", False):
                ...
                if retweeted_tweet.get("author_user") and retweeted_tweet.get(
                    ...
                    retweeting_users = [
                        ...
                        for mention in item.get("entities", {}).get("mentions", [])
                        if mention.get("id") == retweeted_tweet.get("author_id")
                    ]
                    if retweeting_users:
                        # should only ever be one, but this verifies that there IS one and not NONE
                        item["text"] = (
                            "RT @" + retweeting_users[0] + ": " + retweeted_body
                        )

            retweeted_user = (
                retweeted_tweet["author_user"]["username"]
                if retweeted_tweet.get("author_user")
                else retweeted_tweet.get("author_username", "")
            )  # Reference tweets were not always enriched
            ...
            [
                ...
                for tag in retweeted_tweet.get("entities", {}).get("hashtags", [])
            ]
            [
                mentions.append(tag["username"])
                for tag in retweeted_tweet.get("entities", {}).get("mentions", [])
            ]
            [
                urls.append(tag.get("expanded_url", tag["display_url"]))
                for tag in retweeted_tweet.get("entities", {}).get("urls", [])
                if ("display_url" in tag or "expanded_url" in tag)
            ]
            # Images appear to be inheritted by retweets, but just in case
            [
                images.append(attachment["url"])
                for attachment in retweeted_tweet.get("attachments", {}).get(
                    "media_keys", []
                )
                if type(attachment) is dict and attachment.get("type") == "photo"
            ]
        ...
        is_quoted = any(
            [ref.get("type") == "quoted" for ref in item.get("referenced_tweets", [])]
        )
        is_reply = any(
            [
        ...
        public_metrics = {
            k: item["public_metrics"].get(k, MissingMappedField(0))
            for k in expected_metrics
        }
        missing_metrics = [
            m for m in expected_metrics if m not in item["public_metrics"]
        ]
        warning = ""
        if missing_metrics:
        ...
        return MappedItem(
            {
                ...
                "link": "https://x.com/%s/status/%s"
                % (author_username, item.get("id")),
                "subject": "",
                "body": item["text"],
                ...
                **public_metrics,
                ...
                if not is_quoted
                    ...
                    ref for ref in item["referenced_tweets"] if ref["type"] == "quoted"
                ].pop()
                .get("author_user", {})
                .get("username", ""),
