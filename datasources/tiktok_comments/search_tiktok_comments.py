...
class SearchTikTokComments(Search):
    ...
    def get_items(self, query):
        ...
        raise NotImplementedError(
            "TikTok comment datasets can only be created by importing data from elsewhere"
        )

    @staticmethod
    def map_item(item):
        item_datetime = datetime.fromtimestamp(item["create_time"]).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        ...
        return MappedItem(
            {
                ...
                "is_liked_by_post_author": "yes"
                if bool(item.get("author_pin"))
                else "no",
                "is_sticky": "yes" if bool(item["stick_position"]) else "no",
                "is_comment_on_comment": "no"
                if bool(item["reply_id"] == "0")
                else "yes",
