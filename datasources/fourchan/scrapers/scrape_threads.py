...
class ThreadScraper4chan(BasicJSONScraper):
	...
	def process(self, data):
		...
		if missing != set():
			self.log.warning("%s OP is missing required fields %s, ignoring" % (self.datasource, repr(missing)))
			return False
	...
	def save_post(self, post, thread, first_post):
		...
		if missing != set():
			self.log.warning("Missing fields %s in scraped %s post, ignoring" % (repr(missing), self.datasource))
			return False
		...
		try:
			...
			self.db.rollback()
			self.log.error("ValueError (%s) during scrape of %s thread %s" % (e, self.datasource, post["no"]))
	...
	def register_thread(self, first_post, last_reply, last_post, num_replies):
		...
		if thread["num_replies"] == num_replies and num_replies != -1 and thread["timestamp_modified"] == last_reply:
			...
			self.log.debug("No new messages in %s thread %s" % (self.datasource, first_post["no"]))
			return None
		else:
			return thread
