...
class ImageDownloader(BasicWorker):
    ...
    def work(self):
        ...
        """
        try:
            url = "http://i.4cdn.org/%s/%s%s" % (
                self.job.details["board"],
                self.job.details["tim"],
                self.job.details["ext"],
            )
            ...
            if self.job.data["attempts"] > 2:
                self.log.error(
                    "Could not download image %s after 2 retries (%s), aborting"
                    % (self.job.details["tim"], e)
                )
                self.job.finish()
            else:
                self.log.info(
                    "HTTP Error %s while downloading image, retrying later" % e
                )
        ...
        if image.status_code != 200:
            ...
            if self.job.data["attempts"] > 2:
                self.log.error(
                    "Could not download image %s after 2 retries (last response code %i), aborting"
                    % (url, image.status_code)
                )
                self.job.finish()
            else:
                self.log.info(
                    "Got response code %i while trying to download image %s, retrying later"
                    % (image.status_code, url)
                )
