...
class BoardScraper4chan(BasicJSONScraper):
	...
	def save_thread(self, thread):
		...
		try:
			...
			self.queue.add_job(worker_or_type=jobtype, remote_id=thread["no"], details={"board": board_id})
		...
		if unindexed_threads:
			...
			for thread in unindexed_threads:
				...
				try:
					...
					if not remote_id:
						self.queue.add_job(worker_or_type=jobtype, remote_id=str(thread["id"]), details={"board": board_id})
						to_check += 1
