...
class BoardScraper4chan(BasicJSONScraper):
    ...
    def process(self, data):
        ...
        if not data:
            self.log.error(
                "No thread data from board scrape of %s/%s/"
        ...
        for page in data:
            ...
            for thread in page["threads"]:
                ...
                index_thread_ids.append(
                    thread["id"] if "id" in thread else thread["no"]
                )

        self.log.info(
            "Board scrape for %s/%s/ yielded %i new threads"
            % (self.datasource, self.job.data["remote_id"], new_threads)
        )
    ...
    def save_thread(self, thread):
        ...
        if missing != set():
            self.log.warning(
                "Missing fields %s in scraped thread from %s/%s/, ignoring: got %s"
                % (
                    repr(missing),
                    self.datasource,
                    self.job.data["remote_id"],
                    repr(thread),
                )
            )
            return False
        ...
        thread_data = {"id": thread_id, "board": board_id, "index_positions": ""}

        # schedule a job for scraping the thread's posts
        try:
            jobtype = self.type.replace("-board", "-thread")
            self.queue.add_job(
                jobtype=jobtype, remote_id=thread["no"], details={"board": board_id}
            )
        ...
        thread_row = self.db.fetchone(
            "SELECT * FROM threads_" + self.prefix + " WHERE id = %s AND board = %s",
            (str(thread_id), board_id),
        )
        new_thread = 0
        if not thread_row:
            new_thread += 1
        ...
        self.db.execute(
            "UPDATE threads_"
            + self.prefix
            + " SET timestamp_scraped = %s, timestamp_modified = %s"
            + positions_bit
            + " WHERE id = %s AND board = %s",
            replacements,
        )
    ...
    def update_unindexed_threads(self, index_thread_ids):
        ...
        unindexed_threads = self.db.fetchall(
            "SELECT id FROM threads_"
            + self.prefix
            + " WHERE board = %s AND timestamp_deleted = 0 AND timestamp_archived = 0 AND timestamp_modified > (EXTRACT(epoch FROM NOW()) - 3600) AND id NOT IN %s",
        ...
        if unindexed_threads:
            ...
            for thread in unindexed_threads:
                ...
                try:
                    ...
                    jobtype = self.type.replace("-board", "-thread")
                    query = (
                        "SELECT remote_id FROM jobs WHERE remote_id = '%s' AND details = '%s';"
                        % (str(thread["id"]), json.dumps({"board": board_id}))
                    )
            ...
            if to_check:
                self.log.info(
                    "Board scrape for %s/%s/ yielded %s threads that disappeared from the index, updating their status"
                    % (self.datasource, self.job.data["remote_id"], to_check)
                )
