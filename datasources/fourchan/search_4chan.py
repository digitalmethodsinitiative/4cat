...
class Search4Chan(SearchWithScope):
    ...
    title = "4chan search"
    sphinx_index = (
        "4chan"  # sphinx index name; this should match the index name in sphinx.conf
    )
    ...
    options = {
        ...
        "body_match": {"type": UserInput.OPTION_TEXT, "help": "Post contains"},
        "subject_match": {"type": UserInput.OPTION_TEXT, "help": "Subject contains"},
        "deleted_posts": {
            "type": UserInput.OPTION_INFO,
                ...
                if query_ids:
                    ...
                    if not valid_query_ids:
                        self.dataset.update_status(
                            "The IDs inserted are not valid 4chan post IDs."
                        )
                        return None

                    if len(valid_query_ids) > 5000000:
                        self.dataset.update_status(
                            "Too many IDs inserted. Max 5.000.000."
                        )
                        return None
    ...
    def get_items_complex(self, query):
        ...
        if use_sphinx:
            ...
            if query.get("subject_match", None):
                match.append(
                    "@subject " + self.convert_for_sphinx(query["subject_match"])
                )
        else:
            if query.get("body_match", None):
        ...
        if query.get("country_name", None) and not query.get(
            "check_dense_country", None
        ):
            where.append("country_name IN %s")
        ...
        # Query the postgres table immediately if we're not using sphinx.
            ...
            join = (
                " LEFT JOIN posts_%s_deleted ON posts_%s.id_seq = posts_%s_deleted.id_seq "
                % tuple([self.prefix] * 3)
            )
        ...
        self.dataset.update_status(
            "Found %i initial matches. Collecting post data" % len(posts)
        )
        ...
        posts_full = self.fetch_posts(
            tuple([post["post_id"] for post in posts]),
            join=postgres_join,
            where=postgres_where,
            replacements=postgres_replacements,
        )

        self.dataset.update_status("Post data collected")
        self.log.info(
            "Full posts query finished in %i seconds." % (time.time() - datafetch_start)
        )
    ...
    def fetch_threads(self, thread_ids):
        ...
        if self.interrupted:
            raise ProcessorInterruptedException(
                "Interrupted while fetching thread data"
            )
        ...
        return self.db.fetchall_interruptable(
            self.queue,
            "SELECT "
            + columns
            + " FROM posts_"
    ...
    def fetch_sphinx(self, where, replacements, join=""):
        ...
        try:
            sql = (
                "SELECT thread_id, post_id FROM `"
                + self.prefix
                + "_posts` "
                + join
                + " WHERE "
                + where
                + " LIMIT 5000000 OPTION max_matches = 5000000, ranker = none, boolean_simplify = 1, sort_method = kbuffer, cutoff = 5000000"
            )
            ...
            if "query was killed" in str(e):
                self.dataset.update_status(
                    "Search was interruped and will restart later"
                )
                raise ProcessorInterruptedException(
                    "Interrupted while running Sphinx query"
                )
            else:
                self.dataset.update_status(
                    "Error while querying full-text search index", is_final=True
                )
            ...
            self.log.info(
                "Sphinx query timed out after %i seconds" % (time.time() - sphinx_start)
            )
            return None
            ...
            if "invalid packet size" in str(e) or "query timed out" in str(e):
                ...
                self.dataset.update_status(
                    "Error during query. Please try a narrow query and double-check your syntax.",
                    is_final=True,
                )
                self.log.error(
                    "Sphinx crash during query %s: %s" % (self.dataset.key, e)
                )
            return None

        self.log.info(
            "Sphinx query finished in %i seconds, %i results."
            % (time.time() - sphinx_start, len(results))
        )
        return results
    ...
    def get_thread_sizes(self, thread_ids, min_length):
        ...
        thread_sizes = {
            row["thread_id"]: row["num_posts"]
            for row in self.db.fetchall_interruptable(
                self.queue,
                "SELECT COUNT(*) as num_posts, thread_id FROM posts_"
                + self.prefix
                + " WHERE thread_id IN %s GROUP BY thread_id",
                (thread_ids,),
            )
            if int(row["num_posts"]) > min_length
        }
    ...
    def validate_query(query, request, user):
        ...
        if (
            not user.is_admin
            and not config.get(
                "fourchan-search.can_query_without_keyword", False, user=user
            )
            ...
            raise QueryParametersException(
                "Please provide a message or subject search query"
            )
        ...
        if (
            query.get("search_scope") not in ("match-ids",)
            and "valid_ids" in query.keys()
        ):
            del query["valid_ids"]
