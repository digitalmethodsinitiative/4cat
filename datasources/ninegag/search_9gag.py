...
class SearchNineGag(Search):
    ...
    def get_items(self, query):
        ...
        raise NotImplementedError(
            "9gag datasets can only be created by importing data from elsewhere"
        )
    ...
    def map_item(post):
        ...
        image = sorted(
            [v for v in post["images"].values() if "hasAudio" not in v],
            key=lambda image: image["width"] * image["height"],
            reverse=True,
        )[0]
        video = sorted(
            [v for v in post["images"].values() if "hasAudio" in v],
            key=lambda image: image["width"] * image["height"],
            reverse=True,
        )
        ...
        return MappedItem(
            {
                ...
                "author_verified": "yes"
                if post["creator"]["isVerifiedAccount"]
                else "no",
                "type": post["type"],
                "image_url": image["url"],
                "video_url": video_url,
