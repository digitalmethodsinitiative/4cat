...
class SearchTikTok(Search):
    ...
    def get_items(self, query):
        ...
        raise NotImplementedError(
            "TikTok datasets can only be created by importing data from elsewhere"
        )
    ...
    def map_item(post):
        ...
        hashtags = [
            extra["hashtagName"]
            for extra in post.get("textExtra", [])
            if "hashtagName" in extra and extra["hashtagName"]
        ]

        labels = (
            ",".join(post["diversificationLabels"])
            if type(post.get("diversificationLabels")) is list
            else ""
        )
        ...
        thumbnail_url = [
            ...
            if int(parse_qs(urlparse(url).query).get("x-expires", [now])[0]) >= now
        ]
        ...
        return MappedItem(
            {
                ...
                "author_followers": post.get("authorStats", {}).get(
                    "followerCount", ""
                ),
                ...
                "body": post["desc"],
                "stickers": "\n".join(
                    " ".join(s["stickerText"]) for s in post.get("stickersOnItem", [])
                ),
                ...
                "is_duet": "yes"
                if (
                    post.get("duetInfo", {}).get("duetFromId") != "0"
                    if post.get("duetInfo", {})
                    else False
                )
                else "no",
                ...
                "tiktok_url": "https://www.tiktok.com/@%s/video/%s"
                % (user_nickname, post["id"]),
                ...
                "effects": ",".join(
                    [e["name"] for e in post.get("effectStickers", [])]
                ),
