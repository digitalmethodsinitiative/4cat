...
class SearchRedNoteComments(Search):
    ...
    def get_items(self, query):
        ...
        raise NotImplementedError(
            "RedNote/Xiaohongshu comment datasets can only be created by importing data from elsewhere"
        )
    ...
    def map_item(item):
        ...
        timestamp = datetime.fromtimestamp(int(item["create_time"]) / 1000).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
