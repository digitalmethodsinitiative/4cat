...
class SearchBluesky(Search):
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        if max_posts == 0:
            ...
            options["max_posts"]["default"] = (
                options["max_posts"]["default"]
                if options["max_posts"]["default"] <= max_posts
                else max_posts
            )
    ...
    def validate_query(query, request, user):
        ...
        if not query.get("username", None) or not query.get("password", None):
            raise QueryParametersException(
                "You need to provide valid Bluesky login credentials first."
            )
        ...
        session_id = SearchBluesky.create_session_id(
            query["username"], query["password"]
        )
        try:
            SearchBluesky.bsky_login(
            ...
            if e.response.content.message == "Rate Limit Exceeded":
                lifted_at = datetime.fromtimestamp(
                    int(e.response.headers["ratelimit-reset"])
                )
                raise QueryParametersException(
                    f"Bluesky rate limit exceeded. Try again after {lifted_at.strftime('%Y-%m-%d %H:%M:%S')}."
                )
            else:
                raise QueryParametersException(
                    f"Bluesky login failed. {e.response.content.message}"
                )

        # sanitize query
        sanitized_query = [
            q.strip()
            for q in query.get("query").replace("\n", ",").split(",")
            if q.strip()
        ]
        ...
        if not query.get("frontend-confirm"):
            ...
            if expected_tweets > (posts_per_second * 3600):
                ...
                raise QueryNeedsExplicitConfirmationException(
                    f"This query matches approximately {expected_tweets} tweets and may take {expected_time} to complete. Do you want to continue?"
                )
            elif max_posts == 0 and not min_date:
                raise QueryNeedsExplicitConfirmationException(
                    "No maximum number of posts set! This query has no minimum date and thus may take a very, very long time to complete. Do you want to continue?"
                )
            elif max_posts == 0:
                raise QueryNeedsExplicitConfirmationException(
                    "No maximum number of posts set! This query may take a long time to complete. Do you want to continue?"
                )
    ...
    def get_items(self, query):
        ...
        """
        if not query.get("session_id") and (
            not query.get("username") or not query.get("password")
        ):
            return self.dataset.finish_with_error(
                "Your Bluesky login credentials are no longer available in 4CAT; please re-create this datasource."
            )

        session_id = (
            SearchBluesky.create_session_id(
                query.get("username"), query.get("password")
            )
            if not query.get("session_id")
            else query["session_id"]
        )
        try:
            client = SearchBluesky.bsky_login(
                username=query.get("username"),
                password=query.get("password"),
                session_id=session_id,
            )
            ...
            return self.dataset.finish_with_error(
                "Bluesky login failed; please re-create this datasource."
            )

        self.dataset.update_status(
            f"Collecting posts from Bluesky as {client.me.handle}"
        )
        ...
        if self.parameters.get("min_date"):
            query_parameters["since"] = datetime.fromtimestamp(
                self.parameters.get("min_date")
            ).strftime("%Y-%m-%dT%H:%M:%SZ")
        if self.parameters.get("max_date"):
            query_parameters["until"] = datetime.fromtimestamp(
                self.parameters.get("max_date")
            ).strftime("%Y-%m-%dT%H:%M:%SZ")
        ...
        while queries:
            ...
            if query == last_query:
                ...
                self.dataset.log(
                    f"Continuing query ({i} of {num_queries}): {query} from {last_date.strftime('%Y-%m-%dT%H:%M:%SZ')}"
                )
            else:
                # New query
                query_post_ids = set()
                ...
                last_date = None
                self.dataset.update_status(
                    f"Collecting query ({i} of {num_queries}): {query}"
                )
                query_requests = 0
            ...
            while True:
                if self.interrupted:
                    raise ProcessorInterruptedException(
                        "Interrupted while getting posts from the Bluesky API"
                    )
                ...
                while tries < 3:
                    ...
                    try:
                        ...
                        if not search_for_invalid_post:
                            ...
                            self.dataset.log(
                                f"Invalid post detected; searching post by post: {e}"
                            )
                        ...
                        self.dataset.log(
                            f"Bluesky request error for query {query}: {e}"
                        )
                        time.sleep(1)
                        tries += 2
                        continue
                        ...
                        self.dataset.log(
                            f"Bluesky network error for query {query}; retrying: {e}"
                        )
                ...
                if not response:
                    ...
                    if len(queries) == 0:
                        self.dataset.update_status(
                            "Error collecting posts from Bluesky; see log for details",
                            is_final=True,
                        )
                    if query not in queries:
                        # Query was not added back; there was an unexpected issue with the query itself
                        self.dataset.update_status(
                            f"Error continuing {query} from Bluesky (see log for details); continuing to next query"
                        )
                    break
                ...
                if search_for_invalid_post:
                    ...
                    if invalid_post_counter >= 100:
                        ...
                        self.dataset.log(
                            "Unable to identify invalid post; discontinuing search"
                        )
                        query_parameters["limit"] = limit
                        search_for_invalid_post = False
                        invalid_post_counter = 0
                    ...
                    if not items:
                        ...
                        self.dataset.log(
                            f"Query {query} w/ params {query_parameters} returned no posts: {response}"
                        )
                ...
                for item in items:
                    ...
                    last_date = SearchBluesky.bsky_convert_datetime_string(
                        post.get("indexed_at")
                    )
                                ...
                                if feature.get("did"):
                                    if feature["did"] in did_to_handle:
                                        mentions.append(
                                            {
                                                "did": feature["did"],
                                                "handle": did_to_handle[feature["did"]],
                                            }
                                        )
                                    else:
                                        handle = SearchBluesky.bsky_get_handle_from_did(
                                            client, feature["did"]
                                        )
                                        if handle:
                                            if (
                                                handle.lower()
                                                in self.handle_lookup_error_messages
                                            ):
                                                self.dataset.log(
                                                    f"Bluesky: user ({feature['did']}) {handle}"
                                                )
                                            mentions.append(
                                                {
                                                    "did": feature["did"],
                                            ...
                                            mentions.append(
                                                {"did": feature["did"], "handle": None}
                                            )
                                            self.dataset.log(
                                                f"Bluesky: could not lookup the handle for {feature['did']}"
                                            )
                    # Reply to
                    reply_to_handle = None
                    if post["record"].get("reply"):
                        reply_to_did = post["record"]["reply"]["parent"]["uri"].split(
                            "/"
                        )[2]
                        if reply_to_did in did_to_handle:
                            ...
                            if handle:
                                if handle.lower() in self.handle_lookup_error_messages:
                                    self.dataset.log(
                                        f"Bluesky: user ({reply_to_did}) {handle}"
                                    )
                                reply_to_handle = handle
                                did_to_handle[reply_to_did] = handle
                            else:
                                self.dataset.log(
                                    f"Bluesky: could not find handle for {reply_to_did}"
                                )
                ...
                if max_posts != 0 and rank % (max_posts // 10) == 0:
                    self.dataset.update_status(
                        f"Progress query {query}: {rank} posts collected out of {max_posts}"
                    )
                    ...
                    self.dataset.update_status(
                        f"Progress query {query}: {rank} posts collected"
                    )
                ...
                if not cursor:
                    if new_posts:
                        ...
                        self.dataset.log(
                            f"Query {query}: {query_requests} requests; no additional posts returned"
                        )
    ...
    def map_item(item):
        ...
        link = SearchBluesky.get_bsky_link(
            item["author"]["handle"], item["uri"].split("/")[-1]
        )
        author_profile = f"https://bsky.app/profile/{item['author']['handle']}"

        created_at = SearchBluesky.bsky_convert_datetime_string(
            item["record"].get("created_at", item["record"].get("createdAt"))
        )
                ...
                for feature in facet.get("features"):
                    if feature.get("tag"):
                        ...
                        unmapped_data.append(
                            {"loc": "record.facets.features", "obj": feature}
                        )
        ...
        while possible_embeds:
            ...
            py_type = (
                embed.pop("py_type")
                if "py_type" in embed
                else (embed.pop("$type") if "$type" in embed else None)
            )
            if py_type in [
                ...
                "images" in embed
            ):  # py_type in ["app.bsky.embed.images#view", "app.bsky.embed.images", "app.bsky.embed.images#main"]
                for img_ob in embed["images"]:
                    ...
                    if img_link:
                        ...
                        .get("ref", {})
                        .get(
                            "link", img_ob.get("image", {}).get("ref", {}).get("$link")
                        )
                    ):
                        ...
                        image_references.add(
                            img_ob.get("image", {})
                            .get("ref", {})
                            .get(
                                "link",
                                img_ob.get("image", {}).get("ref", {}).get("$link"),
                            )
                        )
                    else:
                ...
                if embed.get("thumbnail"):
                    ...
                    image_references.add(
                        embed.get("video", {}).get("ref", {}).get("link")
                    )
                else:
                    # No thumb for video
                    pass
                ...
                "record" in embed
            ):  # py_type in ["app.bsky.embed.record#view", "app.bsky.embed.record"]
                # Quoted post
                ...
                if embed["record"].get("author", embed["record"].get("creator")):
                    if "handle" not in embed["record"].get(
                        "author", embed["record"].get("creator")
                    ):
                        ...
                        if (
                            ...
                            unmapped_data.append(
                                {
                                    "loc": "embed.record.author",
                                    "obj": embed["record"].get(
                                        "author", embed["record"].get("creator")
                                    ),
                                }
                            )
                        ...
                        quoted_user = embed["record"].get(
                            "author", embed["record"].get("creator")
                        )["handle"]
                        quoted_link = SearchBluesky.get_bsky_link(
                            quoted_user, embed["record"]["uri"].split("/")[-1]
                        )
                ...
                "external" in embed
            ):  # py_type in ["app.bsky.embed.external#view", "app.bsky.embed.external"]
                ...
                if embed["external"].get("thumb"):
                    if isinstance(embed["external"]["thumb"], str):
                        ...
                        image_references.add(
                            embed["external"]["thumb"].get("ref", {}).get("link", "")
                        )
                else:
                    # No thumb for link
                    pass
            else:
                unmapped_data.append({"loc": f"embed.{py_type}", "obj": embed})
        ...
        labels = (
            set()
            if not item["labels"]
            else set([label.get("val") for label in item["labels"]])
        )
        if item["record"].get("labels"):
            labels = labels | set(
                [
                    label.get("val")
                    for label in item["record"]["labels"].get("values", [])
                ]
            )
        ...
        languages = (
            "N/A"
            if not item["record"].get("langs")
            else ",".join(item["record"].get("langs"))
        )
        ...
        if any([ref for ref in image_references if ref not in "".join(embeded_images)]):
            unmapped_data.append(
                {
                    "loc": "missing_image_refs",
                    "obj": [
                        ref
                        for ref in image_references
                        if ref not in "".join(embeded_images)
                    ],
                }
            )
        if quoted_ref:
            if not quoted_link or (
                quoted_link
                not in ["DELETED", "REMOVED BY AUTHOR", "VIEWER BLOCKED BY AUTHOR"]
                and quoted_ref.split("/")[-1] not in quoted_link
            ):
        ...
        mentions = [
            (
                ...
                if (
                    mention.get("handle")
                    and mention["handle"].lower()
                    not in SearchBluesky.handle_lookup_error_messages
                )
                else mention.get("did")
            )
            for mention in item.get("4CAT_metadata", {}).get("mentions", [])
        ]
        # Reply to
        ...
        if item["record"].get("reply"):
            if (
                item["4CAT_metadata"]["reply_to"]
                and item["4CAT_metadata"]["reply_to"].lower()
                not in SearchBluesky.handle_lookup_error_messages
            ):
            ...
            replied_to_post = SearchBluesky.get_bsky_link(
                replied_to_user, item["record"]["reply"]["parent"]["uri"].split("/")[-1]
            )
        ...
        return MappedItem(
            {
                "collected_at": datetime.fromtimestamp(
                    item["4CAT_metadata"]["collected_at"]
                ).isoformat(),
                "query": item["4CAT_metadata"]["query"],
                "rank": item["4CAT_metadata"]["rank"],
                "id": item["uri"],
                "thread_id": item["record"]["reply"]["root"]["uri"]
                if item["record"].get("reply")
                else item["uri"],
                ...
                "author_created_at": SearchBluesky.bsky_convert_datetime_string(
                    item["author"]["created_at"], mode="iso_string", raise_error=False
                ),
    ...
    @staticmethod
    def bsky_convert_datetime_string(
        datetime_string, mode="datetime", raise_error=True
    ):
        """
    ...
    def bsky_login(username, password, session_id):
        ...
        if not session_id:
            ...
            raise ValueError(
                "Must provide both username and password or else session_id."
            )

        session_path = Path(config.get("PATH_ROOT")).joinpath(
            config.get("PATH_SESSIONS"), "bsky_" + session_id + ".session"
        )
                ...
                try:
                    ...
                    if e.response.content.message == "Token has expired":
                        ...
                        if username and password:
                            ...
                            raise ValueError(
                                "Session token has expired; please re-login with username and password."
                            )
        else:
