...
class SearchDouban(Search):
    ...
    options = {
        ...
        "daterange": {"type": UserInput.OPTION_DATERANGE, "help": "Last post between"},
    }
            ...
            while True:
                ...
                if request.status_code != 200:
                    self.dataset.update_status(
                        "Got response code %i for group %s. Continuing with next group..."
                        % (request.status_code, group)
                    )
                    break
                ...
                for topic in overview_page.select("table.olt tr:not(.th)"):
                    if self.interrupted:
                        raise ProcessorInterruptedException(
                            "Interrupted while scraping Douban topics"
                        )
                    ...
                    topic_is_elite = (
                        "yes" if bool(topic.select_one(".elite_topic_lable")) else "no"
                    )
                    ...
                    try:
                        topic_updated = int(
                            datetime.datetime.strptime(
                                topic.select_one(".time").text, "%m-%d %H:%M"
                            ).timestamp()
                        )
                    except ValueError:
                        topic_updated = int(
                            datetime.datetime.strptime(
                                topic.select_one(".time").text, "%Y-%m-%d"
                            ).timestamp()
                        )
                    ...
                    self.dataset.update_status(
                        "%i posts scraped. Scraping topics %i-%i from group %s"
                        % (
                            posts_processed,
                            offset,
                            min(max_topics, offset + 50),
                            group_name,
                        )
                    )
                    ...
                    try:
                        first_post = {
                            ...
                            "body": topic_page.select_one(".topic-richtext")
                            .decode_contents(formatter="html")
                            .strip(),
                            "author": topic.select_one(".user-face img").get("alt"),
                            "author_id": topic.select_one(".user-face a")
                            .get("href")
                            .split("/people/")
                            .pop()
                            .split("/")[0],
                            "author_avatar": topic.select_one(".user-face img")
                            .get("src")
                            .replace("/u", "/ul"),
                            "timestamp": int(
                                datetime.datetime.strptime(
                                    topic.select_one(".create-time").text,
                                    "%Y-%m-%d %H:%M:%S",
                                ).timestamp()
                            ),
                            "likes": 0,
                            ...
                            "image_urls": ",".join(
                                [
                                    img.get("src")
                                    for img in topic.select(".topic-richtext img")
                                ]
                            ),
                        }
                    except (AttributeError, ValueError):
                        self.dataset.log(
                            "Unexpected data format when parsing topic %s/%s, skipping"
                            % (group_name, topic_id)
                        )
                        continue
                    ...
                    for comment in topic_page.select("ul#comments > li"):
                        comment_data = {
                            ...
                            "group_name": group_name,
                            "subject": "",
                            "body": comment.select_one(".reply-content")
                            .decode_contents(formatter="html")
                            .strip(),
                            "author": comment.select_one(".user-face img").get("alt"),
                            "author_id": comment.select_one(".user-face a")
                            .get("href")
                            .split("/people/")
                            .pop()
                            .split("/")[0],
                            "author_avatar": comment.select_one(".user-face img")
                            .get("src")
                            .replace("/u", "/ul"),
                            "timestamp": int(
                                datetime.datetime.strptime(
                                    comment.select_one(".pubtime").text.strip()[:19],
                                    "%Y-%m-%d %H:%M:%S",
                                ).timestamp()
                            ),
                            "likes": convert_to_int(
                                re.sub(
                                    r"[^0-9]",
                                    "",
                                    comment.select_one(".comment-vote.lnk-fav").text,
                                ),
                                0,
                            )
                            if comment.select_one(".comment-vote.lnk-fav")
                            else "Unknown",
                            "is_highlighted": "yes"
                            if comment.get("data-cid")
                            in [
                                hl.get("data-cid")
                                for hl in comment.select("ul#popular-comments li")
                            ]
                            else "no",
                            "is_reply": "yes"
                            if comment.select_one(".reply-quote-content")
                            else "no",
                            "is_topic_elite": topic_is_elite,
                            "image_urls": ",".join(
                                [
                                    img.get("src")
                                    for img in comment.select(".reply-content img")
                                ]
                            ),
                        }
    ...
    def validate_query(query, request, user):
        ...
        # limit to 25
        groups = [
            group.split("/group/").pop().split("/")[0].strip()
            for group in query["groups"].replace("\n", ",").split(",")
        ]
