...
class SearchThreads(Search):
    ...
    def get_items(self, query):
        ...
        raise NotImplementedError(
            "Threads datasets can only be created by importing data from elsewhere"
        )
    ...
    def map_item(post):
        ...
        if post["carousel_media"]:
            image_urls = [
                c["image_versions2"]["candidates"].pop(0)["url"]
                for c in post["carousel_media"]
                if c["image_versions2"] and c["image_versions2"].get("candidates")
            ]
            video_urls = [
                c["video_versions"].pop(0)["url"]
                for c in post["carousel_media"]
                if c["video_versions"]
            ]
            ...
            image_urls = (
                [post["image_versions2"]["candidates"].pop(0)["url"]]
                if post["image_versions2"] and post["image_versions2"].get("candidates")
                else []
            )
            video_urls = (
                [post["video_versions"].pop(0)["url"]] if post["video_versions"] else []
            )
        ...
        if post["text_post_app_info"].get("link_preview_attachment"):
            ...
            if parsed_url:
                ...
                link_thumbnail = post["text_post_app_info"][
        ...
        return MappedItem(
            {
                ...
                "author_is_verified": "yes"
                if post["user"].get("is_verified")
                else "no",
