...
class SearchCustom(BasicProcessor):
    ...
    options = {
        ...
        "data_upload": {"type": UserInput.OPTION_FILE, "help": "File"},
        "format": {
    ...
    def process(self):
        ...
        while possible_dialects:
            ...
            self.dataset.log(
                f"Importing CSV file with dialect: {vars(dialect) if type(dialect) is csv.Dialect else dialect}"
            )
            reader = csv.DictReader(infile, dialect=dialect)

            if (
                tool_format.get("columns")
                and not tool_format.get("allow_user_mapping")
                and set(reader.fieldnames) & set(tool_format["columns"])
                != set(tool_format["columns"])
            ):
            ...
            with self.dataset.get_results_path().open(
                ...
                try:
                    for i, item in enumerate(
                        ...
                        if not writer:
                            writer = csv.DictWriter(
                                output_csv, fieldnames=list(item.keys())
                            )
                            writer.writeheader()
                        ...
                        if not item.get("timestamp"):
                            ...
                            timestamp_missing += 1
                            self.dataset.log(
                                f"Item {i} ({item.get('id')}) has no timestamp."
                            )
                        ...
                        try:
                            ...
                            if not possible_dialects:
                                self.dataset.log(
                                    f"Error ({e}) writing item {i}: {item}"
                                )
                                return self.dataset.finish_with_error(
                                    "Could not parse CSV file. Have you selected the correct "
                                    "format or edited the CSV after exporting? Try importing "
                                    "as custom format."
                                )
                            else:
                                raise CsvDialectException(
                                    f"Error ({e}) writing item {i}: {item}"
                                )
                    ...
                    self.log.warning(
                        f"Unable to import improperly formatted file for {tool_format['name']}. See dataset "
                        "log for details."
                    )
                    infile.close()
                    temp_file.unlink()
                    ...
                    temp_file.unlink()
                    return self.dataset.finish_with_error(
                        "The uploaded file is not encoded with the UTF-8 character set. "
        ...
        if skipped and not done:
            self.dataset.finish_with_error(
                "No valid items could be found in the uploaded file. The column containing "
    ...
    def validate_query(query, request, user):
        ...
        if query.get("format") not in import_formats.tools:
            raise QueryParametersException(
                f"Cannot import CSV from tool {query.get('format')}"
            )
        ...
        try:
            ...
            )  # 128 kB is sent from the frontend at most
            wrapped_file = io.TextIOWrapper(file, encoding=encoding)
            sample = wrapped_file.read(sample_size)

            if not csv.Sniffer().has_header(sample) and not query.get(
                "frontend-confirm"
            ):
            ...
            raise QueryParametersException(
                "The uploaded file does not seem to be a CSV file encoded with UTF-8. "
                "Save the file in the proper format and try again."
            )
        except csv.Error:
            raise QueryParametersException(
                "Uploaded file is not a well-formed, UTF 8-encoded CSV or TAB file."
            )
        ...
        try:
            ...
            raise QueryParametersException(
                "The uploaded file is not a well-formed, UTF 8-encoded CSV or TAB file."
            )

        incomplete_mapping = list(tool_format["columns"])
        for field in tool_format["columns"]:
            if (
                tool_format.get("allow_user_mapping", False)
                and "option-mapping-%s" % field in request.form
            ):
        ...
        if tool_format.get("allow_user_mapping", False):
                        ...
                        **{
                            "mapping-%s" % mappable_column: {
                                ...
                                "options": {
                                    "": "",
                                    **magic_mappings.get(
                                        mappable_column, magic_mappings["empty"]
                                    ),
                                    **{column: column for column in fields},
                                },
                                "default": mappable_column
                                if mappable_column in fields
                                else "",
            ...
            for field in tool_format["columns"]:
                ...
                if (
                    provided_field not in fields
                    and not provided_field.startswith("__4cat")
                ) or not provided_field:
            ...
            raise QueryParametersException(
                "The CSV file does not contain all required columns. The following columns "
