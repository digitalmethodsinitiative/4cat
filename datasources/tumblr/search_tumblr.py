...
class SearchTumblr(Search):
    ...
    description = (
        "Retrieve Tumblr posts by tags or blogs."  # description displayed in UI
    )
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        options["divider"] = {"type": UserInput.OPTION_DIVIDER}
        options["date-intro"] = {
            "type": UserInput.OPTION_INFO,
    ...
    def get_items(self, query):
        ...
        try:
            ...
            self.log.warning(
                "Could not connect to Tumblr API: API keys invalid or not set"
            )
            ...
            self.log.warning(
                f"Could not connect to Tumblr API: {e}; client_info: {client_info}"
            )
            self.dataset.finish_with_error(
        ...
        for query in queries[:10]:
            ...
            if query.startswith("@"):
                ...
                new_results = self.get_posts_by_blog(
                    blog_name, post_id=post_id, max_date=max_date, min_date=min_date
                )
            ...
            # Post URL
                ...
                try:
                    ...
                    # Format https://tumblr.com/{blogname}/{post_id}
                        ...
                        blog_and_id = blog_and_id.replace(
                            "blog/view/", ""
                        )  # Sometimes present in the URL
            ...
            # Get tagged post
                ...
                new_results = self.get_posts_by_tag(
                    query, max_date=max_date, min_date=min_date, api_key=api_key
                )
        ...
        if get_notes:
            for i, post in enumerate(results):
                ...
                self.dataset.update_status(
                    "Retrieving notes for post %i/%i" % (i + 1, len(results))
                )
                ...
                # we may have all the possible notes in the retrieved JSON.
                    ...
                    notes = {
                        "notes": post["notes"],
                        "reply_count": len(
                            [n for n in post["notes"] if n["type"] == "reply"]
                        ),
                        "reblog_count": len(
                            [n for n in post["notes"] if n["type"] == "reblog"]
                        ),
                        "like_count": len(
                            [n for n in post["notes"] if n["type"] == "like"]
                        ),
                    }
                    ...
                    if (
                        reblog_type == "text_or_tag"
                        and reblog_count <= self.max_reblogs
                    ):
                        tag_notes = self.get_notes(
                            post["blog_name"],
                            post["id"],
                ...
                if get_reblogs:
                    for note in notes["notes"]:
                        ...
                        if note["post_id"] not in self.seen_ids:
                            ...
                            extra_posts.append(
                                {"blog": note["blog_name"], "id": note["post_id"]}
                            )

        # Add reblogged posts and reblogs to dataset
        for i, extra_post in enumerate(extra_posts):
            self.dataset.update_status(
                "Adding %s/%s reblogs to the dataset" % (i, len(extra_posts))
            )
            ...
            if extra_post["id"] not in self.seen_ids:
                ...
                if not reblog_outside_daterange and (max_date and min_date):
                    new_post = self.get_posts_by_blog(
                        extra_post["blog"],
                        extra_post["id"],
                        max_date=max_date,
                        min_date=min_date,
                ...
                if new_post:
                    ...
                    if get_notes:
                        new_post = {
                            **new_post,
                            **retrieved_notes[new_post["reblog_key"]],
                        }
    ...
    def get_posts_by_tag(self, tag, max_date=None, min_date=None, api_key=None):
        ...
        while True:
            ...
            if max_date_original:
                posts = [
                    post for post in posts if post["timestamp"] <= max_date_original
                ]
            ...
            if not posts:
                ...
                if date_retries < 96:
                    ...
                    self.dataset.update_status(
                        "No new posts found for #%s - looking for posts before %s"
                        % (
                            tag,
                            datetime.fromtimestamp(max_date).strftime(
                                "%Y-%m-%d %H:%M:%S"
                            ),
                        )
                    )
                ...
                if min_date != 0:
                    if max_date < min_date:
                        ...
                        posts = [
                            post
                            for post in posts
                            if min_date <= post["timestamp"] <= max_date_original
                        ]
            ...
            self.dataset.update_status(
                "Collected %s posts for #%s, retrieving posts before %s"
                % (
                    str(len(all_posts)),
                    tag,
                    max_date_str,
        ...
        while True:
            ...
            try:
                ...
                if post_id:
                    ...
                    self.dataset.update_status(
                        "ConnectionRefused: Unable to collect post %s/%s"
                        % (blog, post_id)
                    )
                else:
                    self.dataset.update_status(
                        "ConnectionRefused: Unable to collect posts for blog %s before %s"
                        % (blog, max_date)
                    )
                time.sleep(10)
                continue
                ...
                self.dataset.update_status(
                    "Couldn't collect posts; likely reached the limit of the Tumblr API (%s)."
            ...
            if max_date and min_date:
                new_posts = [
                    p for p in new_posts if min_date <= p["timestamp"] <= max_date
                ]
            ...
            self.dataset.update_status(
                "Collected %s posts for blog %s" % (str(len(all_posts)), blog)
            )
            time.sleep(0.2)
    ...
    def get_notes(
        self, blog_id, post_id, mode="conversation", max_reblogs=1000
    ) -> dict:
        """
        ...
        while True:
            if notes_retries >= max_reblogs_retries:
                self.dataset.update_status(
                    "Too many connection errors; unable to collect notes for post %s"
                    % post_id
                )
                self.failed_posts.append(post_id)
                break
            ...
            try:
                ...
                notes = self.client.notes(
                    blog_id, id=post_id, before_timestamp=max_date, mode=mode
                )
            except ConnectionRefusedError:
                self.dataset.update_status(
                    "Couldn't get notes for post %s (ConnectionRefusedError), trying again"
                    % post_id
                )
                notes_retries += 1
                time.sleep(10)
                ...
                self.dataset.update_status(
                    "Couldn't get notes for post %s. Unknown error: %s" % (post_id, e)
                )
                notes_retries += 1
                break
            ...
            if "notes" in notes:
                ...
                for note in notes["notes"]:
                    ...
                    if count >= max_reblogs:
                        post_notes = post_notes[
                            : count + note_metrics.get("reply_count", 0)
                        ]
                ...
                if notes.get("_links"):
                    ...
                    self.dataset.update_status(
                        ...
                        % (count, note_type, blog_id, post_id)
                    )
                    time.sleep(0.2)
    ...
    def get_tumblr_keys(user):
        ...
        if not all(config_keys):
            raise ConfigException(
                "Not all Tumblr API credentials are configured. Cannot query Tumblr API."
            )
        return config_keys
    ...
    def connect_to_tumblr(self):
        ...
        # User input keys
        config_keys = [
            self.parameters.get("consumer_key"),
    ...
    def map_item(post):
        ...
        for i in content_order:
            ...
            # Audio file
                ...
                audio_urls.append(
                    block["url"] if "url" in block else block["media"]["url"]
                )
        ...
        return MappedItem(
            {
                ...
                if "original_type" in post
                else post["type"],
                "id": post["id"] if "id" in post else post["post"]["id"],
                "author": post["blog_name"],
                "author_avatar_url": "https://api.tumblr.com/v2/blog/"
                + post["blog_name"]
                + "/avatar",
                "thread_id": post["reblog_key"],
                "timestamp": datetime.fromtimestamp(post["timestamp"]).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
                ...
                if fmt["type"] in ("link", "bold", "italic"):
                    ...
                    for n in [s, e]:
                        ...
                        if fmt_type == "link" and opening:
                            ...
                            inserts[n] = (
                                "*" + inserts[n] if opening else inserts[n] + "*"
                            )
                        elif fmt_type == "bold":
                            inserts[n] = (
                                "**" + inserts[n] if opening else inserts[n] + "**"
    ...
    def after_process(self):
        ...
        if len(self.failed_notes) > 0:
            errors.append(
                "API error(s) when fetching notes %s" % ", ".join(self.failed_notes)
            )
        if len(self.failed_posts) > 0:
            errors.append(
                "API error(s) when fetching reblogs %s" % ", ".join(self.failed_posts)
            )
        if errors:
            self.dataset.log(";\n ".join(errors))
            self.dataset.update_status(
