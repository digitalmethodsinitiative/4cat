...
class SearchTwitterViaZeeschuimer(Search):
    ...
    def get_items(self, query):
        ...
        raise NotImplementedError(
            "Twitter datasets can only be created by importing data from elsewhere"
        )
    ...
    def map_item_modern(tweet):
        ...
        if retweet:
            ...
            if retweet["result"].get("legacy", {}).get("withheld_scope"):
                ...
                t_text = (
                    "RT @"
                    + retweet["result"]["core"]["user_results"]["result"][user_key][
                        "screen_name"
                    ]
                    + ": "
        ...
        quote_withheld = (
            True if (quote_tweet and "tombstone" in quote_tweet["result"]) else False
        )
        ...
        return {
            ...
            "author_fullname": tweet["core"]["user_results"]["result"][user_key][
                "name"
            ],
            "author_id": tweet["legacy"]["user_id_str"],
            "author_avatar_url": tweet["core"]["user_results"]["result"]["avatar"][
                "image_url"
            ]
            if "avatar" in tweet["core"]["user_results"]["result"]
            else tweet["core"]["user_results"]["result"]["legacy"].get(
                "profile_image_url_https", ""
            ),
            "author_banner_url": tweet["core"]["user_results"]["result"]["legacy"].get(
                "profile_banner_url", ""
            ),  # key does not exist when author does not have a banner
            "verified": tweet["core"]["user_results"]["result"].get(
                "is_blue_verified", ""
            ),
            ...
            or tweet["legacy"].get("possibly_sensitive", False)
            else "no",
            ...
            "retweeted_user": retweet["result"]["core"]["user_results"]["result"]
            .get("legacy", {})
            .get("screen_name", "")
            if retweet
            else "",
            ...
            "quote_author": quote_tweet["result"]["core"]["user_results"]["result"]
            .get(user_key, {})
            .get("screen_name", "")
            if (quote_tweet and not quote_withheld)
            else "",
            "quote_body": quote_tweet["result"]["legacy"].get("full_text", "")
            if quote_tweet and not quote_withheld
            else "",
            "quote_images": ",".join(
                [
                    media["media_url_https"]
                    for media in quote_tweet["result"]["legacy"]
                    .get("entities", {})
                    .get("media", [])
                    if media["type"] == "photo"
                ]
            ...
            "is_reply": "yes"
            if str(tweet["legacy"]["conversation_id_str"]) != str(tweet["rest_id"])
            else "no",
            "replied_author": tweet["legacy"].get("in_reply_to_screen_name", ""),
            "is_withheld": "yes" if withheld else "no",
            "hashtags": ",".join(
                [hashtag["text"] for hashtag in tweet["legacy"]["entities"]["hashtags"]]
            ),
            "urls": ",".join(
                [
                    url.get("expanded_url", url["display_url"])
            ...
            "mentions": ",".join(
                [
                    media["screen_name"]
                    for media in tweet["legacy"]["entities"]["user_mentions"]
                ]
            ),
            "long_lat": SearchTwitterViaZeeschuimer.get_centroid(
                tweet["legacy"]["place"]["bounding_box"]["coordinates"]
            )
            if tweet["legacy"].get("place")
            else "",
            "place_name": tweet["legacy"].get("place", {}).get("full_name", "")
            if tweet["legacy"].get("place")
            else "",
        }
    ...
    def map_item_legacy(tweet):
        ...
        if retweet:
            ...
            if retweet["result"].get("legacy", {}).get("withheld_status"):
                ...
                t_text = (
                    "RT @"
                    + retweet["result"]["core"]["user_results"]["result"]["legacy"][
                        "screen_name"
                    ]
                    + " "
                    + retweet["result"]["legacy"]["full_text"]
        ...
        return {
            ...
            "language_guess": tweet["legacy"].get("lang"),
            "possibly_sensitive": "yes"
            if tweet["legacy"].get("possibly_sensitive")
            else "no",
            "retweet_count": tweet["legacy"]["retweet_count"],
            "reply_count": tweet["legacy"]["reply_count"],
            "like_count": tweet["legacy"]["favorite_count"],
            ...
            "is_reply": "yes"
            if str(tweet["legacy"]["conversation_id_str"]) != tweet_id
            else "no",
            "replied_author": tweet["legacy"].get("in_reply_to_screen_name", "")
            if tweet["legacy"].get("in_reply_to_screen_name")
            else "",
            "is_withheld": "yes" if withheld else "no",
            "hashtags": ",".join(
                [hashtag["text"] for hashtag in tweet["legacy"]["entities"]["hashtags"]]
