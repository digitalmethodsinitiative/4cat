...
class SearchWithinTCATBinsV2(Search):
    ...
    def get_items(self, query):
        ...
        instance = [
            ...
            if instance.get("tcat_name") == tcat_name
        ][0]
        ...
        if self.parameters.get("query_type") == "advanced":
            ...
            for result in unbuffered_cursor.fetchall_unbuffered():
                ...
                new_result["thread_id"] = (
                    new_result.get("in_reply_to_status_id")
                    if new_result.get("in_reply_to_status_id")
                    else new_result.get("quoted_status_id")
                    if new_result.get("quoted_status_id")
                    else new_result.get("id")
                )
            ...
            while start_of_match >= 0:
                ...
                if match:
                    ...
                    replacements.append(
                        "%"
                        + text_query[placeholder : placeholder + start_of_match]
                        .lower()
                        .strip()
                        + "%"
                    )
                    ...
                    replacements.append(
                        "%" + text_query[placeholder:].lower().strip() + "%"
                    )
            ...
            if query.get("min_date", None):
                try:
                    if int(query.get("min_date")) > 0:
                        ...
                        replacements.append(
                            datetime.datetime.fromtimestamp(int(query.get("min_date")))
                        )
                except ValueError:
                    pass
            ...
            if query.get("max_date", None):
                try:
                    if int(query.get("max_date")) > 0:
                        where.append("AND created_at < %s")
                        replacements.append(
                            datetime.datetime.fromtimestamp(int(query.get("max_date")))
                        )
                except ValueError:
                    pass
            ...
            self.dataset.log(
                "Replacements: %s" % ", ".join([str(i) for i in replacements])
            )
            unbuffered_cursor = db.connection.cursor(pymysql.cursors.SSCursor)
    ...
    def tweet_mapping(tweet):
        ...
        mapped_tweet = {
            ...
            # with added text)
            "thread_id": tweet.get("in_reply_to_status_id")
            if tweet.get("in_reply_to_status_id")
            else tweet.get("retweet_id")
            if tweet.get("retweet_id")
            else tweet.get("id"),
            "body": tweet.get("text", ""),
            # 'created_at': tweet.get('created_at'),
            "timestamp": int(datetime.datetime.timestamp(tweet.get("created_at")))
            if type(tweet.get("created_at")) is datetime.datetime
            else None,
            "subject": "",
            ...
            # 4CAT specifics
            "hashtags": ",".join(
                re.findall(r"#([^\s!@#$%^&*()_+{}:\"|<>?\[\];'\,./`~]+)", tweet["text"])
            ),
            "urls": ",".join(ural.urls_from_text(tweet["text"])),
            "images": ",".join(
                re.findall(r"https://t\.co/[a-zA-Z0-9]+$", tweet["text"])
            ),
            "mentions": ",".join(
                re.findall(r"@([^\s!@#$%^&*()+{}:\"|<>?\[\];'\,./`~]+)", tweet["text"])
            ),
            ...
            "author_timezone_UTC_offset": int(
                (
                    int(tweet["from_user_utcoffset"])
                    if tweet["from_user_utcoffset"]
                    else 0
                )
                / 60
                / 60
    ...
    def collect_tcat_metadata(cls):
        ...
        for instance in instances:
            ...
            # try:
            instance_bins = db.fetchall(
                "SELECT id, querybin, type from tcat_query_bins"
            )
            # except:
            ...
            for instance_bin in instance_bins:
                ...
                tweet_count = db.fetchone(
                    "SELECT COUNT(id) from " + instance_bin["querybin"] + "_tweets"
                )["COUNT(id)"]
                bin_data["tweet_count"] = tweet_count

                # Collect first and last tweet datetimes
                first_tweet_datetime = db.fetchone(
                    "SELECT created_at from "
                    + instance_bin["querybin"]
                    + "_tweets ORDER BY created_at ASC LIMIT 1"
                )["created_at"]
                last_tweet_datetime = db.fetchone(
                    "SELECT created_at from "
                    + instance_bin["querybin"]
                    + "_tweets ORDER BY created_at DESC LIMIT 1"
                )["created_at"]
