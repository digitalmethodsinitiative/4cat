...
class SearchWithinTCATBins(Search):
    ...
    options = {
        ...
        "divider-1": {"type": UserInput.OPTION_DIVIDER},
        "bin": {"type": UserInput.OPTION_INFO, "help": "Query bin"},
        "query": {
        ...
        "exclude-replies": {
            ...
            "options": {"exclude": "Exclude replies", "include": "Include replies"},
            "help": "Reply tweets",
            "default": "include",
    ...
    def collect_all_bins(cls, force_update=False):
        ...
        for instance in instances:
            ...
            if (
                force_update
                or instance not in cls.bin_data["last_collected"]
                or datetime.datetime.now() - datetime.timedelta(days=1)
                >= cls.bin_data["last_collected"][instance]
            ):
                # Collect Instance data
                try:
                    api_request = requests.get(api_url, timeout=5)
                    instance_bins = json.loads(api_request.content)
                    cls.bin_data["all_bins"][instance] = {
                        k: instance_bins[k] for k in sorted(instance_bins)
                    }
    ...
    def get_options(cls, parent_dataset=None, user=None):
        ...
        if all(
            [
                ...
                for instance, data in cls.bin_data["all_bins"].items()
            ]
        ):
            options["bin"] = {
                "type": UserInput.OPTION_INFO,
    ...
    def get_items(self, query):
        ...
        for available_instance in available_instances:
            hostname = (
                re.sub(r"https?://", "", available_instance)
                .split("@")
                .pop()
                .rstrip("/")
            )
        ...
        if not instance_url:
            return self.dataset.finish_with_error(
                "Invalid DMI-TCAT instance name '%s'" % bin_host
            )
        ...
        try:
            ...
            return self.dataset.finish_with_error(
                f"Lost connection to TCAT instance {bin_host}"
            )
        # Add TCAT metadata to dataset
        self.dataset.tcat_bin_data = current_bin
        if current_bin.get("type") in ["follow", "track", "timeline", "geotrack"] and (
            "phrase_times" not in current_bin or not "user_times" not in current_bin
        ):
            self.dataset.update_status(
                "Warning: TCAT not updated to send phrase and user time ranges; consider updating if you would like to retain this BIN metadata."
            )
        ...
        if self.parameters.get("min_date"):
            ...
            start_date = datetime.datetime.strptime(
                first_tweet_timestamp, "%Y-%m-%d %H:%M:%S"
            ).strftime("%Y-%m-%d")

        end_date = (
            datetime.datetime.fromtimestamp(self.parameters.get("max_date")).strftime(
                "%Y-%m-%d"
            )
        ...
        parameters = {
            ...
            "enddate": end_date,
            "replyto": "yes"
            if self.parameters.get("exclude-replies") == "exclude"
            else "no",
            "whattodo": "",
        ...
        if response.status_code != 200:
            return self.dataset.finish_with_error(
                "Query bin not available: received HTTP Error %i" % response.status_code
            )
        ...
        for chunk in response.iter_content(chunk_size=1024):
            ...
            for line in lines:
                ...
                dummy_file = io.TextIOWrapper(
                    io.BytesIO(line.replace(b"\0", b"")), encoding=encoding
                )
                reader = csv.reader(
                    dummy_file,
                    delimiter=",",
                ...
                if row_data and not fieldnames:
                    ...
                    if items % 250 == 0:
                        self.dataset.update_status(
                            "Loaded %i tweets from bin %s@%s"
                            % (items, bin_name, bin_host)
                        )
                    ...
                    try:
                        ...
                        self.dataset.log(
                            f"Error converting TCAT tweet ({items}) to APIv2 format: {e}"
                        )
                        api_map_errors += 1
                        continue
                    ...
                    try:
                        ...
                        self.dataset.log(
                            f"Error mapping TCAT tweet ({items}) to 4CAT Twitter format: {e}"
                        )
                        mapping_errors += 1
        ...
        if mapping_errors or api_map_errors:
            ...
            if mapping_errors:
                error_message += (
                    f"{mapping_errors} tweets were unable to be imported from TCAT. "
                )
            if api_map_errors:
                error_message += f"{api_map_errors} tweets were unable to be formmated corrected and 4CAT will not be able to analyse them."
            self.log.warning(
                f"SearchWithinTCATBins: import mapping issue detected ({self.dataset.key})"
            )
    ...
    def tcat_to_4cat_time(tcat_time):
        ...
        try:
            ...
            return datetime.datetime.fromtimestamp(tcat_time).strftime(
                "%Y-%m-%dT%H:%M:%S.000Z"
            )
        except ValueError:
            return datetime.datetime.strptime(tcat_time, "%Y-%m-%d %H:%M:%S").strftime(
                "%Y-%m-%dT%H:%M:%S.000Z"
            )
    ...
    def tcat_to_APIv2(tcat_tweet):
        ...
        urls = [
            url.strip()
            for url in (
                tcat_tweet["urls_expanded"].split(";")
                if tcat_tweet["urls_expanded"]
                else tcat_tweet["urls_followed"].split(";")
                if tcat_tweet["urls_followed"]
                else tcat_tweet["urls_followed"].split(";")
            )
            if url
        ]
        ...
        media_type = (
            "video"
            if tcat_tweet["media_id"] == "7"
            else "photo"
            if tcat_tweet["media_id"] == "3"
            else "animated_gif"
            if tcat_tweet["media_id"] == "16"
            else tcat_tweet["media_id"]
        )
        ...
        APIv2_tweet = {
            ...
            if tcat_tweet["possibly_sensitive"] == 1
            else False
            if tcat_tweet["possibly_sensitive"] == 0
            else None,  # bool
            ...
            "created_at": SearchWithinTCATBins.tcat_to_4cat_time(
                tcat_tweet["time"]
            ),  # str
            ...
            "author_user": {
                ...
                "verified": True
                if tcat_tweet["from_user_verified"] == 1
                else False
                if tcat_tweet["from_user_verified"] == 0
                else None,  # bool
                "created_at": SearchWithinTCATBins.tcat_to_4cat_time(
                    tcat_tweet["from_user_created_at"]
                )
                if tcat_tweet["from_user_created_at"]
                else "",  # str; may be Missing in TCAT data
                ...
                "withheld": {
                    "country_codes": tcat_tweet["from_user_withheld_scope"].split(
                        ";"
                    ),  # list; TODO TCAT has column, but have not seen it populated in testing... This is guess
                },
                ...
                "media_keys": [
                    {
                        ...
                        ),  # str; TCAT does not have the URL though it may be in the list of URLs
                        "variants": [
                            {
                                "url": ",".join(
                                    [
                        ...
                        ],  # list; This is not the expected direct link to video, but it is a URL to the video
            ...
            "geo": {
                ...
                "coordindates": {
                    ...
                    "coordinates": [
                        tcat_tweet["lng"],
                        tcat_tweet["lat"],
                    ],  # list i.e. [longitude, latitude]
                },
            },
            "withheld": {
                "copyright": True
                if tcat_tweet["withheld_copyright"] == 1
                else False
                if tcat_tweet["withheld_copyright"] == 0
                else None,  # bool; TODO TCAT has column, but have not seen it populated in testing... This is guess
                "country_codes": tcat_tweet["withheld_scope"].split(
                    ";"
                ),  # list; TODO TCAT has column, but have not seen it populated in testing... This is guess
            },
        }
            ...
            referenced_tweets.append(
                {
                    ...
                    ],  # str; Missing in TCAT data
                }
            )
            # These should NOT be None in case a processor/user attempts to identify a reply using these
            APIv2_tweet["in_reply_to_user_id"] = (
                "UNKNOWN"  # str; Missing from TCAT data
            )
            APIv2_tweet["in_reply_to_user"] = {
                "username": "UNKNOWN"
            }  # dict; Missing from TCAT data
    ...
    def validate_query(query, request, user):
        ...
        if not query.get("bin", "").strip():
            raise QueryParametersException(
                "You must choose a query bin to get tweets from."
            )
