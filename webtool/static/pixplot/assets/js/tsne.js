...
Data.prototype.load = function() {
  get(getPath(config.data.dir + '/' + config.data.file, 'data'),
    function(json) {
      config.data.output_directory = json.output_directory;
      get(getPath(json.imagelist, 'manifest'), function(data) {
...
Data.prototype.parseManifest = function(json) {
  ...
  get(getPath(layout.getLayoutPath(), 'manifest'), function(data) {
    this.addCells(data);
...
function Atlas(obj) {
  ...
  this.onLoad = obj.onLoad;
  this.image = null;
  this.url = getPath(data.json.atlas_dir + '/atlas-' + this.idx + '.jpg', 'manifest');
  this.load();
}
...
Layout.prototype.set = function(layout, enableDelay) {
  ...
  setTimeout(function() {
    get(getPath(this.getLayoutPath(), 'manifest'), function(pos) {
...
Layout.prototype.setText = function() {
  ...
  if (path && text.mesh) {
    get(getPath(path, 'manifest'), text.formatText.bind(text));
...
World.prototype.getHeightMap = function(callback) {
  ...
  img.src = this.heightmap || getPath('assets/images/heightmap.jpg', 'assets');
}
...
Lasso.prototype.getSelectedMetadata = function(callback) {
  ...
  if (data.json.metadata) {
    for (var i=0; i<images.length; i++) {
      var metadata = {};
      get(getPath(config.data.dir + '/metadata/file/' + images[i] + '.json', 'data'), function(data) {
...
Lasso.prototype.downloadSelected = function() {
  ...
  this.getSelectedMetadata(function(arr) {
    if (filetype == 'csv' || filetype == 'json') downloadFile(arr, filename);
      ...
      var nAdded = 0;
      for (var i=0; i<images.length; i++) {
        var imagePath = getPath('data/originals/' + images[i], 'data');
...
Dates.prototype.load = function() {
  get(getPath(config.data.dir + '/metadata/dates.json', 'data'), function(json) {
...
Lines.prototype.plot = function() {
  ...
  get(getPath('data/image-vectors/openpose/vectors.json', 'data'), function(json) {
    this.json = json;
    // draw lines
    var material = new THREE.RawShaderMaterial({
...
Modal.prototype.showCells = function(cellIndices, cellIdx) {
  ...
  var src = data.json.images_cropped
    ...
    : getPath(config.data.dir + '/originals/' + filename, 'data');
  ...
  image.onload = function() {
    ...
    get(getPath(config.data.dir + '/metadata/file/' + filename + '.json', 'data'), function(json) {
...
LOD.prototype.fetchNextImage = function() {
  ...
  if (Number.isInteger(cellIdx)) {
    ...
    // this image isn't in the cache, so load and cache it
      ...
      image.src = getPath(config.data.dir + '/thumbs/' + data.json.images[cellIdx], 'data');
    };
...
Filters.prototype.loadFilters = function() {
  ...
  get(getPath(url, 'data'), function(data) {
...
Filter.prototype.filterImages = function() {
  if (!this.selected) {
    ...
    var filename = this.selected.replace(/\//g, '-').replace(/ /g, '__') + '.json',
        path = getPath(config.data.dir + '/metadata/options/' + filename, 'data');
...
Hotspots.prototype.initialize = function() {
  get(getPath(data.json.custom_hotspots, 'manifest'), this.handleJson.bind(this),
    function(err) {
      get(getPath(data.json.default_hotspots, 'manifest'), this.handleJson.bind(this))
    }.bind(this)
  );
}
...
Globe.prototype.load = function() {
  ...
  get(getPath('assets/json/flat-continents.json', 'assets'), function(json) {
  ...
  get(getPath('assets/json/geographic-features.json', 'assets'), function(json) {
...
function getPath(path, type='data') {
  // var base = window.location.origin;
  // base += window.location.pathname.replace('index.html', '');
  var name = window.location.pathname.split('/').slice(-2, -1).pop()
  if (type == 'data') {
    var base = 'http://localhost/static/pixplots/' + name + '/';
    base += path.replace('\\','/')
  } else if (type == 'assets') {
    var base = 'http://localhost/static/pixplot/';
    base += path.replace('\\','/')
