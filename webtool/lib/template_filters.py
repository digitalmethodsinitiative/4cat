...
def _jinja2_add_colours(data):
    ...
    :return str:  HTML
    """
    if type(data) is not str or not re.match(
        r"#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})\b", data
    ):
        return data
...
def _jinja2_filter_4chan_image(image_4chan, post_id, board, image_md5):
    ...
    if board in plebs_boards:
        ...
        api_url = "https://archive.4plebs.org/_/api/chan/post/?board=%s&num=%s" % (
            board,
            post_id,
        )
        api_json = None
        ...
        img_page = requests.get(
            "https://img.4plebs.org/boards/%s/thumb/%s" % (board, thumb_link),
            headers=headers,
        )
    ...
    if board in archivedmoe_boards:
        img_page = requests.get(
            "https://archived.moe/files/%s/thumb/%s" % (board, thumb_link),
            headers=headers,
        )
        if img_page.status_code == 200:
            return img_page
    ...
    if "hashtag" in base_urls[datasource]:
        ...
        for tag in tags:
            ...
            body = re.sub(
                ...
                % (base_urls[datasource]["hashtag"] + tag[1:], tag),
                body,
            )
    ...
    if "mention" in base_urls[datasource]:
        ...
        for mention in mentions:
            body = re.sub(
                r"(?<!>)(" + mention + ")",
                "<a href='%s' target='_blank'>%s</a>"
                % (base_urls[datasource]["mention"] + mention[1:], mention),
                body,
            )
...
def inject_now():
    ...
    return {
        ...
        "__user_cp_access": any(
            [
                g.config.get(p)
                for p in config.config_definition.keys()
                if p.startswith("privileges.admin")
            ]
        ),
        "__version": version,
        "uniqid": uniqid,
