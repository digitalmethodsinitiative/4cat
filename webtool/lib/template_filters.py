...
def explorer_css(datasource, scope_class="explorer-content-container"):
    """
    Load Explorer CSS for specific data sources. If this doesn't exist,
     use the generic template. Automatically scope so it doesn't affect
     the Explorer metadata box and 4CAT header/footers.

    Usage: {{ datasource|explorer_css }}
    """
    css_name = f"explorer-template/{datasource}-explorer.css"

    # Check if datasource-specific template exists
    try:
        template = current_app.jinja_env.get_template(css_name)
        css_content = template.render()
    except TemplateNotFound:
        # Fallback to generic static CSS
        generic_css_path = os.path.join(
            current_app.root_path,
            'static',
            'css',
            'explorer-generic.css'
        )
        with open(generic_css_path, 'r') as f:
            css_content = f.read()
