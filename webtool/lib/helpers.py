...
class Pagination(object):
    ...
    def __init__(
        self, page, per_page, total_count, route="dataset.show_results", route_args=None
    ):
        """
        ...
        for year in range(first_year, last_year + 1):
            ...
            for month in range(start_month, end_month + 1):
                ...
                if not has_day:
                    ...
                    start_day = (
                        first_day if year == first_year and month == first_month else 1
                    )
                    end_day = (
                        last_day
...
def new_favicon_color(
    color, input_filepath="favicon-bw.ico", output_filepath="favicon.ico"
):
    """
...
def generate_css_colours(force=False):
    ...
    template_file = config.get("PATH_ROOT").joinpath(
        "webtool/static/css/colours.css.template"
    )
    ...
    with template_file.open() as infile:
        ...
        template = (
            template.replace("{{ main_colour }}", make_html_colour(main_colour))
    ...
    new_favicon_color(
        ...
        input_filepath=config.get("PATH_ROOT").joinpath(
            "webtool/static/img/favicon/favicon-bw.ico"
        ),
        output_filepath=config.get("PATH_ROOT").joinpath(
...
def format_chan_post(post):
    ...
    post = re.sub(
        r"&gt;&gt;([0-9]+)",
        '<span class="quote"><a href="#post-\\1">&gt;&gt;\\1</a></span>',
        post,
    )
    post = re.sub(
        r"^&gt;([^\n]+)",
