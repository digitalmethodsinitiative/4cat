...
def log_exception(e):
    ...
    if not status_code or status_code >= 500:
        ...
        current_app.log.error(
            f"{type(cause).__name__}{(' (' + request_url + ')') if request_url else ''}: {cause} at {tb}",
            frame=tb if tb else None,
        )
        return error(
...
def show_about():
    ...
    datasources = {
        k: v
        for k, v in g.modules.datasources.items()
        if k in g.config.get("datasources.enabled") and not v["importable"]
    }
    importables = {
        k: v
        for k, v in g.modules.datasources.items()
        if (v["importable"] and k in g.config.get("datasources.enabled"))
    }
...
def data_overview(datasource=None):
    ...
    datasources = {
        ...
        if metadata["has_worker"] and datasource in g.config.get("datasources.enabled")
    }
    ...
    if datasource:
        ...
        database_db_id = (
            worker_class.prefix if hasattr(worker_class, "prefix") else datasource_id
        )

        # Get description
        description_path = Path(
            datasources[datasource_id].get("path"), "DESCRIPTION.md"
        )
        ...
        is_static = (
            True
            if hasattr(worker_class, "is_static") and worker_class.is_static
            else False
        )
        ...
        if datasource_id not in ["upload"]:  # ignore upload as keys are variable
            example_keys = get_datasource_example_keys(
                db=g.db, modules=g.modules, dataset_type=datasource_id + "-search"
            )

        # Get daily post counts for local datasource to display in a graph
        if is_local == "local":
            total_counts = g.db.fetchall(
                "SELECT board, SUM(count) AS post_count FROM metrics WHERE metric = 'posts_per_day' AND datasource = %s GROUP BY board",
                (database_db_id,),
            )
            ...
            if total_counts:
                ...
                db_counts = g.db.fetchall(
                    "SELECT board, date, count FROM metrics WHERE metric = 'posts_per_day' AND datasource = %s",
                    (database_db_id,),
                )
                ...
                for board in boards:
                    ...
                    daily_counts[board] = [
                        v
                        for k, v in pad_interval(
                            daily_counts[board], first_interval=first_date
                        )[1].items()
                    ]

        references = (
            worker_class.references if hasattr(worker_class, "references") else None
        )
...
def show_page(page):
    ...
    return render_template(
        "page.html", body_content=page_parsed, body_class=page_class, page_name=page
    )
