...
def get_standalone_processors():
    ...
    class FakeDataset:
        ...
        parameters = {
            "datasource": "fake"
        }  # TODO: refactor how processors check module.parameters.get("datasource") to function call we can override here
    ...
    for processor_type, processor in g.modules.processors.items():
        ...
        if hasattr(
            processor, "is_compatible_with"
        ) and not processor.is_compatible_with(fake_dataset):
            continue
...
def process_standalone(processor):
    ...
    if not request.is_json:
        return error(
            402, error="This API endpoint only accepts JSON-formatted data as input"
        )
        ...
        for row in input:
            for field in required:
                if field not in row:
                    return error(
                        402,
                        error="Input is valid JSON, but not a list of data objects (missing field '%s')"
                        % field,
                    )
    except TypeError:
    ...
    if place_in_queue > 5:
        ...
        return error(
            code=503,
            error="Your request could not be handled as there are currently %i other jobs of this type in the queue. Please try again later."
            % place_in_queue,
        )
    ...
    while True:
        if time.time() > start + 30:
            ...
            return error(
                code=503,
                error="The server is currently too busy to handle your request. Please try again later.",
            )
