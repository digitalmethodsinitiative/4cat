...
from webtool.lib.helpers import (
    error,
    Pagination,
...
def frontpage():
    ...
    admin_privileges = g.config.get(
        [
            key
            for key in g.config.config_definition.keys()
            if key.startswith("privileges.admin")
        ]
    )
    ...
    num_items = {
        "day": g.db.fetchone(
            "SELECT SUM(num_rows) AS num FROM datasets WHERE timestamp > %s AND key_parent = '' AND (type LIKE '%%-search' OR type LIKE '%%-import')",
            (now - 86400,),
        )["num"],
        "week": g.db.fetchone(
            "SELECT SUM(num_rows) AS num FROM datasets WHERE timestamp > %s AND key_parent = '' AND (type LIKE '%%-search' OR type LIKE '%%-import')",
        ...
        "overall": g.db.fetchone(
            "SELECT SUM(num_rows) AS num FROM datasets WHERE key_parent = '' AND (type LIKE '%%-search' OR type LIKE '%%-import')"
        )["num"],
    }

    num_datasets = {
        "day": g.db.fetchone(
            "SELECT COUNT(*) AS num FROM datasets WHERE timestamp > %s", (now - 86400,)
        )["num"],
        "week": g.db.fetchone(
            "SELECT COUNT(*) AS num FROM datasets WHERE timestamp > %s",
            (now - (86400 * 7),),
        )["num"],
    ...
    disk_stats = {
        "data": g.db.fetchone(
            "SELECT count FROM metrics WHERE datasource = '4cat' AND metric = 'size_data'"
        ),
        "logs": g.db.fetchone(
            "SELECT count FROM metrics WHERE datasource = '4cat' AND metric = 'size_logs'"
        ),
        "db": g.db.fetchone(
            "SELECT count FROM metrics WHERE datasource = '4cat' AND metric = 'size_db'"
        ),
    }
    ...
    return render_template(
        "controlpanel/frontpage.html",
        flashes=get_flashed_messages(),
        stats={"captured": num_items, "datasets": num_datasets, "disk": disk_stats},
...
def list_users(page):
    ...
    if order == "age":
        ...
        order_bit = (
            "tags @> '[\"admin\"]' DESC, timestamp_token > 0 DESC, is_deactivated DESC"
        )

    num_users = g.db.fetchone(
        "SELECT COUNT(*) AS num FROM users " + filter_bit, replacements
    )["num"]
    users = g.db.fetchall(
    ...
    distinct_tags = set.union(
        *[set(u["tags"]) for u in g.db.fetchall("SELECT DISTINCT tags FROM users")]
    )
    distinct_users = [
        u["name"] for u in g.db.fetchall("SELECT DISTINCT name FROM users")
    ]

    pagination = Pagination(page, page_size, num_users, "admin.list_users")
    return render_template(
        "controlpanel/users.html",
        users=[User(g.db, user) for user in users],
        filter={"tag": tag, "name": filter_name, "sort": order},
...
def get_worker_status():
    ...
    if api_response["status"] != "success":
        return (
            """<p class="content-placeholder">Backend unavailable; View logs</p>""",
            200,
    ...
    workers = [
        {
            **worker,
            "dataset": None
            if not worker["dataset_key"]
            else DataSet(key=worker["dataset_key"], db=g.db),
        }
        for worker in api_response["response"]["running"]
    ]
    return render_template(
        "controlpanel/worker-status.html",
        workers=workers,
        worker_types=g.modules.workers,
        now=time.time(),
    )
...
def get_queue_status():
    ...
    return render_template(
        "controlpanel/queue-status.html",
        queue=queue,
        worker_types=g.modules.workers,
        now=time.time(),
    )
...
def list_jobs(page):
    ...
    jobs = g.queue.get_all_jobs(
        jobtype=filter_jobtype, restrict_claimable=False, limit=page_size, offset=offset
    )
    num_users = g.db.fetchall("SELECT COUNT(*) AS num FROM jobs WHERE jobtype != ''")[
        0
    ]["num"]

    # these are used for autocompletion in the filter form
    distinct_jobs = set.union(
        *[set(u["jobtype"]) for u in g.db.fetchall("SELECT DISTINCT jobtype FROM jobs")]
    )

    pagination = Pagination(page, page_size, num_users, "admin.list_jobs")
    return render_template(
        "controlpanel/jobs.html",
        jobs=jobs,
        filter={"jobtype": filter_jobtype, "sort": order},
        pagination=pagination,
        flashes=get_flashed_messages(),
        all_jobs=distinct_jobs,
        now=time.time(),
    )
...
def delete_job():
    ...
    if dataset:
        ...
        ) and not dataset.is_accessible_by(current_user, "owner"):
    ...
    try:
        call_api(
            "cancel-job",
            {
                "remote_id": job.data["remote_id"],
                "jobtype": job.data["type"],
                "level": BasicWorker.INTERRUPT_CANCEL,
            },
        )
...
def add_user():
        ...
        try:
            ...
            if user is None:
                response = {
                    **response,
                    **{
                        "message": "User was created but could not be instantiated properly."
                    },
                }
            else:
                try:
                    ...
                    response = {
                        **response,
                        **{
                            "message": "User was created but the registration e-mail could not be sent to them (%s)."
                            % e
                        },
                    }
            ...
            if not force:
                ...
                g.db.update(
                    "users",
                    data={"password": "", "timestamp_token": int(time.time())},
                    where={"name": username},
                )
                ...
                try:
                    ...
                    url = "%s://%s/reset-password/?token=%s" % (
                        protocol,
                        url_base,
                        new_token,
                    )
                    response = {
                        **response,
                        **{
                            "message": "Token was reset but registration e-mail could not be sent (%s). Reset password link: [%s](%s)"
                            % (e, url, url)
                        },
                    }
    ...
    if fmt == "html":
        if redirect_to_page:
            ...
            return redirect(
                url_for("admin.manipulate_user", mode="edit", name=username)
            )
        else:
            return render_template(
                "error.html",
...
def reject_user():
    ...
    if incomplete:
        if not form_message:
            form_answer = Path(
                g.config.get("PATH_ROOT"), "webtool/pages/reject-template.md"
            )
    ...
    try:
        ...
        return render_template(
            "error.html",
            message="Could not send e-mail to %s: %s" % (email_address, e),
...
def delete_user():
    ...
    if user.is_special:
        return render_template(
            "error.html",
            message=f"User {username} cannot be deleted.",
...
def manipulate_user(mode):
    ...
    if request.method == "POST":
        ...
        new_tags = [
            re.sub(r"[^a-z0-9_]", "", t.strip().lower())
            for t in request.form.get("tags", "").split(",")
        ]
        if "admin" in old_tags and "admin" not in new_tags:
            admin_users = g.db.fetchall(
                "SELECT name FROM users WHERE tags @> '[\"admin\"]'"
            )
            if len(admin_users) == 1:
                # one admin user that would no longer be an admin - not OK
                flash(
                    "There always needs to be at least one user with the 'admin' tag."
                )
        ...
        if not incomplete:
            ...
            if mode == "edit":
                g.db.update(
                    "users",
                    where={"name": request.form.get("current-name")},
                    data=user_data,
                )
                ...
                try:
                    ...
                    if request.form.get("password"):
                        ...
                        link = (
                            url_for("user.reset_password", _external=True)
                            + "?token=%s" % token
                        )
                        flash(
                            'User created. %s can set a password via<br><a href="%s">%s</a>.'
            ...
            if not incomplete and "autodelete" in request.form:
                ...
                if not autodelete:
                    ...
                    "[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}", autodelete
                ):
                    incomplete.append("autodelete")
                    flash("'Delete after' date must be in YYYY-MM-DD hh:mm format")

                else:
                    autodelete = datetime.datetime.strptime(
                        autodelete, "%Y-%m-%d %H:%M"
                    )
...
def manipulate_tags():
    ...
    all_tags = set.union(
        *[set(user["tags"]) for user in g.db.fetchall("SELECT tags FROM users")],
        set(
            [
                setting["tag"]
                for setting in g.db.fetchall("SELECT DISTINCT tag FROM settings")
                if setting["tag"]
            ]
        ),
    )
    ...
    tags.extend(
        [{"tag": tag, "explicit": False} for tag in all_tags if tag not in tag_priority]
    )
    ...
    for i, tag in enumerate(tags):
        tags[i]["users"] = g.db.fetchone(
            "SELECT COUNT(*) AS count FROM users WHERE tags != '[]' AND tags @> %s",
            ('["' + tag["tag"] + '"]',),
        )["count"]
        if tag["tag"] == "admin":
    ...
    if request.method == "POST":
        try:
            # no empty tags
            order = [
                tag for tag in request.form.get("order", "").split(",") if tag.strip()
            ]
            if not order:
                raise ValueError
...
def manipulate_settings():
    ...
    modules = {
        **{
            datasource + "-search": definition["name"]
            for datasource, definition in g.modules.datasources.items()
        },
        **{
            processor.type: processor.title
            if hasattr(processor, "title")
            else processor.type
            for processor in g.modules.processors.values()
        },
    }
    ...
    if request.method == "POST":
        try:
            ...
            new_settings = UserInput.parse_all(
                definition, request.form, silently_correct=False
            )
    ...
    for option in {*all_settings.keys(), *definition.keys()}:
        ...
        global_value = global_settings.get(
            option, definition.get(option, {}).get("default")
        )
    ...
    options = {
        ...
        for k in sorted(
            options,
            key=lambda o: (options[o]["tabname"], options[o].get("config_order", 0)),
        )
    }
    ...
    expire_override = []
    if all_settings.get("datasources.expiration") and global_settings.get(
        "datasources.expiration"
    ):
        expire_override = [
            datasource
            for datasource, settings in all_settings["datasources.expiration"].items()
            if settings != global_settings["datasources.expiration"].get(datasource)
        ]
...
def manipulate_notifications():
    ...
    if request.method == "POST":
        ...
        if params["expires-seconds"] and params["expires-date"]:
            flash(
                "Please specify either 'expires seconds' or 'expires date', not both."
            )
    ...
    notifications = g.db.fetchall(
        "SELECT * FROM users_notifications ORDER BY username ASC, id ASC"
    )
    return render_template(
        "controlpanel/notifications.html",
        incomplete=incomplete,
        flashes=get_flashed_messages(),
...
def get_log(logfile):
    ...
    if logfile == "backend":
        filename = (
            "4cat.log" if not g.config.get("USING_DOCKER") else "backend_4cat.log"
        )
    elif logfile == "stderr":
    ...
    log_file = (
        g.config.get("PATH_ROOT").joinpath(g.config.get("PATH_LOGS")).joinpath(filename)
    )
    if log_file.exists():
    ...
    if request.method == "POST" and request.files:
        ...
        if prospective_users:
            for user in prospective_users:
                ...
                exists = g.db.fetchone(
                    "SELECT name FROM users WHERE name = %s", (user["name"],)
                )
                if exists:
                ...
                g.db.insert(
                    "users",
                    {"name": user["name"], "timestamp_created": int(time.time())},
                )
                ...
                if user.get("expires"):
                    try:
                        ...
                        user_obj.set_value(
                            "delete-after", int(expires_after.timestamp())
                        )
                ...
                if user.get("password"):
                    ...
                    g.config.get("mail.server")
                    and not mail_fail
                    and "@" in user.get("name")
                ):
        ...
        if dupes:
            flash(
                f"The following users were skipped because the username already exists: {', '.join(dupes)}."
            )
        ...
        if failed_rows and prospective_users:
            flash(
                f"The following rows were skipped because the data in them was invalid: {', '.join([str(r) for r in failed_rows])}."
            )

    return render_template(
        "controlpanel/user-bulk.html",
        flashes=get_flashed_messages(),
        incomplete=incomplete,
    )
...
def dataset_bulk():
    ...
    forminput = {}
    datasources = {
        datasource: meta["name"] for datasource, meta in g.modules.datasources.items()
    }
    ...
    if request.method == "POST":
        ...
        try:
            forminput["filter_date_from"] = (
                datetime.datetime.strptime(
                    forminput["filter_date_from"], "%Y-%m-%d"
                ).timestamp()
                if forminput.get("filter_date_from")
                else None
            )
            forminput["filter_date_to"] = (
                datetime.datetime.strptime(
                    forminput["filter_date_to"], "%Y-%m-%d"
                ).timestamp()
        ...
        if forminput.get("filter_datasource"):
            ...
            where.append(
                "parameters::json->>'datasource' IS NOT NULL AND parameters::json->>'datasource' IN %s"
            )
            replacements.append(tuple(forminput["filter_datasource"]))

        datasets_meta = g.db.fetchall(
            f"SELECT * FROM datasets {'WHERE' if where else ''} {' AND '.join(where)}",
            tuple(replacements),
        )
        ...
        if action == "owner":
            ...
            if not bulk_owner.startswith("tag:"):
                users = g.db.fetchall(
                    "SELECT name FROM users WHERE name LIKE %s", (bulk_owner,)
                )
                if not users:
        ...
        if not incomplete:
            datasets = [
                DataSet(data=dataset, db=g.db, modules=g.modules)
            ...
            if action == "export":
                ...
                response.headers["Content-Disposition"] = (
                    f"attachment; filename=dataset-export-{exporttime}.csv"
                )
                return response
    ...
    return render_template(
        "controlpanel/dataset-bulk.html",
        flashes=get_flashed_messages(),
        incomplete=incomplete,
        form=forminput,
