...
component = Blueprint("scheduler", __name__)


@component.route('/scheduler/', defaults={'page': 1})
@component.route('/scheduler/page/<int:page>/')
@login_required
...
def show_scheduler(page):
    ...
    Scheduler.ensure_database(g.db)
    ...
    scheduler_jobs = g.queue.get_all_jobs(jobtype="scheduler", restrict_claimable=False)
    ...
    if depth == "all":
        ...
        num_jobs = g.db.fetchone(query, replacements)["num"]
...
@component.route('/scheduler/<string:scheduler_id>', defaults={'page': 1})
@component.route('/scheduler/<string:scheduler_id>/page/<int:page>/')
@login_required
def view_scheduler_datasets(scheduler_id, page):
    ...
    results = g.db.fetchall(query, replacements)
    ...
    # Prepare pagination
    pagination = Pagination(page, page_size, len(results), route="scheduler.view_scheduler_datasets", route_args={"scheduler_id": scheduler_id})

    return render_template("scheduler_results.html", filter={}, depth="all",
                           datasets=datasets, pagination=pagination)

@component.route("/api/delete-job/", defaults={"job_id": None}, methods=["DELETE", "GET", "POST"])
@component.route("/api/delete-job/<string:job_id>/", methods=["DELETE", "GET", "POST"])
@login_required
def delete_job(job_id=None):
    ...
    if not g.config.get("privileges.admin.can_manipulate_all_datasets") and not (job_owner and current_user.get_id() == job_owner):
