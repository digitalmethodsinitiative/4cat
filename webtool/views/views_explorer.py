...
def explorer_dataset(dataset_key: str, page=1):
    ...
    annotation_fields = {
        field_id: field_items
        for field_id, field_items in annotation_fields.items()
    ...
    if not sort or (sort == "dataset-order" and not reverse):
        ...
        for row in dataset.iterate_items(
                warn_unmappable=False, get_annotations=False, offset=offset
        ):
            count += 1
        ...
        count = 0
        get_annotations = (
            True if sort in dataset.get_annotation_field_labels() else False
        )
