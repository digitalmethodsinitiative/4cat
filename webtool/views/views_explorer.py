...
def explorer_dataset(dataset_key: str, page=1):
    ...
    if dataset.is_private and not (
        g.config.get("privileges.can_view_all_datasets")
    ...
    if not sort or (sort == "dataset-order" and not reverse):
        ...
        for row in sort_and_iterate_items(
            dataset, sort, reverse=reverse, warn_unmappable=False
        ):
            count += 1
            if count <= offset:
                continue
    ...
    if template == "generic":
        posts_css = Path(
            g.config.get("PATH_ROOT"), "webtool/static/css/explorer/generic.css"
        )
    else:
        posts_css = Path(
            g.config.get("PATH_ROOT"),
            "webtool/static/css/explorer/" + datasource + ".css",
        )
...
@component.route(
    "/explorer/save_annotation_fields/<string:dataset_key>", methods=["POST"]
)
@api_ratelimit
@login_required
...
def explorer_save_annotation_fields(dataset_key: str):
    ...
    for field_id in field_keys:
        if "tohash" in field_id:
            new_field_id = hash_to_md5(
                dataset_key + annotation_fields[field_id]["label"]
            )
...
def has_datasource_template(datasource: str) -> bool:
    ...
    """
    css_exists = Path(
        g.config.get("PATH_ROOT"), "webtool/static/css/explorer/" + datasource + ".css"
    ).exists()
    html_exists = Path(
        g.config.get("PATH_ROOT"),
        "webtool/templates/explorer/datasource-templates/" + datasource + ".html",
    ).exists()
