    ...
    return jsonify(
        {
            ...
            "data": {
                api_id: "http"
                + ("s" if g.config.get("flask.https") else "")
                + "://"
                + g.config.get("flask.server_name")
                + "/api/spec/"
                + api_id
                + "/swagger.json"
...
def api_status():
    ...
    lockfile = Path(
        g.config.get("PATH_ROOT"), g.config.get("PATH_LOCKFILE"), "4cat.pid"
    )
    ...
    response = {
        "code": API_SUCCESS,
        "items": {
            "backend": {"live": backend_live, "queued": jobs_sorted},
            "frontend": {
                "live": True  # duh
            },
        },
...
def datasource_form(datasource_id):
    ...
    if not worker_class:
        return error(
            404, message="Datasource '%s' has no search worker" % datasource_id
        )
    ...
    if not worker_options:
        return error(
            404,
            message="Datasource '%s' has no dataset parameter options defined"
            % datasource_id,
        )
    ...
    is_local = (
        "local"
        if hasattr(worker_class, "is_local") and worker_class.is_local
        else "external"
    )
    is_static = (
        True if hasattr(worker_class, "is_static") and worker_class.is_static else False
    )
    ...
    form = render_template(
        "components/create-dataset-option.html", options=worker_options, labels=labels
    )
    html = render_template_string(
        form, datasource_id=datasource_id, datasource=datasource
    )
...
def import_dataset():
    ...
    if (
        not platform
        or platform not in g.modules.datasources
        or platform not in g.config.get("datasources.enabled")
    ):
    ...
    if pseudonymise_mode in ("pseudonymise", "anonymise"):
        filterable_fields = (
            worker.pseudonymise_fields
            if hasattr(worker, "pseudonymise_fields")
            else ["author*", "user*"]
        )
        dataset.next = [
            {
                "type": "author-info-remover",
                "parameters": {"mode": pseudonymise_mode, "fields": filterable_fields},
            }
        ]
...
def queue_dataset():
    ...
    if search_worker_id not in g.modules.workers:
        return error(
            404, message="Datasource '%s' has no search interface" % datasource_id
        )
    ...
    if hasattr(search_worker, "validate_query"):
        ...
        try:
            ...
            sanitised_query = UserInput.parse_all(
                search_worker.get_options(None, current_user),
                request.form,
            ...
            sanitised_query = search_worker.validate_query(
                sanitised_query, request, current_user
            )
            ...
            # re-submitted
            form = render_template(
                "components/create-dataset-option.html", options=e.config
            )
        ...
        raise NotImplementedError(
            "Data sources MUST sanitise input values with validate_query"
        )
    ...
    extension = (
        search_worker.extension if hasattr(search_worker, "extension") else "csv"
    )
...
def check_dataset():
    ...
    template = (
        "components/result-status.html"
        if block == "status"
        else "components/result-result-row.html"
    )
...
def nuke_dataset(key=None, reason=None):
    ...
    dataset.update_status(
        "Dataset cancelled by instance administrator. Reason: %s" % reason
    )
    dataset.finish(0)
...
@component.route(
    "/api/delete-dataset/", defaults={"key": None}, methods=["DELETE", "GET", "POST"]
)
...
def delete_dataset(key=None):
    ...
    # do we have a parent?
    parent_dataset = (
        DataSet(key=dataset.key_parent, db=g.db, modules=g.modules)
...
def remove_tag():
    ...
    tagged_users = g.db.fetchall(
        "SELECT * FROM users WHERE tags @> %s ", (json.dumps([tag]),)
    )
    all_tags = list(
        set(
            itertools.chain(
                *[u["tags"] for u in g.db.fetchall("SELECT DISTINCT tags FROM users")]
            )
        )
    )
    all_tags += [
        s["tag"]
        for s in g.db.fetchall(
            "SELECT DISTINCT tag FROM settings WHERE tag LIKE 'user:%'"
        )
    ]

    for user in tagged_users:
        user = User.get_by_name(g.db, user["name"])
        user.remove_tag(tag)
    ...
    configured_tags = [
        t["tag"] for t in g.db.fetchall("SELECT DISTINCT tag FROM settings")
    ]
...
def add_dataset_owner(key=None, username=None, role=None):
    ...
    for username in usernames.split(","):
        ...
        new_owner = User.get_by_name(g.db, username)
        if new_owner is None and not username.startswith("tag:"):
            return error(
                404,
                error=f"The user '{username}' does not exist. Use tag:example to add a tag as an owner.",
            )
    ...
    if request.args.get("redirect") is not None:
        ...
        return jsonify({"status": "success", "key": dataset.key, "html": html})
...
def remove_dataset_owner(key=None, username=None):
    ...
    owner = User.get_by_name(g.db, username)
...
def check_search_queue():
    ...
    """
    unfinished_jobs = g.db.fetchall(
        "SELECT jobtype, COUNT(*)count FROM jobs WHERE jobtype LIKE '%-search' GROUP BY jobtype ORDER BY count DESC;"
    )
...
def toggle_favourite(key):
    ...
    current_status = g.db.fetchone(
        "SELECT * FROM users_favourites WHERE name = %s AND key = %s",
        (current_user.get_id(), dataset.key),
    )
    if not current_status:
        g.db.insert(
            "users_favourites", data={"name": current_user.get_id(), "key": dataset.key}
        )
    else:
        g.db.delete(
            "users_favourites",
            where={"name": current_user.get_id(), "key": dataset.key},
        )
...
def queue_processor(key=None, processor=None):
    ...
    available_processors = dataset.get_available_processors(
        user=current_user, exclude_hidden=True
    )
    ...
    try:
        sanitised_query = UserInput.parse_all(
            processor_worker.get_options(dataset, current_user),
            request.form,
            silently_correct=False,
        )
        ...
        children.append(
            {
                ...
                "resultrow_html": render_template(
                    "components/result-result-row.html", dataset=top_parent
                ),
...
def export_packed_dataset(key=None, component=None):
    ...
    if component == "metadata":
        ...
        children = [
            d["key"]
            for d in g.db.fetchall(
                "SELECT key FROM datasets WHERE key_parent = %s AND is_finished = TRUE",
                (dataset.key,),
            )
        ]
        return jsonify(children)
        ...
        filepath = (
            ...
            if component == "data"
            else dataset.get_results_path().with_suffix(".log")
        )
